...
use hangar_mutation::{
    create_backup, ensure_journal_schema, inspect_local_mutation_file, load_verified_backup,
    permanent_delete_entry, quarantine, restore_entry, BackupItem, BackupLevel, BackupRequest,
    ConfirmAction, ConfirmTokenStore, ItemOutcome, PurgeError, QuarantineItem, QuarantineRequest,
    RestoreOutcome,
};
...
fn item(source: &Path, relative: &str) -> QuarantineItem {
    let (expected_source_stamp, backup_hash) = inspect_local_mutation_file(source).unwrap();
    QuarantineItem {
...
fn backed_quarantine(
            ...
            items: rels
                ...
                .map(|rel| {
                    ...
                    let (expected_source_stamp, expected_source_hash) =
                        inspect_local_mutation_file(&source).unwrap();
                    BackupItem {
                        source,
    ...
    let items = rels
        ...
        .map(|rel| {
            let src = project.join(rel);
            let copy = verified.copy_for(&src.to_string_lossy()).unwrap();
            QuarantineItem {
                source: src.clone(),
                relative: (*rel).to_string(),
                expected_source_stamp: copy.source_stamp().unwrap().clone(),
...
fn t04_locked_file_fails_cleanly_others_proceed() {
    ...
    let holding = dir.path().join("holding");
    let conn = journaled_conn();
    let result = quarantine(&conn, basic_request(&holding, reviewed)).unwrap();
...
// ---------------------------------------------------------------------------
// 8. Cross-volume (C: temp → D: holding): refuse before creating a held copy
// ---------------------------------------------------------------------------

#[cfg(windows)]
#[test]
fn t08_cross_volume_quarantine_fails_closed_without_metadata_complete_backup() {
    let dir = tempfile::tempdir().unwrap();
    let temp_drive = dir
        .path()
        .to_string_lossy()
    ...
    assert_eq!(result.moved, 0, "cross-volume request must not move bytes");
    assert_eq!(result.failed, 1, "the refused item must be explicit");
    assert_eq!(result.space_recovered, 0);
    assert_eq!(result.entries[0].outcome, ItemOutcome::Failed);
    assert!(result.entries[0]
        .detail
        .as_deref()
        .is_some_and(|detail| detail.contains("metadata-complete backup")));
    assert_eq!(
        fs::read(&file).unwrap(),
        payload,
    ...
    assert!(
        entries_for_op(&conn, result.operation_id).is_empty(),
        "a refused cross-volume request must not invent a restorable held entry"
    );
    assert_eq!(op_status(&conn, result.operation_id), "failed");
    assert!(
        !holding.exists(),
        "the destination volume must remain untouched when the safety precondition is absent"
    );
    println!(
        "t08 OK: cross-volume quarantine refused without metadata-complete backup; source intact"
    );
}

// ---------------------------------------------------------------------------
// 9. Legacy purge authority stays fail-closed
// ---------------------------------------------------------------------------

#[test]
fn t09_legacy_purge_authority_stays_fail_closed() {
    let dir = tempfile::tempdir().unwrap();
    let conn = journaled_conn();
    ...
    let (entry_b, held_b, _payload_b) = &backed[1];
    ...
    let wrong_action = tokens.issue(ConfirmAction::EnterMutationMode).unwrap();
    assert!(matches!(
    ...
    // (c) Action-only permanent-delete grants are deliberately impossible.
    // The shipping executor requires a fresh immutable preview plus exact
    // topology-group binding, which this compatibility command cannot carry.
    assert!(tokens.issue(ConfirmAction::PermanentDelete).is_err());
    assert!(matches!(
        permanent_delete_entry(&conn, &tokens, "unscoped-token-was-never-issued", *entry_b),
        Err(PurgeError::ConfirmRequired)
    ));
    assert!(held_a.exists());
    ...
    // The already-authorized historical body's backup/identity/exact-handle
    // invariants remain covered inside purge.rs unit tests. It is intentionally
    // private so an integration or production caller cannot bypass the batch grant.
    println!(
        "t09 OK: missing, wrong-action and action-only permanent-delete grants all fail closed"
    );
}
