...
pub(crate) enum FsMoveError {
    ...
    #[error("refusing to move cloud-backed source: {path}")]
    SourceIsCloudFile { path: String },
    #[error("refusing to move a reparse source: {path}")]
    SourceIsReparse { path: String },
}
...
pub(crate) fn move_path(
    ...
    // Last-line no-follow check shared by restore and crash recovery. This is
    // deliberately inside the move primitive so a caller-side TOCTOU change to a
    // Cloud File or another reparse tag cannot reach rename/copy/remove_file.
    let identity = hangar_fs::inspect_path_identity(from);
    if crate::is_cloud_reparse_kind(identity.reparse_kind.as_deref()) {
        return Err(FsMoveError::SourceIsCloudFile {
            path: from.to_string_lossy().to_string(),
        });
    }
    if identity.is_reparse {
        return Err(FsMoveError::SourceIsReparse {
            path: from.to_string_lossy().to_string(),
        });
    }
