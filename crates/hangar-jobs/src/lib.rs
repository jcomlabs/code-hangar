...
pub enum RunningJobAdmission {
    Created(String),
    Existing { job_id: String, root_id: i64 },
}
...
impl JobStore {
    ...
    /// Atomically create a running job unless one of the requested roots is
    /// already owned by a running/cancelling job. The check and insertion share
    /// the same jobs-map lock, so concurrent scan requests cannot both reserve
    /// the same root.
    pub fn admit_running_for_roots_with_estimate(
        &self,
        message: impl Into<String>,
        root_ids: Vec<i64>,
        root_paths: Vec<String>,
        ...
        let mut jobs = self.jobs.lock().expect("job mutex poisoned");
        for (job_id, status) in jobs.iter() {
            if !matches!(status.state.as_str(), "running" | "cancelling") {
                continue;
            }
            if let Some(root_id) = root_ids
                .iter()
                .copied()
                .find(|root_id| status.root_ids.contains(root_id))
            {
                return RunningJobAdmission::Existing {
                    job_id: job_id.clone(),
                    root_id,
                };
            }
        ...
        prune_terminal_jobs(&mut jobs);
        // Publish the cancellation token before the status becomes visible.
        // Otherwise a caller that reused the just-admitted job could request
        // cancellation in the small gap and the worker would miss it.
        self.cancels
            .lock()
            .expect("job cancel mutex poisoned")
            .insert(job_id.clone(), Arc::new(AtomicBool::new(false)));
        jobs.insert(job_id.clone(), status);
        RunningJobAdmission::Created(job_id)
    }
    ...
    pub fn running_job_id_for_root(&self, root_id: i64) -> Option<String> {
        self.jobs
            .lock()
            .expect("job mutex poisoned")
            .iter()
...
#[cfg(test)]
mod tests {
    use super::{JobStore, RunningJobAdmission};
    ...
    #[test]
    fn concurrent_admission_reserves_each_root_once() {
        let jobs = JobStore::default();
        ...
        let handles = (0..8)
            .map(|_| {
                ...
                std::thread::spawn(move || {
                    barrier.wait();
                    jobs.admit_running_for_roots_with_estimate(
                        "Scanning.",
                        vec![42],
                        vec![r"C:\shared-root".to_string()],
                        None,
                    )
                })
            })
        ...
        assert_eq!(
            admissions
                .iter()
                .filter(|admission| matches!(admission, RunningJobAdmission::Created(_)))
                .count(),
            1
        );
        let created_id = admissions
            .iter()
            .find_map(|admission| match admission {
                RunningJobAdmission::Created(job_id) => Some(job_id.as_str()),
                RunningJobAdmission::Existing { .. } => None,
            })
            .unwrap();
        assert!(admissions.iter().all(|admission| match admission {
            RunningJobAdmission::Created(job_id) => job_id == created_id,
            RunningJobAdmission::Existing { job_id, root_id } => {
                job_id == created_id && *root_id == 42
            }
        }));
        assert_eq!(
            jobs.running_job_id_for_root(42).as_deref(),
            Some(created_id)
        );
    }
}
