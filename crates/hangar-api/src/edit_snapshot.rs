...
pub(crate) fn compare_snapshot(
    ...
    let (node_id, project_id, recorded_path, backup_id, before_hash) = db
        ...
        .map_err(super::to_message)?;
    let (current_path, project_paths) = super::resolve_editable_inventory_target(state, node_id)?;
    super::validate_editable_disk_target(&current_path, &project_paths)?;
...
fn snapshot_and_write_with_conn(
    ...
    validate_edit_content(content)?;
    let origin = validate_origin(origin)?;
    hangar_mutation::validate_local_mutation_path(target)
        .map_err(|error| format!("Not saved: target is not a proven local path ({error})."))?;
    hangar_mutation::validate_local_mutation_path(snapshot_root)
        .map_err(|error| format!("Not saved: snapshot root is not local ({error})."))?;
    let (expected_source_stamp, bound_before_hash) =
        hangar_mutation::inspect_local_mutation_file(target).map_err(|error| {
            format!("Not saved: target identity could not be proven ({error}).")
        })?;
    ...
    if before_hash != bound_before_hash {
        return Err(
            "Not saved: the file changed while its pre-edit identity was being proven. Reload it before applying this edit."
                .to_string(),
        );
    }
    ...
    let backup = hangar_mutation::create_backup(
        ...
        hangar_mutation::BackupRequest {
            ...
            items: vec![hangar_mutation::BackupItem {
                ...
                expected_source_hash: bound_before_hash.clone(),
            }],
    ...
    let written_hash = hangar_mutation::inspect_local_mutation_file(target)
        .map(|(_, hash)| hash)
