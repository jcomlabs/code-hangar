...
pub(crate) fn mark_project_reviewed(
    ...
    let undated_session_fingerprint =
        validate_undated_session_fingerprint(undated_session_fingerprint)?;
    let project = state
        .db()?
        .project_get(project_id)
        .map_err(super::to_message)?
...
fn validate_undated_session_fingerprint(value: Option<&str>) -> Result<Option<&str>, String> {
    ...
    let valid_hash = |part: Option<&str>| {
        part.is_some_and(|part| {
            part.len() == 8 && part.bytes().all(|byte| byte.is_ascii_hexdigit())
        })
    };
    let valid_count = count.is_some_and(|count| {
        !count.is_empty()
            && count.len() <= 10
    ...
    if version != Some("v1")
        || !valid_count
        || !valid_hash(primary)
        || !valid_hash(secondary)
        || parts.next().is_some()
    {
        return Err("The undated-session checkpoint fingerprint is malformed.".to_string());
    }
    Ok(Some(value))
}
...
fn require_read_only_git_command(args: &[&str]) -> Result<(), String> {
    ...
    if matches!(subcommand, "diff" | "status" | "rev-parse" | "remote") {
        return Ok(());
    }
    Err(format!(
...
/// Bounded, read-only worktree dirtiness probe for the objective Safe Manage
/// pass. This reuses the same hostile-repository hardening as Recap: no hooks,
/// external diff, pager, prompts, optional locks, global config or network
/// subcommand. Unknown/error remains unknown to the caller and is never treated
/// as a clean worktree.
pub(crate) fn project_git_dirty(root: &Path) -> Result<bool, String> {
    if !root.is_dir() || !root.join(".git").exists() {
        return Err("This project has no local Git working tree.".to_string());
    }
    let output = run_git(
        root,
        &["status", "--porcelain=v1", "--untracked-files=normal"],
    )?;
    Ok(output.truncated || !output.text.trim().is_empty())
}

/// Bounded, read-only probe for whether the current local Git configuration
/// records at least one remote. `git remote` reads names only: it does not
/// contact a remote, resolve DNS or expose remote URLs. The same hostile-repo
/// process hardening and output cap used by the working-tree probe apply.
pub(crate) fn project_git_has_remote(root: &Path) -> Result<bool, String> {
    if !root.is_dir() || !root.join(".git").exists() {
        return Err("This project has no local Git working tree.".to_string());
    }
...
fn compare_file_change(
    ...
    let content = match hangar_fs::read_local_text_no_recall(&target, REALITY_READ_CAP) {
...
mod tests {
    ...
    fn local_git_runner_accepts_only_the_read_only_subcommands() {
        ...
        for subcommand in [
            ...
            "rebase", "reset", "restore", "switch", "tag",
        ] {
    ...
    fn retained_ledger_respects_the_review_checkpoint_cutoff() {
        let checkpoint = ProjectReviewCheckpoint {
            ...
            undated_session_fingerprint: None,
    ...
    #[test]
    fn undated_session_fingerprint_accepts_only_the_bounded_wire_format() {
        assert_eq!(
            validate_undated_session_fingerprint(Some("v1:2:0123abcd:89abcdef")),
            Ok(Some("v1:2:0123abcd:89abcdef"))
        );
        assert_eq!(validate_undated_session_fingerprint(None), Ok(None));
        for malformed in [
            "",
            "v1:0:0123abcd:89abcdef",
            "v2:2:0123abcd:89abcdef",
            "v1:2:0123abc:89abcdef",
            "v1:2:0123abcd:89abcdeg",
            "v1:2:0123abcd:89abcdef:extra",
        ] {
            assert!(validate_undated_session_fingerprint(Some(malformed)).is_err());
        }
    }
