...
impl PerformanceMode {
    pub fn parse(value: Option<&str>) -> Self {
        match value.unwrap_or_default().to_ascii_lowercase().as_str() {
            "background" | "resident" | "quiet" => Self::Background,
    ...
    pub fn batch_pause(self) -> Option<std::time::Duration> {
        self.is_background()
    ...
    fn process_priority_label(self) -> &'static str {
        match self {
            Self::Background => "below normal on the resident scan thread",
...
pub struct PerformanceScope {
    ...
    background_thread_priority: Option<i32>,
}
...
impl PerformanceScope {
    pub fn enter(mode: PerformanceMode) -> Self {
        ...
        let background_thread_priority = mode
            .is_background()
            .then(platform::enter_background_thread)
            .flatten();
        Self {
            mode,
...
impl Drop for PerformanceScope {
    fn drop(&mut self) {
        ...
        if self.mode.is_background() {
            platform::leave_background_thread(self.background_thread_priority);
        }
    }
}
...
pub fn scan_limits(resume: bool, mode: PerformanceMode) -> ScanLimits {
    ...
    if mode.is_background() {
        ...
        limits.build_dir_batch_pause = mode.batch_pause();
    } else if mode.is_max() {
...
fn notes_for_mode(mode: PerformanceMode) -> Vec<String> {
    match mode {
        PerformanceMode::Background => vec![
            "Resident scans use one worker, small batches and a duty-cycle pause.".to_string(),
            "The scan thread runs below normal priority on Windows.".to_string(),
        ],
...
mod platform {
    ...
    use windows_sys::Win32::System::Threading::{
        GetCurrentProcess, GetCurrentThread, GetPriorityClass, GetProcessTimes, GetThreadPriority,
        SetPriorityClass, SetThreadPriority, ABOVE_NORMAL_PRIORITY_CLASS, NORMAL_PRIORITY_CLASS,
        THREAD_PRIORITY_BELOW_NORMAL,
    };

    static CPU_SAMPLE: Mutex<Option<(u64, Instant)>> = Mutex::new(None);
    const THREAD_PRIORITY_ERROR_RETURN_VALUE: i32 = i32::MAX;
    ...
    pub fn enter_background_thread() -> Option<i32> {
        let thread = unsafe { GetCurrentThread() };
        let previous = unsafe { GetThreadPriority(thread) };
        if previous == THREAD_PRIORITY_ERROR_RETURN_VALUE {
            return None;
        }
        let changed = unsafe { SetThreadPriority(thread, THREAD_PRIORITY_BELOW_NORMAL) };
        (changed != 0).then_some(previous)
    }

    pub fn leave_background_thread(previous: Option<i32>) {
        if let Some(previous) = previous {
            let thread = unsafe { GetCurrentThread() };
            let _ = unsafe { SetThreadPriority(thread, previous) };
        }
    }
...
mod platform {
    ...
    pub fn leave_background_thread(_previous: Option<i32>) {}
    pub fn memory_status() -> MemoryStatus {
        MemoryStatus {
            total_memory_bytes: None,
            available_memory_bytes: None,
...
mod tests {
    ...
    fn parses_unknown_modes_as_balanced() {
        ...
        assert_eq!(
            PerformanceMode::parse(Some("resident")),
            PerformanceMode::Background
        );
    }

    #[test]
    fn background_uses_one_small_throttled_worker() {
        let background = scan_limits(false, PerformanceMode::Background);
        let balanced = scan_limits(false, PerformanceMode::Balanced);
        assert_eq!(background.worker_count, 1);
        assert!(!background.measure_build_dir_sizes);
        assert_eq!(background.batch_size, 32);
        assert_eq!(
            background.build_dir_batch_pause,
            Some(std::time::Duration::from_secs(1))
        );
        assert!(background.batch_size < balanced.batch_size);
        assert_eq!(
            PerformanceMode::Background.batch_pause(),
            Some(std::time::Duration::from_secs(1))
        );
    }
