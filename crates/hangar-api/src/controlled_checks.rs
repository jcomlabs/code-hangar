...
const CONTROLLED_CHECK_POLICY_VERSION: &str = "controlled-check-v2-offline";
const RISK_DISCLOSURE: &str = "This runs code supplied by this project. A known command is not a sandbox and does not make the project harmless. Package managers are forced offline, but project code is not isolated from arbitrary sockets. Approval applies only to this project and this exact manifest fingerprint.";

/// Fixed environment inherited by every supported project check and its descendants. These
/// settings make the package/tool managers fail closed when required material is absent locally;
/// they are deliberately not described as a network sandbox because arbitrary project code can
/// ignore environment policy and open sockets of its own.
#[cfg(feature = "mutation")]
const CONTROLLED_CHECK_FIXED_ENV: &[(&str, &str)] = &[
    ("CI", "1"),
    ("NO_COLOR", "1"),
...
pub(crate) fn static_correction_check(
    ...
    let (path, project_paths) = super::resolve_editable_inventory_target(state, node_id)?;
    super::validate_editable_disk_target(&path, &project_paths)?;
...
fn reference_check(
    ...
    let relationships = super::node_relationships(state, project_id, node_id)?;
...
pub(crate) fn run_project_check(
    ...
    Ok(ControlledCheckRun {
        ...
        limits_summary: format!(
            "{}s wall timeout, {} MiB job memory, {} processes maximum, below-normal priority, 64 KiB retained per output stream. Package managers are forced offline; project code is not network-sandboxed.",
...
fn make_spec(
    ...
    for value in [
        CONTROLLED_CHECK_POLICY_VERSION,
        id,
        command_label,
        manifest_path,
        executable,
    ...
    // Offline execution policy is approval-relevant. Any future change to these settings must
    // invalidate old approvals instead of silently broadening an already-approved command.
    for (name, value) in CONTROLLED_CHECK_FIXED_ENV {
        hasher.update(&[0]);
...
#[cfg(feature = "mutation")]
fn configure_controlled_check_environment(
    command: &mut std::process::Command,
    inherited: impl IntoIterator<Item = (&'static str, std::ffi::OsString)>,
) {
    command
        .env_clear()
        .envs(inherited)
        .envs(CONTROLLED_CHECK_FIXED_ENV.iter().copied());
}
...
fn run_bounded_process(root: &Path, spec: &CommandSpec) -> Result<ProcessResult, String> {
    ...
    configure_controlled_check_environment(&mut command, inherited);
    command
        .args(&spec.args)
        .current_dir(root)
        .stdin(Stdio::null())
...
mod tests {
    ...
    #[cfg(feature = "mutation")]
    #[test]
    fn controlled_check_environment_forces_supported_managers_offline() {
        use std::collections::{BTreeMap, BTreeSet};
        use std::ffi::OsString;
        use std::process::Command;

        let names = CONTROLLED_CHECK_FIXED_ENV
            .iter()
            .map(|(name, _)| *name)
            .collect::<BTreeSet<_>>();
        assert_eq!(names.len(), CONTROLLED_CHECK_FIXED_ENV.len());

        let mut command = Command::new("project-check-fixture");
        configure_controlled_check_environment(
            &mut command,
            [("PATH", OsString::from("fixture-path"))],
        );
        ...
        for (name, expected) in [
            ...
            assert_eq!(configured.get(name).map(String::as_str), Some(expected));
        }
        assert_eq!(
            configured.get("PATH").map(String::as_str),
            Some("fixture-path")
        );
        assert!(RISK_DISCLOSURE.contains("not isolated from arbitrary sockets"));
        assert_eq!(
            CONTROLLED_CHECK_POLICY_VERSION,
            "controlled-check-v2-offline"
        );
    }
