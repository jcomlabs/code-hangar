...
pub enum AgentMethod {
    ...
    GetProjectGraph,
    /// A node's incoming/outgoing relationships in an explicit project membership
    /// (`projectId` + `nodeId`; cross-project edges are dropped). Needs `read_graph`
    /// + the exact project grant.
    NodeRelationships,
    /// Candidate orphan assets within a project. Needs `read_graph` + membership.
    ListOrphanAssets,
    /// Whether a node looks orphaned (and why) in an explicit project membership.
    /// Needs `read_graph` + the exact project grant.
    NodeOrphanStatus,
    ...
    /// Highest-friction connected-app gate + its separate final-remove authorization.
    /// This does not govern the local owner's project/batch cleanup flow.
