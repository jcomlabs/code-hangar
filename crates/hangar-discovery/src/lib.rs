...
const CLOUD_DEEP_ROOT_DEPTH: usize = 8;
const CLOUD_DEEP_ROOT_MAX_DIRS: usize = 10_000;
const CLOUD_DEEP_ROOT_BUDGET: Duration = Duration::from_secs(10);
...
// Project/app attribution runs on the project-list hot path. OpenCode is queried
// only through bounded metadata rows, while Oh My Pi is walked with explicit
// directory, entry, file, depth, time and per-file byte ceilings.
const OPENCODE_ATTRIBUTION_ROW_LIMIT: usize = 10_000;
const OMP_ATTRIBUTION_MAX_DEPTH: usize = 6;
const OMP_ATTRIBUTION_MAX_DIRS: usize = 512;
const OMP_ATTRIBUTION_MAX_ENTRIES: usize = 5_000;
const OMP_ATTRIBUTION_MAX_FILES: usize = 1_500;
const OMP_ATTRIBUTION_BUDGET: Duration = Duration::from_secs(2);
...
pub struct DiscoveryOptions {
    /// Maximum number of project candidates returned. Zero keeps every result
    /// produced by the already-bounded discovery pass.
    pub limit: usize,
    /// Maximum number of sessions returned, independently of the project limit.
    /// A shared cap used to hide whole providers once the project list reached
    /// its limit (for example OpenCode and Oh My Pi after 500 earlier rows).
    /// Zero keeps the complete, already-bounded local session inventory.
...
/// One immutable WSL view for a synchronous project-discovery request.
///
/// `system(false)` returns before consulting the distro cache, starting
/// `wsl.exe`, or probing a WSL share. `system(true)` resolves the distro homes
/// exactly once and the same homes are then used by source and registry
/// discovery. Controlled snapshots are for deterministic embedders/tests: they
/// contain only explicitly supplied local fixture homes and never authorize
/// probes of real WSL UNC paths.
#[derive(Debug, Clone)]
pub struct WslDiscoverySnapshot {
    homes: Vec<(String, PathBuf)>,
    allow_wsl_unc: bool,
    include_system_sources: bool,
}

impl WslDiscoverySnapshot {
    pub fn system(enabled: bool) -> Self {
        if !enabled {
            return Self {
                homes: Vec::new(),
                allow_wsl_unc: false,
                include_system_sources: true,
            };
        }

        #[cfg(windows)]
        let homes = with_wsl_scan_snapshot(true, wsl_distro_homes);
        #[cfg(not(windows))]
        let homes = Vec::new();

        Self {
            homes,
            allow_wsl_unc: cfg!(windows),
            include_system_sources: true,
        }
    }

    /// Construct a hermetic discovery view from caller-owned local fixture
    /// homes. This deliberately excludes host sources/registries and keeps WSL
    /// UNC probes denied even though the logical WSL source is selected.
    #[cfg(debug_assertions)]
    #[doc(hidden)]
    pub fn controlled(homes: Vec<(String, PathBuf)>) -> Self {
        Self {
            homes,
            allow_wsl_unc: false,
...
/// Progress from the dedicated, read-only session pass used by Safe Manage.
/// A source counts as processed even when its path is absent: the absence was
/// checked under the same fresh pass and is therefore part of its provenance.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SessionDiscoveryProgress {
...
/// Result of a dedicated session inventory. `complete` is deliberately
/// separate from the report shape so callers cannot mistake a cancelled or
/// budget-limited pass for authoritative zero-session evidence.
#[derive(Debug, Clone)]
pub struct BoundedSessionDiscoveryReport {
    pub report: ProjectDiscoveryReport,
    pub complete: bool,
    pub cancelled: bool,
    pub processed_sources: usize,
    pub total_sources: usize,
...
fn truncate_to_limit<T>(values: &mut Vec<T>, limit: usize) {
...
enum SourceMode {
    ...
    TextMetadata,
    SqliteMetadata,
    /// OpenCode's current SQLite store is routinely larger than the generic
    /// metadata cap. Read only its project/session tables with schema-aware,
    /// bounded queries; never scan message bodies during discovery.
    OpenCodeState,
    PinokioApps,
...
pub fn discover_known_projects(
    ...
    let snapshot = WslDiscoverySnapshot::system(wsl_scan_enabled());
    discover_known_projects_with_wsl_snapshot(registered_roots, options, &snapshot)
}

/// Run the production project-discovery implementation against one already
/// resolved WSL view. The snapshot's UNC policy is scoped to this synchronous
/// call and restored even if parsing unwinds.
pub fn discover_known_projects_with_wsl_snapshot(
    registered_roots: &[RegisteredRoot],
    options: DiscoveryOptions,
    snapshot: &WslDiscoverySnapshot,
) -> ProjectDiscoveryReport {
    with_wsl_scan_snapshot(snapshot.allow_wsl_unc, || {
        discover_known_projects_inner(registered_roots, options, snapshot)
    })
}

fn discover_known_projects_inner(
    registered_roots: &[RegisteredRoot],
    options: DiscoveryOptions,
    snapshot: &WslDiscoverySnapshot,
) -> ProjectDiscoveryReport {
    ...
    for source in discovery_sources_with_wsl_snapshot(snapshot) {
        let exists = discovery_metadata_entry_exists(&source.path);
        let broad_cloud_source =
            source.mode == SourceMode::KnownFolder && is_cloud_backed_path(&source.path);
        searched_locations.push(DiscoverySourceHit {
            source_kind: source.kind.to_string(),
            source_label: source.label.to_string(),
            path: display_path(&source.path),
            exists,
            detail: if broad_cloud_source {
                Some(
                    "Cloud-backed broad folder not traversed automatically. Registered projects and app/session-linked roots inside it remain supported; use an explicit bounded Deep Scan for wider discovery."
                        .to_string(),
                )
            } else {
                source.detail.clone()
    ...
    add_all_registry_projects_with_wsl_snapshot(&mut candidates, snapshot);
...
/// Discover only local coding-tool sessions for Safe Manage. Unlike the broad
/// project-discovery entry point, this pass never walks generic project roots or
/// app registries. It has one shared wall-clock budget, observes cancellation at
/// every bounded source boundary, and reports progress after each source.
///
/// If the pass cannot inspect every eligible source, `complete` is false and the
/// returned report is deliberately shaped as truncated (`total_sessions` is
/// greater than `sessions.len()`). This fail-closed shape preserves unknown
/// session evidence in callers that only understand `ProjectDiscoveryReport`.
pub fn discover_known_sessions_bounded<C, P>(
    registered_roots: &[RegisteredRoot],
    options: DiscoveryOptions,
    budget: Duration,
    mut should_cancel: C,
    mut progress: P,
) -> BoundedSessionDiscoveryReport
where
    C: FnMut() -> bool,
    P: FnMut(SessionDiscoveryProgress),
{
    let sources = discovery_sources()
        .into_iter()
        .filter(source_lists_sessions)
        .filter(|source| options.include_agents || !is_agent_session_kind(&source.kind))
        .collect::<Vec<_>>();
    discover_known_sessions_bounded_with_sources(
        registered_roots,
        options,
        budget,
        &sources,
...
fn discover_known_sessions_bounded_with_sources<C, P>(
    registered_roots: &[RegisteredRoot],
    options: DiscoveryOptions,
    budget: Duration,
    ...
    P: FnMut(SessionDiscoveryProgress),
{
    ...
    let mut source_scan_complete = true;
    let mut searched_locations = Vec::with_capacity(total_sources);
    let mut candidates: BTreeMap<String, CandidateBuilder> = BTreeMap::new();
    let mut sessions: BTreeMap<String, SessionBuilder> = BTreeMap::new();
    ...
    for source in sources {
        ...
        if exists {
            source_scan_complete &= scan_session_source_bounded(
                source,
                &mut candidates,
                &mut sessions,
                started,
        ...
        if started.elapsed() >= budget && processed_sources < total_sources {
            break;
        }
    }

    let complete = !cancelled && source_scan_complete && processed_sources == total_sources;
    let mut sessions = finalize_sessions(sessions, registered_roots);
    filter_sessions_by_options(&mut sessions, &options);
    let discovered_sessions = sessions.len() as u64;
    truncate_to_limit(&mut sessions, options.session_limit);
    let mut total_sessions = discovered_sessions;
    if !complete {
        total_sessions = total_sessions.max((sessions.len() as u64).saturating_add(1));
    }
...
pub fn discover_projects_in_root(
    ...
    let snapshot = WslDiscoverySnapshot::system(wsl_scan_enabled());
    discover_projects_in_root_with_wsl_snapshot(root, registered_roots, options, &snapshot)
}

/// Run the production bounded folder-discovery implementation against one
/// immutable WSL view.
pub fn discover_projects_in_root_with_wsl_snapshot(
    root: &Path,
    registered_roots: &[RegisteredRoot],
    options: DiscoveryOptions,
    snapshot: &WslDiscoverySnapshot,
    ...
    with_wsl_scan_snapshot(snapshot.allow_wsl_unc, || {
        let known_sources = discovery_sources_with_wsl_snapshot(snapshot);
        discover_projects_in_root_with_sources(root, registered_roots, options, &known_sources)
    })
}

fn discover_projects_in_root_with_sources(
    root: &Path,
    registered_roots: &[RegisteredRoot],
    options: DiscoveryOptions,
    known_sources: &[DiscoverySource],
) -> ProjectDiscoveryReport {
    let started = Instant::now();
    ...
    let exists = hangar_fs::local_metadata_directory_exists(&source.path);
    ...
    let mut sessions: BTreeMap<String, SessionBuilder> = BTreeMap::new();
    if exists {
        let limits = if is_cloud_backed_path(root) {
            FolderScanLimits {
                max_depth: CLOUD_DEEP_ROOT_DEPTH,
                max_dirs: CLOUD_DEEP_ROOT_MAX_DIRS,
                max_duration: Some(CLOUD_DEEP_ROOT_BUDGET),
            }
        } else {
            FolderScanLimits {
                max_depth: DEEP_ROOT_DEPTH,
                max_dirs: DEEP_ROOT_MAX_DIRS,
                max_duration: None,
            }
        };
        scan_folder_source(
        ...
        for known_source in known_sources {
            if !discovery_metadata_entry_exists(&known_source.path) {
                continue;
            }
...
fn discovery_sources() -> Vec<DiscoverySource> {
    let snapshot = WslDiscoverySnapshot::system(wsl_scan_enabled());
    discovery_sources_with_wsl_snapshot(&snapshot)
}

fn discovery_sources_with_wsl_snapshot(snapshot: &WslDiscoverySnapshot) -> Vec<DiscoverySource> {
    let mut sources = Vec::new();
    if !snapshot.include_system_sources {
        #[cfg(windows)]
        push_wsl_sources_from(&mut sources, &snapshot.homes);
        return sources;
    }
    if let Some(home) = home_dir() {
        ...
        push_source(
            &mut sources,
            "opencode_state",
            "OpenCode projects and sessions",
            opencode_state_db_for_home(&home),
            SourceMode::OpenCodeState,
            "Reads OpenCode's local project/session tables read-only; message bodies are opened only for an explicitly selected session preview.",
        );
        ...
        push_source(
            ...
            "omp_sessions",
            "Oh My Pi sessions",
            omp_sessions_dir_for_home(&home),
            SourceMode::TextMetadata,
            "Reads Oh My Pi JSONL session headers for their recorded cwd/project path.",
        );
    ...
    for drive in 'C'..='Z' {
        ...
        if !discovery_metadata_directory_exists(&root) {
            continue;
        }
        for suffix in [
...
fn wsl_distro_homes() -> Vec<(String, PathBuf)> {
    ...
    for distro in wsl_distros() {
        ...
        for entry in entries.flatten() {
            let home = entry.path();
            if discovery_metadata_directory_exists(&home) {
...
#[cfg(windows)]
fn push_wsl_sources_from(sources: &mut Vec<DiscoverySource>, homes: &[(String, PathBuf)]) {
...
pub fn wsl_hermes_state_dbs() -> Vec<PathBuf> {
    ...
    for distro in wsl_distros() {
        ...
        for entry in entries.flatten() {
            ...
            if discovery_metadata_file_exists(&db) {
                out.push(db);
            }
        }
    }
...
fn push_known_folder_if_dir(
    ...
    if discovery_metadata_directory_exists(&path)
        && sources.iter().all(|existing| existing.path != path)
    {
...
fn scan_discovery_source(
    ...
    match source.mode {
        ...
        SourceMode::OpenCodeState => scan_opencode_state_source(source, candidates, sessions),
...
fn source_lists_sessions(source: &DiscoverySource) -> bool {
    match source.mode {
        SourceMode::TextMetadata
        | SourceMode::OpenCodeState
        | SourceMode::CursorProjectTranscripts
        | SourceMode::CursorIdeChats
        | SourceMode::HermesState
        | SourceMode::OpenClawState => true,
        // The generic SQLite reader mostly extracts path hints. The Codex state
        // reader is the exception: it supplies authoritative cwd/title metadata
        // for rollout sessions already found in the preceding transcript pass.
        SourceMode::SqliteMetadata => source.kind.contains("codex_state"),
        SourceMode::KnownFolder | SourceMode::PinokioApps => false,
    }
}

fn session_database_source_is_readable(source: &DiscoverySource) -> bool {
    let Ok(conn) = open_discovery_sqlite(&source.path) else {
        return false;
    };
    match source.mode {
        SourceMode::HermesState => sqlite_has_table(&conn, "sessions"),
        SourceMode::OpenCodeState => {
            sqlite_has_table(&conn, "project") && sqlite_has_table(&conn, "session")
        }
        SourceMode::OpenClawState => {
            sqlite_has_table(&conn, "acp_replay_sessions")
                || sqlite_has_table(&conn, "agent_databases")
        }
        SourceMode::CursorIdeChats => sqlite_has_table(&conn, "ItemTable"),
        SourceMode::SqliteMetadata => true,
        _ => true,
    }
}

/// Scan one already-bounded session source and conservatively report whether it
/// could have been exhausted. Cancellation and the shared deadline are checked
/// before and after each source; directory-based scanners also carry their own
/// 12-second ceiling, so one source cannot repeatedly consume the global budget.
fn scan_session_source_bounded<C>(
    source: &DiscoverySource,
    candidates: &mut BTreeMap<String, CandidateBuilder>,
    sessions: &mut BTreeMap<String, SessionBuilder>,
...
where
    C: FnMut() -> bool,
{
    if should_cancel() || overall_started.elapsed() >= overall_budget {
        return false;
    }
    if matches!(
        source.mode,
        SourceMode::SqliteMetadata
            ...
            | SourceMode::HermesState
            | SourceMode::OpenClawState
    ) && !session_database_source_is_readable(source)
    {
        return false;
    }
    let source_started = Instant::now();
    let sessions_before = sessions.len();
    let source_complete = match source.mode {
        SourceMode::TextMetadata => {
            let mut stop = || {
                should_cancel()
                    || overall_started.elapsed() >= overall_budget
                    || source_started.elapsed() >= SOURCE_SCAN_BUDGET
            };
            scan_text_metadata_source_interruptible(source, candidates, sessions, &mut stop)
        }
        SourceMode::CursorProjectTranscripts => {
            let mut stop = || {
                should_cancel()
            ...
            scan_cursor_project_transcripts_source_interruptible(
                source, candidates, sessions, &mut stop,
            )
        }
        _ => {
            scan_discovery_source(source, candidates, sessions);
            true
        }
    };
    let added_sessions = sessions.len().saturating_sub(sessions_before);

    if !source_complete || should_cancel() || overall_started.elapsed() >= overall_budget {
        return false;
    }

    // These readers have explicit SQL row ceilings. Reaching a ceiling may be
    // exact, but treating it as incomplete avoids silently converting a hidden
    // next row into authoritative absence.
    match source.mode {
        SourceMode::HermesState if added_sessions >= HERMES_STATE_SESSION_LIMIT => false,
        SourceMode::OpenCodeState if added_sessions >= OPENCODE_STATE_SESSION_LIMIT => false,
        SourceMode::OpenClawState if added_sessions >= OPENCLAW_SESSION_LIMIT => false,
        _ => true,
    }
}
...
fn scan_known_folder_source(
    ...
    scan_folder_source(
        ...
        candidates,
        FolderScanLimits {
            max_depth: KNOWN_ROOT_DEPTH,
            max_dirs: KNOWN_ROOT_MAX_DIRS,
            max_duration: None,
        },
        "folder_marker",
        0,
...
fn scan_folder_source(
    ...
    while let Some((dir, depth)) = queue.pop_front() {
        ...
        if !discovery_metadata_directory_exists(&dir) {
            continue;
        }
        visited += 1;
...
fn scan_pinokio_apps_source(
    ...
    if !discovery_metadata_directory_exists(&source.path) {
        return;
    }
...
fn scan_text_metadata_source(
    ...
    let started = Instant::now();
    let mut should_stop = || started.elapsed() >= SOURCE_SCAN_BUDGET;
    let _ = scan_text_metadata_source_interruptible(source, candidates, sessions, &mut should_stop);
}

fn scan_text_metadata_source_interruptible<S>(
    source: &DiscoverySource,
    candidates: &mut BTreeMap<String, CandidateBuilder>,
    sessions: &mut BTreeMap<String, SessionBuilder>,
    should_stop: &mut S,
    ...
    S: FnMut() -> bool + ?Sized,
{
    ...
    if discovery_metadata_file_exists(&source.path) {
        if hangar_fs::open_local_file_no_recall(&source.path).is_err() {
            return false;
        }
        scan_metadata_file(&source.path, source, candidates, sessions);
        return !should_stop();
    }
    let files = bounded_metadata_files_interruptible(
        &source.path,
        SOURCE_SCAN_DEPTH,
        SOURCE_SCAN_MAX_FILES,
        should_stop,
    );
    let mut complete = files.complete;
    for file in files.files {
        if should_stop() {
            complete = false;
            break;
        }
        if hangar_fs::open_local_file_no_recall(&file).is_err() {
            complete = false;
            continue;
        }
        scan_metadata_file(&file, source, candidates, sessions);
    }
    complete && !should_stop()
}
...
fn link_cwd_only_session(
    ...
    let paths: Vec<PathBuf> = cwd
        .filter(|path| {
            discovery_metadata_directory_exists(path) && is_explicit_session_cwd_link_path(path)
        })
        .into_iter()
        .collect();
    if should_list_session_file(source_file, &source.kind) {
        let display_name = (source.kind == "omp_sessions")
            .then(|| omp_session_title(source_file))
            .flatten();
...
fn omp_session_title(path: &Path) -> Option<String> {
    let text = read_text_prefix(path, SOURCE_FILE_BYTES)?;
    text.lines().take(4).find_map(|line| {
        let value = serde_json::from_str::<serde_json::Value>(line).ok()?;
        (value.get("type").and_then(serde_json::Value::as_str) == Some("title"))
            .then(|| value.get("title").and_then(serde_json::Value::as_str))
            .flatten()
            .map(str::to_string)
    })
}
...
fn scan_metadata_file(
    ...
    if !discovery_metadata_file_exists(source_file) {
        return;
    }
    ...
    if fs::symlink_metadata(source_file)
    ...
    // subfolders (e.g. cwd `…\SampleGame-Lab` would also add
    // `…\SampleGame-Lab\engine`) as separate projects. Each conversation is
...
fn scan_cursor_project_transcripts_source(
    ...
    let started = Instant::now();
    let mut should_stop = || started.elapsed() >= SOURCE_SCAN_BUDGET;
    let _ = scan_cursor_project_transcripts_source_interruptible(
        source,
        candidates,
        sessions,
        &mut should_stop,
...
fn scan_cursor_project_transcripts_source_interruptible<S>(
    source: &DiscoverySource,
    candidates: &mut BTreeMap<String, CandidateBuilder>,
    sessions: &mut BTreeMap<String, SessionBuilder>,
    should_stop: &mut S,
    ...
    S: FnMut() -> bool + ?Sized,
{
    ...
    for project_entry in project_entries.flatten() {
        ...
        if !discovery_metadata_directory_exists(&project_dir) {
            continue;
        }
        let transcripts_dir = project_dir.join("agent-transcripts");
        if !discovery_metadata_directory_exists(&transcripts_dir) {
            continue;
        }
        ...
        for session_entry in session_entries.flatten() {
            ...
            if !discovery_metadata_directory_exists(&session_dir) {
                continue;
            }
...
fn cursor_main_transcript_file(session_dir: &Path, session_id: &str) -> Option<PathBuf> {
    ...
    if discovery_metadata_file_exists(&preferred) {
    ...
    let mut files = fs::read_dir(session_dir)
        ...
        .filter(|path| {
            discovery_metadata_file_exists(path)
                && path
                    .extension()
...
fn scan_openclaw_sessions_file(
    ...
    for (session_key, entry) in entries {
        ...
        let transcript = session_id
            ...
            .filter(|path| discovery_metadata_file_exists(path));
...
const OPENCODE_STATE_SESSION_LIMIT: usize = 10_000;
const OPENCODE_TRANSCRIPT_MAX_MESSAGES: usize = 4_000;
pub const OPENCODE_TRANSCRIPT_MAX_BYTES: usize = 768 * 1024;
...
fn openclaw_agent_database_paths(global_db: &Path) -> Vec<PathBuf> {
    if !discovery_metadata_file_exists(global_db) {
        return Vec::new();
    }
...
/// OpenCode keeps projects, sessions and message parts in one local SQLite
/// database. Discovery deliberately reads only the small project/session tables;
/// message bodies are queried solely when the user opens one particular session.
fn scan_opencode_state_source(
    source: &DiscoverySource,
    candidates: &mut BTreeMap<String, CandidateBuilder>,
    sessions: &mut BTreeMap<String, SessionBuilder>,
) {
    ...
    if !sqlite_has_table(&conn, "project") || !sqlite_has_table(&conn, "session") {
        return;
    }

    let mut project_paths = BTreeMap::<String, PathBuf>::new();
    if let Ok(mut stmt) = conn.prepare(
        "SELECT id, worktree FROM project \
         WHERE worktree IS NOT NULL AND TRIM(worktree) <> '' LIMIT 10000",
    ) {
        if let Ok(rows) = stmt.query_map([], |row| {
            Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?))
        }) {
            for row in rows.flatten() {
                let (project_id, raw_path) = row;
                let path = session_path_for_source(source, Path::new(&raw_path));
                if !discovery_metadata_directory_exists(&path)
                    || !is_explicit_session_cwd_link_path(&path)
                {
                    continue;
                }
                let path = canonical_or_original(&path);
                project_paths.insert(project_id, path.clone());
                add_opencode_project_link(candidates, source, &path, "OpenCode project registry");
            }
        }
    }

    // Recent OpenCode versions keep additional directories for a project in a
    // separate table. Treat these as explicit app registry entries too, while
    // tolerating older schemas where the table does not exist.
    if sqlite_has_table(&conn, "project_directory") {
        if let Ok(mut stmt) = conn.prepare(
            "SELECT project_id, directory FROM project_directory \
             WHERE directory IS NOT NULL AND TRIM(directory) <> '' LIMIT 10000",
        ) {
            if let Ok(rows) = stmt.query_map([], |row| {
                Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?))
            }) {
    ...
    let Ok(mut stmt) = conn.prepare(
        "SELECT id, project_id, directory, title, time_created, time_updated \
         FROM session ORDER BY COALESCE(time_updated, time_created) DESC LIMIT ?1",
    ) else {
        return;
    };
    let Ok(rows) = stmt.query_map([OPENCODE_STATE_SESSION_LIMIT as i64], |row| {
        Ok((
            row.get::<_, String>(0)?,
            row.get::<_, Option<String>>(1).unwrap_or_default(),
            row.get::<_, Option<String>>(2).unwrap_or_default(),
    ...
    for row in rows.flatten() {
        let (session_id, project_id, directory, title, created_at, updated_at) = row;
        let direct_path = directory
            .as_deref()
            .map(PathBuf::from)
            .map(|path| session_path_for_source(source, &path))
            .filter(|path| {
        ...
        let linked_path = direct_path.or_else(|| {
            project_id
                .as_deref()
                .and_then(|project_id| project_paths.get(project_id).cloned())
        });
        let linked_paths = linked_path.into_iter().collect::<Vec<_>>();
        for path in &linked_paths {
            add_opencode_project_link(
                candidates,
                source,
                path,
                "OpenCode conversation working directory",
            );
            add_activity_signal(candidates, path, &source.path, "session_recent_activity");
        }
        ...
        let display_name = title
            ...
            .filter(|value| !value.trim().is_empty())
            .map(str::to_string)
            .unwrap_or_else(|| format!("OpenCode · {}", short_session_id(&session_id)));
        let suffix = format!("opencode-session={session_id}");
        add_session_candidate_with_display(
            sessions,
            &source.path,
            source,
        ...
        let session_path = source_file_with_fragment(&source.path, &suffix);
        if let Some(entry) = sessions.get_mut(&candidate_key(&session_path)) {
            entry.modified_ms = updated_at
                .or(created_at)
                .and_then(|value| sqlite_time_to_epoch_ms(value as f64))
                .or(entry.modified_ms);
        }
    }
}

fn add_opencode_project_link(
    candidates: &mut BTreeMap<String, CandidateBuilder>,
    source: &DiscoverySource,
    path: &Path,
    label: &'static str,
...
/// Resolve one app-owned cwd/worktree into a safe local project root. Attribution
/// is metadata-only: WSL stays behind its opt-in gate, absent/non-directory paths
/// are ignored, and canonicalization happens only after the cheap existence check.
fn app_attribution_project_root(source: &DiscoverySource, raw: &str) -> Option<PathBuf> {
    let raw = raw.trim();
    if raw.is_empty() {
        return None;
    }
    let path = session_path_for_source(source, Path::new(raw));
    if !discovery_metadata_directory_exists(&path) || !is_explicit_session_cwd_link_path(&path) {
        return None;
    }
    Some(canonical_or_original(&path))
}

/// Bounded project roots from OpenCode's SQLite store. Only the path columns of
/// `project`, optional `project_directory`, and `session` are queried; message and
/// part tables are never touched. Each query has its own hard row ceiling and the
/// database is opened read-only/no-recall by `open_discovery_sqlite`.
fn opencode_project_roots_from_store(store: &Path) -> Vec<PathBuf> {
    if !discovery_metadata_file_exists(store) {
        return Vec::new();
    }
    let Ok(conn) = open_discovery_sqlite(store) else {
        return Vec::new();
    };
    let source = DiscoverySource {
        kind: "opencode_state".to_string(),
        label: "OpenCode projects and sessions".to_string(),
        path: store.to_path_buf(),
        detail: None,
        mode: SourceMode::OpenCodeState,
    };
    ...
    let mut collect = |sql: &str| {
        ...
        let Ok(rows) = stmt.query_map([OPENCODE_ATTRIBUTION_ROW_LIMIT as i64], |row| {
            row.get::<_, String>(0)
        }) else {
            return;
        };
        for raw in rows.flatten() {
            if let Some(path) = app_attribution_project_root(&source, &raw) {
                roots.entry(candidate_key(&path)).or_insert(path);
            }
        }
    };

    if sqlite_has_table(&conn, "project") {
        collect(
            "SELECT worktree FROM project \
             WHERE worktree IS NOT NULL AND TRIM(worktree) <> '' LIMIT ?1",
        );
    }
    if sqlite_has_table(&conn, "project_directory") {
        collect(
            "SELECT directory FROM project_directory \
             WHERE directory IS NOT NULL AND TRIM(directory) <> '' LIMIT ?1",
        );
    }
    if sqlite_has_table(&conn, "session") {
        collect(
...
/// Bounded cwd roots from Oh My Pi's JSONL session headers. This is intentionally
/// smaller than general discovery: only `.jsonl` files are eligible, every file is
/// read through the no-recall 64 KiB prefix reader, and the walk has independent
/// depth/directory/entry/file/time ceilings. It never opens message bodies beyond
/// that prefix and never traverses a reparse-point directory.
fn omp_project_roots_from_sessions(sessions_dir: &Path) -> Vec<PathBuf> {
    if !discovery_metadata_directory_exists(sessions_dir) {
        return Vec::new();
    }
    let source = DiscoverySource {
        kind: "omp_sessions".to_string(),
        label: "Oh My Pi sessions".to_string(),
        path: sessions_dir.to_path_buf(),
        detail: None,
        mode: SourceMode::TextMetadata,
    };
    let started = Instant::now();
    let mut queue = VecDeque::from([(sessions_dir.to_path_buf(), 0_usize)]);
    ...
    'walk: while let Some((dir, depth)) = queue.pop_front() {
        if started.elapsed() >= OMP_ATTRIBUTION_BUDGET
            || visited_dirs >= OMP_ATTRIBUTION_MAX_DIRS
            || visited_entries >= OMP_ATTRIBUTION_MAX_ENTRIES
            || inspected_files >= OMP_ATTRIBUTION_MAX_FILES
        {
            break;
        }
        if !discovery_metadata_directory_exists(&dir) {
        ...
        for entry in entries.flatten() {
            ...
            if file_type.is_dir() {
                if depth < OMP_ATTRIBUTION_MAX_DEPTH && is_traversable_dir(&path) {
                    queue.push_back((path, depth + 1));
                }
                continue;
            }
            ...
            inspected_files += 1;
            let Some(text) = read_text_prefix(&path, SOURCE_FILE_BYTES) else {
                continue;
            };
            let Some(cwd) = extract_session_cwd(&text) else {
                continue;
            };
            if let Some(root) = app_attribution_project_root(&source, &cwd.to_string_lossy()) {
                roots.entry(candidate_key(&root)).or_insert(root);
            }
        }
    }
    roots.into_values().collect()
...
pub fn is_opencode_state_db(path: &Path) -> bool {
    path.file_name()
        .and_then(|value| value.to_str())
        .is_some_and(|name| name.eq_ignore_ascii_case("opencode.db"))
        && open_discovery_sqlite(path).is_ok_and(|conn| {
            sqlite_has_table(&conn, "session")
                && sqlite_has_table(&conn, "message")
                && sqlite_has_table(&conn, "part")
        })
}

/// Render one OpenCode conversation as bounded JSONL. The query selects only
/// text parts belonging to the requested session and ignores reasoning/tool
/// records, keeping discovery and preview read-only and narrowly scoped.
pub fn opencode_session_transcript_window(
    path: &Path,
    session_id: &str,
    max_bytes: usize,
    load_full: bool,
) -> Option<(String, bool)> {
    if !is_opencode_state_db(path) || session_id.trim().is_empty() || max_bytes == 0 {
        return None;
    }
    let max_bytes = if load_full { usize::MAX } else { max_bytes };
    let max_messages = progressive_transcript_item_limit(
        OPENCODE_TRANSCRIPT_MAX_MESSAGES,
        OPENCODE_TRANSCRIPT_MAX_BYTES,
        max_bytes,
        load_full,
    );
    let query_limit = max_messages.saturating_add(1).min(i64::MAX as usize) as i64;
    ...
    let mut stmt = conn
        .prepare(
            "WITH recent_messages AS (\
             ...
             ORDER BY rm.time_created ASC, p.time_created ASC, p.id ASC",
        )
        .ok()?;
    let rows = stmt
        .query_map(rusqlite::params![session_id, query_limit], |row| {
    ...
    for row in rows.flatten() {
        let (message_id, message_json, part_json) = row;
        if messages.last().map(|message| message.id.as_str()) != Some(message_id.as_str()) {
            let value = serde_json::from_str::<serde_json::Value>(&message_json).ok()?;
            let role = value
                .get("role")
                .and_then(serde_json::Value::as_str)
                .unwrap_or("assistant")
        ...
        let Ok(value) = serde_json::from_str::<serde_json::Value>(&part_json) else {
            continue;
        };
    ...
    if truncated {
        let remove_count = messages.len().saturating_sub(max_messages);
        messages.drain(..remove_count);
    }
    let mut newest_first = Vec::new();
    let mut used = 0_usize;
    for message in messages.into_iter().rev() {
        let content = message.text_parts.join("\n\n");
...
fn read_raw_text_file(path: &Path, max_bytes: u64) -> Option<String> {
    let mut file = hangar_fs::open_local_file_no_recall(path).ok()?;
...
struct DiscoveryConnection {
    ...
    // Holding no-recall read handles prevents a cloud provider from dehydrating
    // the main database or an existing SQLite sidecar while rusqlite is reading.
    _local_guards: Vec<fs::File>,
}
...
fn open_discovery_sqlite(path: &Path) -> rusqlite::Result<DiscoveryConnection> {
    let mut local_guards = Vec::new();
    local_guards.push(
    ...
    for suffix in ["-wal", "-shm", "-journal"] {
        ...
        if discovery_metadata_entry_exists(&sidecar) {
            local_guards.push(
                hangar_fs::open_local_file_no_recall(&sidecar)
                    .map_err(|_| rusqlite::Error::InvalidPath(sidecar.clone()))?,
            );
        }
    }
...
fn cursor_ide_conversations_from_json(
    ...
    for composer in composers {
        ...
        let mut cwd = composer
            ...
            .and_then(|workspace| workspace.get("uri"))
            .and_then(cursor_uri_to_path)
            .filter(|path| discovery_metadata_directory_exists(path));
...
fn add_cursor_ide_conversation_session(
    ...
    let linked: Vec<PathBuf> = conversation
        .cwd
        .as_ref()
        .filter(|path| !is_agent_scratch_path(path) && discovery_metadata_directory_exists(path))
        .cloned()
        .into_iter()
        .collect();
...
fn antigravity_conversation_uuids_in_dir(dir: Option<&Path>) -> BTreeSet<String> {
    ...
    if let Some(dir) = dir.filter(|dir| discovery_metadata_directory_exists(dir)) {
...
fn main_antigravity_conversation_uuids() -> std::sync::Arc<BTreeSet<String>> {
    ...
    let modified = dir
        .as_ref()
        .filter(|d| discovery_metadata_directory_exists(d))
...
fn antigravity_proto_map_cached(
    ...
    let (modified, len) = path
        .as_ref()
        .filter(|p| discovery_metadata_file_exists(p))
...
fn parse_antigravity_proto_map(path: &Path) -> AntigravityProtoMap {
    let Ok(mut file) = hangar_fs::open_local_file_no_recall(path) else {
        return AntigravityProtoMap::new();
    };
    if file
        .metadata()
        .map(|metadata| metadata.len() > REGISTRY_MAX_BYTES)
        .unwrap_or(true)
    {
        return AntigravityProtoMap::new();
    }
    ...
    if file
        .by_ref()
        .take(REGISTRY_MAX_BYTES.saturating_add(1))
        .read_to_end(&mut bytes)
        .is_err()
        || bytes.len() as u64 > REGISTRY_MAX_BYTES
    {
        return AntigravityProtoMap::new();
    }
...
/// `file:///c%3A/Synthetic/CodeHangar`) into a Windows path (`C:\Synthetic\CodeHangar`).
...
fn antigravity_registry_uri_directories(folder_uri: &str) -> Vec<PathBuf> {
    match antigravity_folder_uri_to_path(folder_uri) {
        Some(path) if discovery_metadata_directory_exists(&path) => vec![path],
        _ => Vec::new(),
    }
}
    ...
    if let Some(uuid) = uuid {
        match proto_map.get(uuid) {
            Some(AntigravityResolution::Project { path, project_uuid }) => {
                ...
                if discovery_metadata_directory_exists(path) && seen.insert(candidate_key(path)) {
                    out.push(path.clone());
                }
...
fn add_directory_if_project(
    ...
    let marker_exists = discovery_metadata_entry_exists;
    let has_git = marker_exists(&dir.join(".git"));
    let cursor_rules = marker_exists(&dir.join(".cursor").join("rules"));
    let roo_rules = marker_exists(&dir.join(".roo").join("rules"));
    let continue_rules = marker_exists(&dir.join(".continue").join("rules"))
        || marker_exists(&dir.join(".continue").join("config.yaml"))
        || marker_exists(&dir.join(".continue").join("assistant"))
        || marker_exists(&dir.join(".continue").join("assistants"));
    let windsurf_rules = marker_exists(&dir.join(".windsurf").join("rules"))
        || marker_exists(&dir.join(".devin").join("rules"));
    if markers.is_empty()
        && !has_git
        && !cursor_rules
        && !roo_rules
...
fn add_marker_signals(candidates: &mut BTreeMap<String, CandidateBuilder>, dir: &Path) {
    ...
    if marker_exists(&dir.join(".cursor").join("rules")) {
        add_signal(
            candidate,
            "cursor_rules_dir",
    ...
    if marker_exists(&dir.join(".roo").join("rules")) {
        add_signal(
            candidate,
            "roo_rules_dir",
    ...
    if marker_exists(&dir.join(".continue").join("rules")) {
        add_signal(
            candidate,
            "continue_rules_dir",
    ...
    if marker_exists(&dir.join(".continue").join("config.yaml"))
        || marker_exists(&dir.join(".continue").join("assistant"))
        || marker_exists(&dir.join(".continue").join("assistants"))
    {
        add_signal(
    ...
    if marker_exists(&dir.join(".windsurf").join("rules")) {
        add_signal(
            candidate,
            "windsurf_rules_dir",
    ...
    if marker_exists(&dir.join(".devin").join("rules")) {
        add_signal(
            candidate,
            "devin_rules_dir",
...
fn session_visible_identity_key(session: &SessionDiscoveryCandidate) -> String {
    if let Some((_, fragment)) = session.path.rsplit_once('#') {
        ...
        if let Some(session_id) = fragment.strip_prefix("opencode-session=") {
            return format!("opencode-session:{}", session_id.to_ascii_lowercase());
        }
    }
    if session.source_kind == "omp_sessions" {
        let file_path = session.path.split('#').next().unwrap_or(&session.path);
        if let Some(stem) = Path::new(file_path)
            .file_stem()
            .and_then(|value| value.to_str())
        {
            return format!("omp-session:{}", stem.to_ascii_lowercase());
        }
    }
    ...
    // session uuid (e.g. `33333333-3333-4333-8333-333333333333.jsonl`). New code titles
...
fn session_candidate_rank(session: &SessionDiscoveryCandidate) -> u8 {
    if session.source_kind.contains("hermes_state")
        ...
        || session.source_kind.contains("opencode_state")
    {
        return 3;
    }
...
fn project_markers_in_dir(dir: &Path) -> Vec<ProjectMarker> {
    ...
    for marker in PROJECT_MARKERS {
        if discovery_metadata_entry_exists(&dir.join(marker.file_name)) {
            markers.push(*marker);
        }
    }
    markers
...
fn bounded_metadata_files(root: &Path, max_depth: usize, max_files: usize) -> Vec<PathBuf> {
    bounded_metadata_files_interruptible(root, max_depth, max_files, &mut || false).files
}

struct BoundedMetadataFiles {
    files: Vec<PathBuf>,
    complete: bool,
}

fn bounded_metadata_files_interruptible<S>(
    root: &Path,
    max_depth: usize,
    max_files: usize,
    should_stop: &mut S,
) -> BoundedMetadataFiles
where
    S: FnMut() -> bool + ?Sized,
{
    ...
    while let Some((dir, depth)) = queue.pop_front() {
        ...
        let mut entries_with_times = Vec::new();
        for entry in entries.flatten() {
            if should_stop() {
                return BoundedMetadataFiles {
                    files,
            ...
            let path = entry.path();
            // `DirEntry::metadata` follows the entry on Windows. Sorting is
            // advisory, so use no-follow metadata and never touch its target.
            let modified = fs::symlink_metadata(&path)
                .and_then(|metadata| metadata.modified())
                .unwrap_or(SystemTime::UNIX_EPOCH);
            entries_with_times.push((path, modified));
        }
        let mut entries = entries_with_times;
        ...
        for (path, _) in entries {
            ...
            if is_traversable_dir(&path) {
                if depth < max_depth && !skip_dir(&path) {
                    queue.push_back((path, depth + 1));
                } else if depth >= max_depth {
                    complete = false;
                }
            } else if looks_like_metadata_file(&path) && discovery_metadata_file_exists(&path) {
                files.push(path);
                if files.len() >= max_files {
                    return BoundedMetadataFiles {
                        files,
    ...
    BoundedMetadataFiles { files, complete }
}

fn is_traversable_dir(path: &Path) -> bool {
    !skip_dir(path) && discovery_metadata_directory_exists(path)
}
...
fn existing_directory_candidate(raw: &str) -> Option<PathBuf> {
    // Extracted text can name an arbitrary network share. Reject generic UNC
    // before trimming or walking parents; an explicitly enabled WSL share is the
    // sole existing opt-in exception handled by the metadata helper below.
    let raw_path = Path::new(raw.trim());
    if !discovery_metadata_probe_allowed(raw_path) {
        return None;
    }
...
fn prefer_project_container_for_embedded_dependency(path: &Path) -> PathBuf {
    ...
    for marker in [".external", ".vendor"] {
        ...
        if discovery_metadata_directory_exists(&ancestor) && has_project_identity(&ancestor) {
            return ancestor;
        }
    }
    path.to_path_buf()
...
fn has_project_identity(path: &Path) -> bool {
    !project_markers_in_dir(path).is_empty()
        || discovery_metadata_entry_exists(&path.join(".git"))
        || discovery_metadata_entry_exists(&path.join(".cursor").join("rules"))
        || discovery_metadata_entry_exists(&path.join(".roo").join("rules"))
        || discovery_metadata_entry_exists(&path.join(".continue").join("rules"))
        || discovery_metadata_entry_exists(&path.join(".continue").join("config.yaml"))
        || discovery_metadata_entry_exists(&path.join(".windsurf").join("rules"))
        || discovery_metadata_entry_exists(&path.join(".devin").join("rules"))
}

/// Find the closest ancestor that looks like a deliberate project root.
///
/// Shell-open uses this for files such as `project/docs/guide.md`: the file's
/// immediate folder is useful context, but the nearest marker-bearing ancestor
/// is the boundary that should be catalogued. No broad discovery is performed
/// here and no file contents are read.
pub fn nearest_project_root_for_path(path: &Path) -> Option<PathBuf> {
    // Determine the final entry type without following it. Reject an unsafe
    // final directory/file before looking for ancestor markers so a shell path
    // cannot turn marker discovery into a junction or UNC probe.
    if !discovery_metadata_probe_allowed(path) {
        return None;
    }
    let metadata = fs::symlink_metadata(path).ok()?;
    let start = if metadata.is_dir() {
        discovery_metadata_directory_exists(path).then_some(path)?
    } else if metadata.is_file() {
        if !discovery_metadata_file_exists(path) {
            return None;
        }
        path.parent()?
    } else {
    ...
    start
        .ancestors()
        .find(|candidate| has_project_identity(candidate))
...
fn claude_project_roots(home: &Path) -> Vec<PathBuf> {
    ...
    if !discovery_metadata_directory_exists(&projects_dir) {
        return Vec::new();
    }
    ...
    for entry in entries.flatten() {
        ...
        if !discovery_metadata_directory_exists(&cwd) || !is_explicit_session_cwd_link_path(&cwd) {
            continue;
        }
...
/// (e.g. `.../projects/<enc>/33333333-3333-4333-8333-333333333333.jsonl`). Any `#`
...
fn apply_codex_state_threads(
    ...
    for session in sessions.values_mut() {
        ...
        if let Some(cwd) = meta.cwd.as_ref().filter(|path| {
            !is_agent_scratch_path(path) && discovery_metadata_directory_exists(path)
        }) {
...
fn probe_session_cwd(file: &Path) -> Option<PathBuf> {
    let mut handle = hangar_fs::open_local_file_no_recall(file).ok()?;
...
fn codex_session_project_roots(home: &Path) -> Vec<PathBuf> {
    ...
    while let Some((dir, depth)) = stack.pop() {
        ...
        for entry in entries.flatten() {
            ...
            if !discovery_metadata_directory_exists(&cwd)
...
fn read_vscode_workspace_registry(user_dir: &Path) -> Vec<PathBuf> {
    ...
    if !discovery_metadata_directory_exists(&workspace_storage) {
        return Vec::new();
    }
...
fn gemini_project_roots_for_uuid(home: &Path, project_uuid: &str) -> Vec<PathBuf> {
    ...
    for root in gemini_project_file_roots(&file) {
        if discovery_metadata_directory_exists(&root) && seen.insert(candidate_key(&root)) {
            out.push(root);
        }
    }
    out
...
/// a project named "SampleProject" may be rooted at `D:\Samples` and carry that
/// name as a "named: SampleProject" label. Best-effort: empty when the
/// registry is absent.
...
fn registry_app_specificity(app: &str) -> u8 {
    match app {
        "antigravity" => 5,
        "cursor" => 4,
        // OpenCode's project/worktree tables are an explicit app registry, more
        // deliberate than a session-only cwd but below an editor workspace store.
        "opencode" => 3,
        "codex" => 2,
        // OMP and Claude attribution comes from bounded session cwd metadata.
        "omp" | "claude_code" | "claude" => 1,
        _ => 0,
    }
}
...
fn apply_project_app_attribution(
    states: &mut std::collections::HashMap<String, ProjectAppState>,
    attribution: std::collections::HashMap<String, (String, Vec<String>)>,
) {
    for (key, (app, apps)) in attribution {
...
///     can keep a registry project current when its conversations are `.pb` and never
///     link a `.db`.
...
/// path (see [`registry_app_specificity`]). OpenCode reads only bounded path rows
/// from its SQLite store; Oh My Pi reads only bounded JSONL header prefixes.
pub fn project_app_states() -> std::collections::HashMap<String, ProjectAppState> {
    ...
    let home_codex_roots: Vec<PathBuf> = home
        ...
        .unwrap_or_default();
    let home_opencode_roots: Vec<PathBuf> = home
        .as_ref()
        .map(|h| opencode_project_roots_from_store(&opencode_state_db_for_home(h)))
        .unwrap_or_default();
    let home_omp_roots: Vec<PathBuf> = home
        .as_ref()
        .map(|h| omp_project_roots_from_sessions(&omp_sessions_dir_for_home(h)))
        .unwrap_or_default();

    // 1) App attribution from every registry (path key -> most specific app + all apps).
    apply_project_app_attribution(
        &mut states,
        registry_app_attribution(
            &home_claude_roots,
            &home_codex_roots,
            &home_opencode_roots,
            &home_omp_roots,
    ...
    //    SampleProject is registered at D:\Samples\RootA but was chatted from an
    ...
    //    it counts as current too — keeps trusted-but-session-less Codex projects out
    //    of Archived.
    ...
    for path in &home_opencode_roots {
        states.entry(candidate_key(path)).or_default().is_current = true;
    }
    for path in &home_omp_roots {
        states.entry(candidate_key(path)).or_default().is_current = true;
    }
...
type RegistryOwners = std::collections::HashMap<String, (u8, String, BTreeSet<String>)>;

fn record_registry_owner(owners: &mut RegistryOwners, path_key: String, app: &str) {
...
fn record_precomputed_app_roots(
    owners: &mut RegistryOwners,
    home_claude_roots: &[PathBuf],
    home_codex_roots: &[PathBuf],
    home_opencode_roots: &[PathBuf],
    home_omp_roots: &[PathBuf],
) {
    for path in home_claude_roots {
        record_registry_owner(owners, candidate_key(path), "claude_code");
    }
    for path in home_codex_roots {
        record_registry_owner(owners, candidate_key(path), "codex");
    }
    for path in home_opencode_roots {
        record_registry_owner(owners, candidate_key(path), "opencode");
    }
    for path in home_omp_roots {
        record_registry_owner(owners, candidate_key(path), "omp");
    }
}

fn finish_registry_owners(
...
///
/// The root slices are precomputed once by the caller and shared so this hot path
/// does not re-walk transcript stores. OpenCode roots come from bounded SQLite path
/// queries; OMP roots come from bounded JSONL-header reads.
fn registry_app_attribution(
    ...
    let mut owners = RegistryOwners::new();

    if let Some(home) = home_dir() {
        for (path, _trusted) in read_codex_project_registry(&home) {
            record_registry_owner(&mut owners, candidate_key(&path), "codex");
        }
        for path in read_claude_project_registry(&home) {
            record_registry_owner(&mut owners, candidate_key(&path), "claude_code");
        }
        ...
        if let Some(appdata) = env_path("APPDATA") {
            for (dir, app) in [
                ...
                for path in read_vscode_workspace_registry(&dir) {
                    record_registry_owner(&mut owners, candidate_key(&path), app);
                }
            }
        }
        // Antigravity's authoritative per-project store (Windows-side).
        for path in read_gemini_project_registry(&home) {
            record_registry_owner(&mut owners, candidate_key(&path), "antigravity");
        }
    }
    ...
    for distro in wsl_distros() {
        ...
        for entry in entries.flatten() {
            ...
            if !discovery_metadata_directory_exists(&user_home) {
                continue;
            }
...
// A discovery request must use one immutable WSL preference snapshot from its
// owning AppState. The legacy atomic remains for older call sites, but scoped
// project-discovery entry points override it on the current thread so concurrent
// ON and OFF AppStates cannot change each other's enumeration or UNC-probe policy.
// Discovery is synchronous today; if it gains worker threads this policy must be
// propagated explicitly to those workers rather than relying on thread-local state.
#[cfg(windows)]
thread_local! {
    static WSL_SCAN_SNAPSHOT: Cell<Option<bool>> = const { Cell::new(None) };
}

#[cfg(windows)]
struct WslScanSnapshotGuard {
    previous: Option<bool>,
}

#[cfg(windows)]
impl Drop for WslScanSnapshotGuard {
    fn drop(&mut self) {
        WSL_SCAN_SNAPSHOT.with(|snapshot| snapshot.set(self.previous));
    }
}

/// Run one synchronous discovery operation with an immutable WSL opt-in
/// snapshot. Nested calls restore the outer value, including during unwinding.
/// This is deliberately per-thread/per-call rather than a process-wide toggle.
pub fn with_wsl_scan_snapshot<T>(enabled: bool, operation: impl FnOnce() -> T) -> T {
    #[cfg(windows)]
    {
        let previous = WSL_SCAN_SNAPSHOT.with(|snapshot| snapshot.replace(Some(enabled)));
        let _guard = WslScanSnapshotGuard { previous };
        operation()
    }
    #[cfg(not(windows))]
    {
...
pub fn wsl_scan_enabled() -> bool {
    #[cfg(windows)]
    {
        WSL_SCAN_SNAPSHOT
            .with(Cell::get)
            .unwrap_or_else(|| WSL_SCAN_ENABLED.load(std::sync::atomic::Ordering::Relaxed))
    }
    #[cfg(not(windows))]
    {
        false
...
/// Whether a path uses any UNC spelling. This is deliberately lexical: callers
/// use it before `stat`, `canonicalize`, `is_dir` or `is_file`, because even a
/// failed metadata probe can connect to a remote server or wake a WSL distro.
fn is_unc_path(path: &Path) -> bool {
    hangar_fs::path_uses_unc_syntax(path)
}
...
/// Metadata probes used by discovery are local-only by default. WSL is the one
/// existing opt-in exception: its enumerator is explicitly enabled by the user
/// and already bounded. Arbitrary UNC shares never become implicit discovery
/// input, even if a session or application registry records one.
fn discovery_metadata_probe_allowed(path: &Path) -> bool {
    discovery_metadata_probe_allowed_with_wsl(path, wsl_scan_enabled())
}

fn discovery_metadata_probe_allowed_with_wsl(path: &Path, wsl_enabled: bool) -> bool {
    if is_unc_path(path) {
        return is_wsl_unc_path(path) && wsl_enabled;
    }
    hangar_fs::local_metadata_probe_allowed(path)
}

/// Existence/type checks for paths sourced from session metadata or registries.
/// The hangar-fs helpers prove every ancestor and inspect the final entry without
/// following ordinary symlinks, junctions or unknown reparse points.
fn discovery_metadata_entry_exists(path: &Path) -> bool {
    discovery_metadata_probe_allowed(path) && hangar_fs::local_metadata_entry_exists(path)
}

fn discovery_metadata_directory_exists(path: &Path) -> bool {
    discovery_metadata_probe_allowed(path) && hangar_fs::local_metadata_directory_exists(path)
}

fn discovery_metadata_file_exists(path: &Path) -> bool {
    discovery_metadata_entry_exists(path)
...
fn add_all_registry_projects_with_wsl_snapshot(
    candidates: &mut BTreeMap<String, CandidateBuilder>,
    snapshot: &WslDiscoverySnapshot,
) {
    if snapshot.include_system_sources {
    ...
    for (distro, user_home) in &snapshot.homes {
        // The VS Code-family editors are Windows apps; only the home registries
        // (Claude, Codex) are read from a WSL distro.
...
fn should_list_session_file(source_file: &Path, kind: &str) -> bool {
    if kind == "omp_sessions" {
        // Oh My Pi stores sidecar logs/markdown beside real conversations. Only
        // its versioned JSONL files carry an authoritative session header/cwd.
        return source_file
            .extension()
            .and_then(|value| value.to_str())
            .is_some_and(|value| value.eq_ignore_ascii_case("jsonl"));
...
fn session_kind_for_source(kind: &str) -> &'static str {
    if kind.contains("opencode") {
        "OpenCode"
    } else if kind.contains("omp") {
        "Oh My Pi"
...
fn decode_cursor_project_segments(base: &Path, parts: &[&str]) -> Option<PathBuf> {
    if parts.is_empty() {
        return discovery_metadata_directory_exists(base).then(|| canonical_or_original(base));
    }
    for width in 1..=parts.len().min(6) {
        let segment = parts[..width].join("-");
        let candidate = base.join(segment);
        if !discovery_metadata_directory_exists(&candidate) {
            continue;
        }
    ...
    while let Some((dir, depth)) = queue.pop_front() {
        ...
        for entry in entries.flatten() {
            ...
            if entry.file_type().is_ok_and(|kind| kind.is_dir()) {
                ...
                if depth < ESTIMATE_DEPTH && is_traversable_dir(&child) {
                    queue.push_back((child, depth + 1));
                } else if depth >= ESTIMATE_DEPTH {
                    partial = true;
                }
            } else if discovery_metadata_file_exists(&child) {
...
fn candidate_key(path: &Path) -> String {
    ...
    // `C:\proj` and `C:/proj`). Filesystem canonicalization is intentionally forbidden
    // here, so this lexical fold is what keeps those two registry spellings together.
...
fn canonical_or_original(path: &Path) -> PathBuf {
    // Discovery keys are built on hot startup/list paths and often originate in
    // app registries. Filesystem canonicalization follows the final entry and can
    // therefore cross a junction or contact an arbitrary UNC share before a later
    // policy check rejects it. Keep this operation purely lexical; candidate_key
    // below already folds separators and case, and unsafe aliases are rejected by
    // the no-follow existence checks before they can become candidates.
...
/// True for a path managed by a Cloud Files provider, including custom OneDrive
/// roots advertised through the standard environment variables. This is a
/// metadata-only check: it never opens file content.
fn is_cloud_backed_path(path: &Path) -> bool {
    ...
    let path_key = lexical_path_key(path);
    ...
    roots.into_iter().any(|root| {
        let root_key = lexical_path_key(&root);
...
fn opencode_state_db_for_home(home: &Path) -> PathBuf {
...
fn omp_sessions_dir_for_home(home: &Path) -> PathBuf {
...
pub fn detect_installed_apps() -> Vec<hangar_core::InstalledApp> {
    ...
    let local_app_join = |seg: &str| local_appdata.as_ref().map(|a| a.join(seg));
    let any_exists = |paths: Vec<Option<PathBuf>>| {
        paths
            .into_iter()
            .flatten()
            .any(|path| discovery_metadata_entry_exists(&path))
    };
    ...
    vec![
        app(
            ...
            "OpenCode",
            vec![
                env_path("XDG_DATA_HOME").map(|path| path.join("opencode").join("opencode.db")),
                home.as_ref().map(|h| {
                    h.join(".local")
                        .join("share")
                        .join("opencode")
                ...
                local_app_join(r"Programs\@opencode-aidesktop"),
            ],
        ),
        app(
            "omp",
...
mod tests {
    ...
    #[test]
    fn bounded_session_discovery_cancels_inside_a_text_source_and_fails_closed() {
        let dir = tempdir().unwrap();
        for index in 0..64 {
            fs::write(dir.path().join(format!("session-{index:03}.jsonl")), "{}\n").unwrap();
        }
        let sources = vec![
            DiscoverySource {
                kind: "test_sessions".to_string(),
                label: "Synthetic sessions".to_string(),
                path: dir.path().to_path_buf(),
                detail: None,
                mode: SourceMode::TextMetadata,
            },
            DiscoverySource {
                kind: "later_sessions".to_string(),
                label: "Later sessions".to_string(),
                path: dir.path().join("later"),
                detail: None,
                mode: SourceMode::TextMetadata,
            },
        ];
        let mut cancel_checks = 0_usize;
        let mut progress = Vec::new();
        let outcome = discover_known_sessions_bounded_with_sources(
            &[],
            DiscoveryOptions {
                limit: 0,
                session_limit: 5_000,
            ...
            &mut |item| progress.push(item.processed_sources),
        );

        assert!(outcome.cancelled);
        assert!(!outcome.complete);
        assert_eq!(outcome.processed_sources, 1);
        assert_eq!(outcome.total_sources, 2);
        assert_eq!(progress, vec![1]);
        assert!(outcome.report.total_sessions > outcome.report.sessions.len() as u64);
    }

    #[test]
    fn bounded_metadata_file_depth_and_count_caps_are_incomplete() {
        let dir = tempdir().unwrap();
        fs::write(dir.path().join("root.jsonl"), "{}\n").unwrap();
        let child = dir.path().join("child");
        fs::create_dir(&child).unwrap();
        fs::write(child.join("nested.jsonl"), "{}\n").unwrap();

        let depth_limited = bounded_metadata_files_interruptible(dir.path(), 0, 10, &mut || false);
        assert!(!depth_limited.complete);
        assert_eq!(depth_limited.files.len(), 1);

        fs::write(dir.path().join("second.jsonl"), "{}\n").unwrap();
        let count_limited = bounded_metadata_files_interruptible(dir.path(), 4, 1, &mut || false);
        assert!(!count_limited.complete);
        assert_eq!(count_limited.files.len(), 1);
    }

    #[test]
    fn bounded_session_discovery_proves_zero_only_after_every_source_is_checked() {
        let dir = tempdir().unwrap();
        let sources = vec![DiscoverySource {
            kind: "absent_sessions".to_string(),
            label: "Absent sessions".to_string(),
            path: dir.path().join("missing"),
            detail: None,
            mode: SourceMode::TextMetadata,
        }];
        let mut progress = Vec::new();
        let complete = discover_known_sessions_bounded_with_sources(
            &[],
            DiscoveryOptions::default(),
            Duration::from_secs(1),
            &sources,
            &mut || false,
            &mut |item| progress.push(item.source_kind),
        );
        assert!(complete.complete);
        assert!(!complete.cancelled);
        assert_eq!(complete.report.total_sessions, 0);
        assert!(complete.report.sessions.is_empty());
        assert_eq!(progress, vec!["absent_sessions"]);

        let exhausted = discover_known_sessions_bounded_with_sources(
            &[],
            DiscoveryOptions::default(),
            Duration::ZERO,
            &sources,
        ...
        assert_eq!(exhausted.processed_sources, 0);
        assert!(
            exhausted.report.total_sessions > exhausted.report.sessions.len() as u64,
            "a budget-exhausted pass must not prove zero sessions"
        );
    }

    #[test]
    fn unreadable_session_database_never_becomes_authoritative_zero() {
        let dir = tempdir().unwrap();
        let malformed = dir.path().join("state.db");
        fs::write(&malformed, "not a sqlite database").unwrap();
        let sources = vec![DiscoverySource {
            kind: "hermes_state_sessions".to_string(),
            label: "Hermes conversations".to_string(),
            path: malformed,
            detail: None,
        ...
        assert!(!outcome.cancelled);
        assert_eq!(outcome.processed_sources, 1);
        assert!(outcome.report.sessions.is_empty());
        assert!(outcome.report.total_sessions > 0);
    }
    ...
    #[test]
    fn global_sources_never_add_broad_onedrive_folder_walks() {
        let kinds = discovery_sources()
            .into_iter()
            .filter(|source| source.mode == SourceMode::KnownFolder)
            .map(|source| source.kind)
            .collect::<Vec<_>>();
        assert!(!kinds.iter().any(|kind| kind.starts_with("onedrive_")));
    }

    #[test]
    fn paths_under_the_default_onedrive_root_are_cloud_backed_lexically() {
        let Some(home) = home_dir() else {
            return;
        };
        assert!(is_cloud_backed_path(
            &home.join("OneDrive").join("Projects").join("example")
        ));
    }
    ...
    #[test]
    fn unlimited_session_inventory_does_not_drop_late_providers_at_candidate_limit() {
        let mut sessions = (0..500)
            .map(|index| format!("ChatGPT {index}"))
            .collect::<Vec<_>>();
        sessions.push("OpenCode".to_string());
        sessions.push("Oh My Pi".to_string());

        // The project-candidate cap is independent. Zero means the complete,
        // already-bounded local session inventory, so later providers survive.
        truncate_to_limit(&mut sessions, 0);
        assert_eq!(sessions.len(), 502);
        assert!(sessions.iter().any(|value| value == "OpenCode"));
        assert!(sessions.iter().any(|value| value == "Oh My Pi"));

        truncate_to_limit(&mut sessions, 500);
    ...
    fn cursor_empty_window_transcript_is_loose_session() {
        ...
        assert_eq!(sessions.len(), 1);
        assert_eq!(sessions[0].association, "loose_session");
        assert_eq!(sessions[0].display_name, "Cursor empty window · 77777777");
        assert!(candidates.is_empty());
    }
    ...
    fn codex_date_slug_folders_are_loose_sessions_not_projects() {
        ...
        // A synthetic project folder is unaffected.
        let dir = tempdir().unwrap();
        let project = dir.path().join("synthetic-project");
        fs::create_dir_all(&project).unwrap();
        fs::write(project.join("README.md"), "# Synthetic").unwrap();
        assert!(!is_agent_scratch_path(&project));
        assert!(is_session_project_link_path(&project, true));
    }
    ...
    fn app_project_registries_yield_only_deliberate_projects() {
        ...
        // by no other app. It must still
        // survive on the trust_level signal alone.
        let trusted_no_marker = home.join("trusted-markerless");
    ...
    fn trusted_codex_folder_without_marker_becomes_candidate() {
        // End-to-end: a folder that is ONLY
        ...
        let dir = tempdir_projectlike();
        let home = dir.path();
        let trusted_project = home.join("trusted-markerless");
        fs::create_dir_all(&trusted_project).unwrap();
        fs::create_dir_all(home.join(".codex")).unwrap();
        fs::write(
            home.join(".codex").join("config.toml"),
            format!(
                "[projects.'{}']\ntrust_level = \"trusted\"\n",
                trusted_project.to_string_lossy()
            ),
        )
        .unwrap();
        ...
        let cand = finalized
            .iter()
            .find(|c| candidate_key(Path::new(&c.path)) == candidate_key(&trusted_project))
    ...
    /// Opt-in local-config consistency check. It tests every trusted, marker-less
    /// folder without embedding or printing any machine-specific project identity.
    #[test]
    ...
    fn real_codex_trusted_folders_without_markers_pass_filter() {
        ...
        for (path, trusted) in entries {
            if !trusted || !path.is_dir() || has_project_identity(&path) {
                continue;
            }
            assert!(is_registry_project_path(&path, true, 1));
            checked += 1;
        }
        eprintln!("checked {checked} trusted marker-less local folders");
    }
    ...
    fn gemini_registry_reads_folder_uri_and_skips_missing() {
        let dir = tempdir().unwrap();
        let home = dir.path();
        // An existing synthetic root and a missing one (an offline
        // external drive). Only the existing dir should surface.
        let real = home.join("SampleProject");
        fs::create_dir_all(&real).unwrap();
        fs::write(real.join("README.md"), "# SampleProject").unwrap();
        ...
        let example = finalized
            .iter()
            .find(|c| c.path.ends_with("SampleProject"))
    ...
    fn wsl_drive_mounts_translate_to_windows_drive_paths() {
        ...
        assert_eq!(
            translate_wsl_path("Ubuntu-24.04", Path::new("/mnt/c/Synthetic/CodeHangar")),
            PathBuf::from(r"C:\Synthetic\CodeHangar")
        );
        assert_eq!(
...
Caminho absoluto da pasta do projecto: {} (incluindo a subpasta ComponentA)
Nome do projecto: Project Beta
Caminhos absolutos das pastas do projecto: {} (incluindo SampleProject e RootA) e {} (incluindo SampleProject e RootB)
...
"#,
    ...
    #[cfg(windows)]
    #[test]
    fn linked_marker_directories_do_not_create_project_identity_or_signals() {
        let dir = tempdir().unwrap();
        let project = dir.path().join("apparent-project");
        let outside_git = dir.path().join("outside-git");
        let outside_cursor = dir.path().join("outside-cursor");
        fs::create_dir_all(&project).unwrap();
        fs::create_dir_all(&outside_git).unwrap();
        fs::create_dir_all(&outside_cursor).unwrap();
        fs::write(outside_git.join("HEAD"), "ref: refs/heads/private\n").unwrap();
        fs::write(outside_cursor.join("private.md"), "# outside rules").unwrap();
        create_test_directory_link(&outside_git, &project.join(".git")).unwrap();
        fs::create_dir_all(project.join(".cursor")).unwrap();
        create_test_directory_link(&outside_cursor, &project.join(".cursor").join("rules"))
            .unwrap();

        assert!(project.join(".git").is_dir());
        assert!(project.join(".cursor").join("rules").is_dir());
        assert!(!has_project_identity(&project));
        let source = DiscoverySource {
            kind: "test_known".to_string(),
            label: "Test known folder".to_string(),
            path: dir.path().to_path_buf(),
        ...
        fs::remove_dir(project.join(".cursor").join("rules")).unwrap();
        assert!(outside_git.join("HEAD").is_file());
        assert!(outside_cursor.join("private.md").is_file());
    }

    #[cfg(windows)]
    #[test]
    fn linked_discovery_roots_and_registry_cwds_fail_closed() {
        let dir = tempdir().unwrap();
        let outside = dir.path().join("outside-project");
        let linked = dir.path().join("recorded-project");
        fs::create_dir_all(&outside).unwrap();
        fs::write(outside.join("package.json"), "{}").unwrap();
        fs::write(outside.join("session.json"), "{\"cwd\":\"private\"}").unwrap();
        create_test_directory_link(&outside, &linked).unwrap();

        assert!(!discovery_metadata_directory_exists(&linked));
        assert_eq!(canonical_or_original(&linked), linked);
        assert_eq!(nearest_project_root_for_path(&linked), None);
        assert!(!is_registry_project_path(&linked, true, 2));
        let source = DiscoverySource {
            kind: "test_registry".to_string(),
            label: "Test registry".to_string(),
            path: dir.path().join("registry.json"),
            detail: None,
            mode: SourceMode::TextMetadata,
        };
        assert_eq!(
            app_attribution_project_root(&source, &linked.to_string_lossy()),
            None
        );
        assert!(bounded_metadata_files(&linked, 3, 50).is_empty());

        fs::remove_dir(&linked).unwrap();
        assert!(outside.join("package.json").is_file());
        assert!(outside.join("session.json").is_file());
    }

    #[test]
    fn generic_unc_discovery_paths_are_rejected_lexically() {
        let unc = Path::new(r"\\CodeHangar-NoSuch-Host\NoSuchShare\project");
        let wsl = Path::new(r"\\wsl.localhost\CodeHangar-Test\home\user\project");

        assert!(is_unc_path(unc));
        assert!(!is_wsl_unc_path(unc));
        assert!(!discovery_metadata_probe_allowed(unc));
        assert!(!discovery_metadata_probe_allowed_with_wsl(unc, true));
        assert!(!discovery_metadata_probe_allowed_with_wsl(wsl, false));
        assert!(discovery_metadata_probe_allowed_with_wsl(wsl, true));
        assert_eq!(canonical_or_original(unc), unc.to_path_buf());
        assert_eq!(existing_directory_candidate(&unc.to_string_lossy()), None);
        assert_eq!(nearest_project_root_for_path(unc), None);
        assert!(bounded_metadata_files(unc, 2, 10).is_empty());
    }
    ...
    fn deep_scan_finds_nested_project_and_selected_project_root() {
        ...
        let report = discover_projects_in_root_with_sources(
            dir.path(),
            &[],
            DiscoveryOptions {
                limit: 20,
        ...
        let root_report = discover_projects_in_root_with_sources(
            &project,
            &[],
            DiscoveryOptions {
                limit: 20,
    ...
    fn codex_index_thread_names_retitle_rollout_sessions() {
        ...
        let rollout =
            day.join("rollout-2026-06-14T10-00-00-11111111-1111-4111-8111-111111111111.jsonl");
        fs::write(
            &rollout,
            format!(
        ...
        fs::write(
            &index,
            "\u{feff}{\"id\":\"11111111-1111-4111-8111-111111111111\",\"thread_name\":\"Fix the flaky build\",\"updated_at\":\"2026-06-14T10:00:00Z\"}\n{\"id\":\"ffffffff-ffff-ffff-ffff-ffffffffffff\",\"thread_name\":\"Unmatched\"}\n",
        )
        .unwrap();
        ...
        assert_eq!(
            codex_rollout_session_id(Path::new(
                "transcript-2026-06-14T10-00-00-11111111-1111-4111-8111-111111111111.jsonl"
            )),
            None
        );
    }
    ...
    fn untitled_claude_transcript_uses_first_user_message_as_display_name() {
        ...
        let dir = home
            ...
            .join("NamedProj");
        fs::create_dir_all(&dir).unwrap();
        let transcript = dir.join("33333333-3333-4333-8333-333333333333.jsonl");
        fs::write(
            &transcript,
            format!(
    ...
    fn claude_local_json_uses_title_instead_of_local_uuid() {
        ...
        fs::create_dir_all(&dir).unwrap();
        let session = dir.join("local_55555555-5555-4555-8555-555555555555.json");
        fs::write(
            &session,
            format!(
    ...
    fn technical_claude_display_name_is_replaced_when_title_is_derivable() {
        ...
        add_session_candidate_with_display(
            ...
            &source,
            &[project],
            Some("33333333-3333-4333-8333-333333333333.jsonl".to_string()),
            None,
        );
    ...
    fn archived_codex_rollout_ignores_stray_body_dirs() {
        ...
        let rollout = archived_dir
            .join("rollout-2026-06-14T10-00-00-11111111-1111-4111-8111-111111111111.jsonl");
    ...
    fn distinct_codex_rollouts_sharing_title_and_cwd_are_not_deduped() {
        ...
        let a = archived("rollout-2026-06-14T10-00-00-11111111-1111-4111-8111-111111111111.jsonl");
        let b = archived("rollout-2026-06-14T11-00-00-22222222-2222-4222-8222-222222222222.jsonl");
        assert_ne!(
        ...
        let a_again =
            archived("rollout-2026-06-14T10-00-00-11111111-1111-4111-8111-111111111111.jsonl");
        assert_eq!(
            session_visible_identity_key(&a),
    ...
    fn distinct_claude_transcripts_sharing_title_and_cwd_are_not_deduped() {
        ...
        let a = transcript("33333333-3333-4333-8333-333333333333.jsonl");
        let b = transcript("44444444-4444-4444-8444-444444444444.jsonl");
        assert_ne!(
            session_visible_identity_key(&a),
            session_visible_identity_key(&b),
        ...
        let a_again = transcript("33333333-3333-4333-8333-333333333333.jsonl");
        assert_eq!(
            session_visible_identity_key(&a),
            session_visible_identity_key(&a_again),
        ...
        let mut a_moved = transcript("33333333-3333-4333-8333-333333333333.jsonl");
        a_moved.path =
            "D:/backup/.claude/projects/other/33333333-3333-4333-8333-333333333333.jsonl"
                .to_string();
        assert_eq!(
            session_visible_identity_key(&a),
    ...
    #[test]
    #[ignore = "depends on the local user's real OpenCode installation"]
    fn real_opencode_store_discovers_and_renders_one_session_without_logging_content() {
        let source = discovery_sources()
            .into_iter()
            .find(|source| source.mode == SourceMode::OpenCodeState && source.path.is_file())
            .expect("installed OpenCode state database");
        let mut candidates = BTreeMap::new();
        let mut sessions = BTreeMap::new();
        scan_opencode_state_source(&source, &mut candidates, &mut sessions);
        assert!(
            !candidates.is_empty(),
            "OpenCode project rows resolve locally"
        );
        let session = sessions
            .into_values()
            .next()
            .expect("at least one OpenCode session");
        let fragment = session
            .path
            .to_string_lossy()
            .rsplit_once('#')
            .map(|(_, fragment)| fragment.to_string())
            .expect("OpenCode session fragment");
        let session_id = fragment
            .strip_prefix("opencode-session=")
            .expect("OpenCode session id");
        let (transcript, _) = opencode_session_transcript_window(
            &source.path,
            session_id,
            OPENCODE_TRANSCRIPT_MAX_BYTES,
            false,
        )
        .expect("read one OpenCode transcript");
        assert!(
            !transcript.is_empty(),
            "selected conversation has renderable text"
        );
        assert!(transcript.len() <= OPENCODE_TRANSCRIPT_MAX_BYTES);
    }
    ...
    fn discovery_completeness_breakdown() {
        ...
        assert!(has("chatgpt"), "ChatGPT sessions present");
        assert!(has("claude"), "Claude sessions present");
        assert!(has("opencode"), "OpenCode sessions present");
        assert!(has("oh my pi"), "Oh My Pi sessions present");
        assert!(
    ...
    fn write_synthetic_opencode_db(path: &Path, project: &Path) {
        let conn = Connection::open(path).unwrap();
        conn.execute_batch(
            "CREATE TABLE project (\
                 id TEXT PRIMARY KEY, worktree TEXT, time_updated INTEGER\
             );\
             CREATE TABLE project_directory (\
                 project_id TEXT, directory TEXT\
             );\
        ...
        conn.execute(
            "INSERT INTO project (id, worktree, time_updated) VALUES ('project-1', ?1, 2000)",
            [&project_path],
        )
        .unwrap();
        conn.execute(
            "INSERT INTO project_directory (project_id, directory) VALUES ('project-1', ?1)",
            [&project_path],
        )
        .unwrap();
        conn.execute(
            "INSERT INTO session (id, project_id, directory, title, time_created, time_updated) \
             VALUES ('session-1', 'project-1', ?1, 'Implementar pesquisa incremental', 1000, 2000)",
            [&project_path],
        )
        .unwrap();
        for (id, role, time) in [
            ("message-1", "user", 1000_i64),
            ("message-2", "assistant", 2000),
        ] {
            let data = serde_json::json!({"role": role, "time": {"created": time}}).to_string();
            conn.execute(
        ...
        for (id, message_id, time, data) in [
            (
                "part-1",
                "message-1",
                1000_i64,
                serde_json::json!({"type": "text", "text": "primeira pergunta"}),
            ),
            (
                "part-2",
                "message-2",
                2000_i64,
                serde_json::json!({"type": "reasoning", "text": "raciocinio privado"}),
            ),
            (
                "part-3",
                "message-2",
                2001_i64,
                serde_json::json!({"type": "text", "text": "resposta final"}),
            ),
        ] {
            conn.execute(
                "INSERT INTO part (id, message_id, session_id, time_created, time_updated, data) \
                 VALUES (?1, ?2, 'session-1', ?3, ?3, ?4)",
                rusqlite::params![id, message_id, time, data.to_string()],
            )
            .unwrap();
        }
    }
    ...
    #[test]
    fn opencode_state_discovers_registered_project_and_named_session() {
        let dir = tempdir_projectlike();
        let project = dir.path().join("open-code-project");
        fs::create_dir_all(&project).unwrap();
        fs::write(project.join("README.md"), "# Project").unwrap();
        let store = dir.path().join("opencode.db");
        write_synthetic_opencode_db(&store, &project);
        let source = DiscoverySource {
            kind: "opencode_state".to_string(),
            label: "OpenCode projects and sessions".to_string(),
            path: store,
        ...
        assert!(candidates.iter().any(|candidate| {
            same_path(Path::new(&candidate.path), &project)
                && candidate
                    .source_kinds
                    .iter()
                    .any(|kind| kind == "opencode_state")
        }));
        let sessions = finalize_sessions(sessions, &[]);
        assert_eq!(sessions.len(), 1);
        assert_eq!(sessions[0].display_name, "Implementar pesquisa incremental");
        assert_eq!(sessions[0].session_kind, "OpenCode");
        assert!(sessions[0].path.contains("#opencode-session=session-1"));
        assert!(sessions[0]
            .linked_project_paths
            .iter()
            .any(|path| same_path(Path::new(path), &project)));
    }

    #[test]
    fn opencode_preview_reads_only_text_parts_in_chronological_order() {
        let dir = tempdir_projectlike();
        let project = dir.path().join("preview-project");
        fs::create_dir_all(&project).unwrap();
        fs::write(project.join("README.md"), "# Project").unwrap();
        let store = dir.path().join("opencode.db");
        write_synthetic_opencode_db(&store, &project);

        let (transcript, truncated) = opencode_session_transcript_window(
            &store,
            "session-1",
            OPENCODE_TRANSCRIPT_MAX_BYTES,
            false,
        )
        .expect("OpenCode transcript");
        assert!(!truncated);
        assert!(
            transcript.find("primeira pergunta").unwrap()
                < transcript.find("resposta final").unwrap()
        );
        assert!(!transcript.contains("raciocinio privado"));
        assert!(opencode_session_transcript_window(
            &store,
            "another-session",
            OPENCODE_TRANSCRIPT_MAX_BYTES,
            false,
    ...
    #[test]
    fn omp_jsonl_uses_title_and_authoritative_session_cwd() {
        let dir = tempdir_projectlike();
        let project = dir.path().join("omp-project");
        let sessions_dir = dir.path().join(".omp").join("agent").join("sessions");
        fs::create_dir_all(&project).unwrap();
        fs::create_dir_all(&sessions_dir).unwrap();
        fs::write(project.join("README.md"), "# OMP project").unwrap();
        let transcript = sessions_dir.join("session-omp.jsonl");
        fs::write(
            &transcript,
            format!(
                "{}\n{}\n{}\n",
                serde_json::json!({"type": "title", "title": "Rever o scanner"}),
                serde_json::json!({"type": "session", "id": "omp-1", "cwd": project}),
                serde_json::json!({"type": "message", "message": {"role": "user", "content": "olá"}}),
            ),
        )
        .unwrap();
        let source = DiscoverySource {
        ...
        let mut candidates = BTreeMap::new();
        let mut sessions = BTreeMap::new();
        scan_metadata_file(&transcript, &source, &mut candidates, &mut sessions);
        let sessions = finalize_sessions(sessions, &[]);
        assert_eq!(sessions.len(), 1);
        assert_eq!(sessions[0].display_name, "Rever o scanner");
        assert_eq!(sessions[0].session_kind, "Oh My Pi");
        assert!(sessions[0]
            .linked_project_paths
            .iter()
            .any(|path| same_path(Path::new(path), &project)));
    ...
    #[test]
    fn opencode_and_omp_roots_flow_into_project_app_attribution() {
        let dir = tempdir_projectlike();
        let opencode_project = dir.path().join("opencode-only");
        let omp_project = dir.path().join("omp-only");
        let shared_project = dir.path().join("shared-project");
        for project in [&opencode_project, &omp_project, &shared_project] {
            fs::create_dir_all(project).unwrap();
            fs::write(project.join("README.md"), "# Project").unwrap();
        }

        let store = dir.path().join("opencode.db");
        write_synthetic_opencode_db(&store, &opencode_project);
        let conn = Connection::open(&store).unwrap();
        conn.execute(
            "INSERT INTO project_directory (project_id, directory) VALUES ('project-2', ?1)",
            [shared_project.to_string_lossy().to_string()],
        )
        .unwrap();
        drop(conn);

        let sessions_dir = dir.path().join(".omp").join("agent").join("sessions");
        let nested_sessions = sessions_dir.join("2026").join("08");
        fs::create_dir_all(&nested_sessions).unwrap();
        for (name, project) in [
            ("omp-only.jsonl", &omp_project),
            ("shared.jsonl", &shared_project),
        ] {
            fs::write(
                nested_sessions.join(name),
                format!(
                    "{}\n{}\n",
                    serde_json::json!({"type": "title", "title": "Bounded attribution"}),
                    serde_json::json!({"type": "session", "cwd": project})
                ),
            )
            .unwrap();
        }
        // Sidecars are not conversations and must not contribute cwd roots.
        fs::write(
            sessions_dir.join("sidecar.log"),
            serde_json::json!({"cwd": dir.path().join("not-a-project")}).to_string(),
        )
        .unwrap();

        let opencode_roots = opencode_project_roots_from_store(&store);
        let omp_roots = omp_project_roots_from_sessions(&sessions_dir);
        let opencode_keys = opencode_roots
            .iter()
            .map(|path| candidate_key(path))
            .collect::<BTreeSet<_>>();
        let omp_keys = omp_roots
            .iter()
            .map(|path| candidate_key(path))
            .collect::<BTreeSet<_>>();
        assert_eq!(opencode_keys.len(), 2);
        assert!(opencode_keys.contains(&candidate_key(&opencode_project)));
        assert!(opencode_keys.contains(&candidate_key(&shared_project)));
        assert_eq!(omp_keys.len(), 2);
        assert!(omp_keys.contains(&candidate_key(&omp_project)));
        assert!(omp_keys.contains(&candidate_key(&shared_project)));

        let mut owners = RegistryOwners::new();
        record_precomputed_app_roots(&mut owners, &[], &[], &opencode_roots, &omp_roots);
        let mut states = std::collections::HashMap::new();
        apply_project_app_attribution(&mut states, finish_registry_owners(owners));

        let opencode_state = states.get(&candidate_key(&opencode_project)).unwrap();
        assert_eq!(opencode_state.app.as_deref(), Some("opencode"));
        assert_eq!(opencode_state.apps, ["opencode"]);
        let omp_state = states.get(&candidate_key(&omp_project)).unwrap();
        assert_eq!(omp_state.app.as_deref(), Some("omp"));
        assert_eq!(omp_state.apps, ["omp"]);
        let shared_state = states.get(&candidate_key(&shared_project)).unwrap();
        assert_eq!(shared_state.app.as_deref(), Some("opencode"));
        assert_eq!(shared_state.apps, ["omp", "opencode"]);
    }
    ...
    fn antigravity_transcript_recovers_steps_in_chronological_order() {
        ...
        assert!(transcript.contains("final synthetic marker"));
        // Oldest-first ordering: step 1 appears before step 3.
        let first = transcript.find("first user prompt").unwrap();
        let last = transcript.find("final synthetic marker").unwrap();
    ...
    fn antigravity_session_title_uses_first_human_text_instead_of_uuid() {
        let dir = tempdir().unwrap();
        let db = dir.path().join("88888888-8888-4888-8888-888888888888.db");
        write_synthetic_conversation_db(
            &db,
            &[
    ...
    fn gemini_project_roots_for_uuid_collects_all_existing_roots() {
        ...
        // drive, using a synthetic offline nested path. An absent drive letter means
        // none of its ancestors
        // exist either, so the shared `file://`-decoder (which walks up to the nearest
        // existing dir) surfaces nothing for it.
        let missing = PathBuf::from("Z:\\offline\\Nested\\SampleProject");
    ...
    fn antigravity_conversation_links_all_project_roots() {
        // The registry-handoff case: the proto pins the conversation to a folderUri that no
        ...
        // using a synthetic disconnected-drive situation.
        let disconnected = PathBuf::from("Z:\\offline\\Nested\\SampleProject");
    ...
    fn antigravity_proto_parse_extracts_project_and_loose() {
        let proto = encode_antigravity_proto(&[
            encode_antigravity_record(
                ...
                Some("file:///c%3A/Synthetic/CodeHangar"),
        ...
        assert_eq!(
            ...
            Some(&AntigravityResolution::Project {
                path: PathBuf::from("C:\\Synthetic\\CodeHangar"),
                project_uuid: "99999999-9999-4999-8999-999999999999".to_string(),
            }),
    ...
    fn antigravity_current_keys_collects_project_paths_only() {
        ...
        // The synthetic drive path does not exist in the test env, but the key is still recorded
    ...
    fn registry_app_specificity_prefers_most_deliberate_owner() {
        // Authoritative/project registries beat session-only cwd attribution.
        assert!(registry_app_specificity("antigravity") > registry_app_specificity("cursor"));
        assert!(registry_app_specificity("cursor") > registry_app_specificity("opencode"));
        assert!(registry_app_specificity("opencode") > registry_app_specificity("codex"));
        assert!(registry_app_specificity("codex") > registry_app_specificity("claude_code"));
        assert_eq!(
            registry_app_specificity("omp"),
            registry_app_specificity("claude_code")
        );
    ...
    /// Opt-in local-store consistency check. It never embeds or prints project names,
    /// paths, conversation identifiers or transcript content.
    #[test]
    #[ignore = "depends on the local user's real Antigravity data"]
    fn real_antigravity_current_set_matches_project_resolutions() {
    ...
    /// Opt-in structural validation of a local summaries proto. Real identifiers,
    /// project names and paths remain outside the source tree and test output.
    #[test]
    #[ignore = "depends on the local user's real Antigravity data"]
    fn real_antigravity_proto_resolutions_are_structurally_valid() {
        let Some(proto) = antigravity_summaries_proto_path() else {
            eprintln!("skipping: no home dir");
            return;
        };
        ...
        assert!(
            !map.is_empty(),
            "local summaries proto should contain records"
        );
        assert!(map.values().all(|resolution| match resolution {
            AntigravityResolution::Loose => true,
            AntigravityResolution::Project { path, project_uuid } =>
                !path.as_os_str().is_empty() && !project_uuid.is_empty(),
        }));
    }

    /// Opt-in local-store check that resolved conversation links point only at
    /// existing roots. Output is aggregate-only and contains no local identifiers.
    #[test]
    #[ignore = "depends on the local user's real Antigravity data"]
    fn real_antigravity_conversation_links_are_existing_roots() {
        let Some(home) = home_dir() else {
            eprintln!("skipping: no home dir");
            return;
        };
        ...
        for entry in entries.flatten() {
            ...
            linked += links.len();
            assert!(links.iter().all(|path| path.is_dir()));
        }
        eprintln!("inspected {inspected} local conversation records; {linked} existing links");
    }
    ...
    fn real_project_app_states_have_multi_app_membership() {
        ...
        if opencode_state_db_for_home(&home).is_file() {
            assert!(
                states
                    .values()
                    .any(|state| state.apps.iter().any(|app| app == "opencode")),
                "installed OpenCode store should attribute at least one project"
            );
        }
        if omp_sessions_dir_for_home(&home).is_dir() {
            assert!(
                states
                    .values()
                    .any(|state| state.apps.iter().any(|app| app == "omp")),
                "installed OMP store should attribute at least one project"
            );
        }
    }
    ...
    fn extract_session_cwd_reads_payload_and_toplevel() {
        ...
        let codex = concat!(
            "{\"type\":\"other\",\"foo\":1}\n",
            "{\"type\":\"session_meta\",\"payload\":{\"cwd\":\"C:\\\\Synthetic\\\\CodeHangarDemo\"}}\n"
        );
        assert_eq!(
            extract_session_cwd(codex),
            Some(PathBuf::from("C:\\Synthetic\\CodeHangarDemo"))
        );
    ...
    #[cfg(windows)]
    #[test]
    fn wsl_sources_absent_from_disabled_request_snapshot() {
        // `system(false)` returns before the distro cache/enumerator or any WSL
        // share probe. Building the production source list from that immutable
        // snapshot therefore cannot register a WSL source.
        let snapshot = WslDiscoverySnapshot::system(false);
        let sources = discovery_sources_with_wsl_snapshot(&snapshot);
        assert!(
            !sources.iter().any(|s| {
    ...
    fn codex_session_links_only_cwd() {
        ...
        let parent = home.path().join("SampleGame-Lab");
        let child = parent.join("engine");
    ...
    /// Opt-in local-store smoke check. It validates only aggregate/root invariants
    /// and never embeds or prints machine-specific project identities.
    #[test]
    #[ignore]
    fn real_session_cwd_discovery() {
        ...
        for path in claude.iter().chain(codex.iter()) {
            assert!(path.is_absolute());
            assert!(path.is_dir());
            assert!(!is_agent_scratch_path(path));
        }
        ...
        let codex_keys = codex
            ...
            .map(|path| candidate_key(path))
            .collect::<BTreeSet<_>>();
        assert_eq!(claude_keys.len(), claude.len());
        assert_eq!(codex_keys.len(), codex.len());
        eprintln!(
            "validated {} Claude roots and {} Codex roots",
            claude.len(),
            codex.len()
        );
    }
    ...
    fn cursor_uri_to_path_decodes_all_three_shapes() {
        ...
        let uri = serde_json::json!({ "external": "file:///c%3A/Synthetic/CodeHangarDemo" });
        assert_eq!(
            cursor_uri_to_path(&uri),
            Some(PathBuf::from("C:\\Synthetic\\CodeHangarDemo"))
        );
    ...
    #[test]
    fn nearest_project_root_prefers_closest_marker_ancestor() {
        let outer = tempfile::tempdir().unwrap();
        fs::write(outer.path().join("package.json"), "{}").unwrap();
        let nested = outer.path().join("packages").join("docs-app");
        let docs = nested.join("docs");
        fs::create_dir_all(&docs).unwrap();
        fs::write(nested.join("Cargo.toml"), "[package]\nname='docs-app'").unwrap();
        let target = docs.join("guide.md");
        fs::write(&target, "# Guide").unwrap();

        assert_eq!(nearest_project_root_for_path(&target), Some(nested));
    }

    #[test]
    fn nearest_project_root_returns_none_without_markers() {
        let folder = tempfile::tempdir().unwrap();
        let target = folder.path().join("notes.md");
        fs::write(&target, "# Notes").unwrap();

        assert_eq!(nearest_project_root_for_path(&target), None);
    }
