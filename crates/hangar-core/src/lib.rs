...
/// Read-only inspection of a file or folder supplied by the operating-system
/// shell. Unknown paths are inspected before any root is registered or scanned
/// so the UI can ask how the user wants to open them.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct OpenTargetInspection {
    pub input_path: String,
    pub target_kind: String,
    pub known_project_root: Option<String>,
    pub automatic_project_root: String,
    pub viewer_root: String,
}

/// Result of resolving a file or folder supplied by the operating-system shell.
/// The request is always a local path; Code Hangar never accepts a URL here.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct OpenTargetPreparation {
    pub input_path: String,
    pub target_kind: String,
    ...
    pub scan_already_running: bool,
    pub open_mode: String,
    pub temporary: bool,
}

/// Atomic outcome of asking the resident scanner to refresh a prepared
/// shell-open root. `started_here` is authoritative ownership information from
/// the same admission decision that created or reused `job_id`; callers must not
/// infer it from an earlier root snapshot.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct OpenTargetScanStart {
    pub job_id: String,
    pub started_here: bool,
}

/// Current per-user Windows shell registration. These are preferences, not
/// claims that Windows has made Code Hangar the user's default Markdown app.
...
pub struct ShellIntegrationStatus {
    ...
    /// Whether the current user's Code Hangar Run entry is present. Ownership
    /// is reported separately so another installed edition is never shown as
    /// disabled while Windows is still launching it.
    pub run_at_login: bool,
    pub run_at_login_owned_by_current_edition: bool,
    pub run_at_login_owner_executable: Option<String>,
    /// Resident refresh is part of the desktop base build and stays local-only.
    pub background_refresh_enabled: bool,
    /// Closing the main window hides it while the tray process remains alive.
    pub close_to_tray: bool,
    pub owned_by_current_edition: bool,
    pub default_guide_pending: bool,
    pub owner_executable: Option<String>,
}
...
pub struct ProjectSummary {
    ...
    /// even when no parsed session links to it (for example, when conversations are
...
/// top-level markdown, and manifest files. It remains heuristic and read-only.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq, Default)]
#[serde(rename_all = "camelCase")]
...
pub struct NavItem {
    ...
    /// True when Windows reports an online-only Cloud Files placeholder. Its
    /// metadata is present, but Code Hangar has not opened or indexed its body.
    #[serde(default)]
    pub cloud_only: bool,
...
pub struct GraphEdge {
    ...
    /// Project membership that produced this edge. Synthetic graph-only edges
    /// such as project containment and duplicate warnings have no persisted
    /// source membership and therefore leave this unset.
    #[serde(default)]
...
pub struct ProjectReviewCheckpoint {
    ...
    /// Content-free identity for the set of undated local session records that
    /// was acknowledged at this checkpoint.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub undated_session_fingerprint: Option<String>,
...
/// A metadata-only file event recorded by the resident local change tracker.
/// Project files are never modified and file bodies are never copied into this
/// log; the encrypted inventory stores only path, timestamps and size signals.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct FileChangeEvent {
    ...
    pub modified_at: Option<String>,
    pub size_bytes: Option<u64>,
    /// Local AI/editor applications currently associated with this project.
    /// Correlation is best-effort and is derived when the event is read.
    pub apps: Vec<String>,
}
...
pub enum AutomationAgentKind {
    LocalTool,
    ConnectedApp,
}

#[cfg(feature = "agent_automation")]
impl AutomationAgentKind {
    ...
    pub fn from_storage(value: &str) -> Option<Self> {
        match value {
            "local_tool" => Some(Self::LocalTool),
            "connected_app" => Some(Self::ConnectedApp),
            _ => None,
        }
    }
}
...
impl AutomationTransport {
    ...
    pub fn from_storage(value: &str) -> Option<Self> {
        match value {
            "named_pipe" => Some(Self::NamedPipe),
...
pub struct AutomationAgentSummary {
    pub id: i64,
    /// Stable, opaque identity used for authorization and record ownership.
    /// Display names are never treated as an authority boundary.
    pub identity_id: String,
    pub name: String,
    pub agent_kind: AutomationAgentKind,
    pub allowed_transport: AutomationTransport,
    pub connected_host: Option<ConnectedAppHost>,
...
pub struct OperationPlan {
    ...
    pub plan_stale: bool,
    pub partial_footprint: bool,
    /// False means relationship indexes are still pending/failed somewhere;
    /// empty shared/dangling sections must not be interpreted as proof of none.
...
mod tests {
    ...
    fn project_summary_serializes_is_current_and_app_camel_case() {
        let summary = ProjectSummary {
            ...
            scan_root_id: None,
            antigravity_name: Some("SampleProject".to_string()),
            is_current: true,
