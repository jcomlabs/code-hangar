...
const COLLAPSED_REGISTRY_FILE_COUNT: usize = 1_000;
...
fn large_adversarial_inventory_stays_bounded_and_cancellable() {
    ...
    assert!(
        summary.scanned_files > PROJECT_FILE_COUNT as u64,
        "the project files and their directory nodes should be inventoried: {summary:?}"
    );
    ...
    assert!(!paths
        .iter()
        .any(|path| path.starts_with(".local/cargo/registry/")));

    let registry = items
        .iter()
        .find(|item| item.relative_path == ".local/cargo/registry")
        .unwrap();
    assert!(registry.collapse_default);
    assert!(registry
        .identity
        .as_ref()
        .and_then(|identity| identity.size_apparent)
        .is_some_and(
            |bytes| bytes >= (COLLAPSED_REGISTRY_FILE_COUNT * b"fn vendored() {}".len()) as u64
        ));
