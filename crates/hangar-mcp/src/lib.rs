...
//! - **Transport-bound identity.** The startup token is consumed once to bind
//!   this stdio child to an immutable connected-host identity. Tool calls carry
//!   no token and dispatch through hangar-api's MCP-only allowlist, live
//!   revocation, scope/project gates and audit log. A connected-app credential
//!   is rejected at the named-pipe ingress.
//! - **No file bodies.** No tool or resource exposes file contents; the read_body
//!   path is never linked here.

use std::io::{self, BufRead, Read, Write};

use hangar_agent::{AgentMethod, AgentResponse};
...
enum ToolGate {
    ...
    /// A queue-only `request_*` tool: needs the given scope AND the advanced-request
    /// tier. It only files a request; any applicable local action still needs review.
    Request(&'static str),
    /// `request_final_remove`: like `Request`, but ALSO needs the connected-app
    /// recommendation authorization. It can only direct the owner to local Recovery.
...
impl ToolGate {
    ...
    /// additionally requires the connected-app final-removal authorization. When the token did not resolve
...
fn project_node_id_schema() -> Value {
    json!({
        "type": "object",
        "properties": {
            "projectId": { "type": "integer", "description": "The exact Code Hangar project membership to query." },
            "nodeId": { "type": "integer", "description": "The node id within that project membership." }
        },
...
fn tools() -> Vec<ToolDef> {
    vec![
        ...
        ToolDef {
            ...
            description: "A node's incoming and outgoing relationships within an explicit project membership. Both projectId and nodeId are required; edges into projects you cannot see are omitted.",
            method: AgentMethod::NodeRelationships,
            input_schema: project_node_id_schema,
            gate: ToolGate::Read("read_graph"),
            behavior: ToolBehavior::ReadOnly,
        },
        ToolDef {
        ...
        ToolDef {
            ...
            description: "Whether a single node looks orphaned in an explicit project membership, and why (reference count, confidence). Both projectId and nodeId are required.",
        ...
        ToolDef {
            ...
            title: "Recommend final-removal review",
            description: "Recommend that the owner review a prior holding-area entry in Code Hangar's separate local Recovery flow. This queues a recommendation only: it never deletes, approves or starts a final-removal batch.",
...
/// `request_final_remove` only with the connected-app final-removal authorization too), comment-writes
...
/// The MCP server bound to one opened encrypted database and one immutable
/// connected-host identity. The plaintext startup token is not retained.
pub struct ConnectedAppServer {
    state: AppState,
    binding: Option<hangar_api::McpTransportBinding>,
}
...
impl ConnectedAppServer {
    pub fn bind(
        ...
        let binding =
            hangar_api::bind_mcp_transport(&state, &token, &host_id, &expected_identity_id)?;
        // `token` is consumed and dropped here. It never becomes session state or
        // an AgentRequest field.
        drop(token);
        Ok(Self {
            state,
    ...
    #[cfg(test)]
    fn unbound_for_protocol_tests(state: AppState) -> Self {
        Self {
            state,
            binding: None,
        }
    ...
    fn visible_tools(&self) -> Value {
        let Some(binding) = self.binding.as_ref() else {
            return tool_definitions_for(&hangar_api::McpCatalogContext {
                scopes: None,
                total_control_enabled: false,
                final_remove_enabled: false,
            });
        };
        match hangar_api::mcp_catalog_context_bound(&self.state, binding) {
    ...
    fn tools_call(&self, id: Value, params: Value) -> String {
        ...
        let arguments = params
            ...
            .cloned()
            .unwrap_or_else(|| json!({}));
        let tool = match tool_for_name(name) {
            Some(tool) => tool,
            None => {
        ...
        let Some(binding) = self.binding.as_ref() else {
            let request_id = request_id_label(&id);
            return success_response(
                id,
                agent_response_to_tool_result(AgentResponse::failure(
                    request_id,
                    "This MCP session has no bound connected-app identity.",
                )),
            );
        };
        let context = match hangar_api::mcp_catalog_context_bound(&self.state, binding) {
            Ok(context) => context,
            Err(_) => {
                let request_id = request_id_label(&id);
                return success_response(
        ...
        if !tool.gate.visible_to(&context) {
            let message = if context.scopes.is_none() {
                "The authentication token has no effective connected-app scopes."
            } else {
                "This tool is not enabled for this connected app."
            };
            let request_id = request_id_label(&id);
            return success_response(
                id,
                agent_response_to_tool_result(AgentResponse::failure(request_id, message)),
            );
        }
        let response = hangar_api::dispatch_mcp_bound_request(
            &self.state,
            binding,
            request_id_label(&id),
            tool.method,
...
mod tests {
    ...
    fn server() -> ConnectedAppServer {
        ConnectedAppServer::unbound_for_protocol_tests(AppState::memory().unwrap())
    }
    ...
    #[test]
    fn membership_sensitive_node_tools_require_project_and_node_ids() {
        for name in ["node_relationships", "node_orphan_status"] {
            let definition = tools()
                .into_iter()
            ...
            assert!(required.iter().any(|value| value == "projectId"), "{name}");
            assert!(required.iter().any(|value| value == "nodeId"), "{name}");
            assert_eq!(required.len(), 2, "{name}");
    ...
    fn tools_list_is_scope_and_tier_aware() {
        ...
        // The one-click connector scopes are body-free. History search and
        // mutation requests appear only when their separate scopes are present.
        let base_scopes = vec![
            "comments_read".to_string(),
            "comments_write".to_string(),
            "read_structure".to_string(),
        ...
        let base = names_of(&tool_definitions_for(&hangar_api::McpCatalogContext {
            scopes: Some(base_scopes.clone()),
            total_control_enabled: true,
            final_remove_enabled: true,
        }));
        assert!(!base.contains(&"search_sessions".to_string()));
        assert!(!base.contains(&"request_backup_protected".to_string()));
        assert!(!base.contains(&"request_move_to_holding".to_string()));
        assert!(!base.contains(&"request_final_remove".to_string()));
        assert!(!base.iter().any(|name| name.contains("body")));

        let mut history_scopes = base_scopes.clone();
        history_scopes.push("history_search".to_string());
        let history = names_of(&tool_definitions_for(&hangar_api::McpCatalogContext {
            scopes: Some(history_scopes),
            total_control_enabled: false,
            final_remove_enabled: false,
        }));
        assert!(history.contains(&"search_sessions".to_string()));
        assert!(!history.iter().any(|name| name.starts_with("request_")));

        let mut execute_scopes = base_scopes;
        execute_scopes.push("execute_plan".to_string());
        let execute_without_tier =
            names_of(&tool_definitions_for(&hangar_api::McpCatalogContext {
                scopes: Some(execute_scopes.clone()),
                total_control_enabled: false,
                final_remove_enabled: true,
            }));
        assert!(!execute_without_tier
            .iter()
            .any(|name| name.starts_with("request_")));
        let execute_with_tier = names_of(&tool_definitions_for(&hangar_api::McpCatalogContext {
            scopes: Some(execute_scopes),
            total_control_enabled: true,
            final_remove_enabled: false,
        }));
        assert!(execute_with_tier.contains(&"request_backup_protected".to_string()));
        assert!(execute_with_tier.contains(&"request_move_to_holding".to_string()));
        assert!(!execute_with_tier.contains(&"request_final_remove".to_string()));

        // execute_plan under total control shows backup/move requests, but
        // request_final_remove ONLY when the connected-app final-removal authorization is also on.
    ...
    #[test]
    fn tool_call_without_bound_transport_identity_is_reported_as_tool_error() {
        // Protocol-only sessions have no token field and no bound identity. A
        // tool call fails as tool content, not as a JSON-RPC protocol error.
        let response = parse(
            &server()
                .handle_line(
        ...
        let text = response["result"]["content"][0]["text"]
            ...
            .unwrap()
            .to_lowercase();
        assert!(text.contains("bound") && text.contains("identity"));
    }
    ...
    fn authenticated_list_comments_round_trip() {
        ...
        let credential = hangar_api::mcp_test_register_transport(
            &state,
            "cursor",
        ...
        let server = ConnectedAppServer {
            state,
            binding: Some(credential.binding),
        };
        let request = format!(
    ...
    #[cfg(windows)]
    #[test]
    fn bound_stdio_identity_refuses_pipe_replay_forbidden_methods_and_stale_binding() {
        let state = AppState::memory().unwrap();
        ...
        // A connected-app token is typed mcp_stdio and cannot be replayed into
        // the authenticated named-pipe AgentRequest body.
        let pipe_replay = hangar_api::dispatch_agent_request(
            &state,
            hangar_agent::AgentRequest {
                protocol: hangar_agent::PROTOCOL_VERSION.to_string(),
                request_id: "pipe-replay".to_string(),
                token: Some(credential.token.clone()),
                method: AgentMethod::AgentProjectContext,
                params: json!({ "projectId": project_id }),
            },
        );
        assert!(!pipe_replay.ok);

        for method in [
            AgentMethod::AgentReadBody,
            AgentMethod::AgentPlanBuild,
            AgentMethod::AgentPlanExecute,
            AgentMethod::RequestReadBody,
        ] {
            let response = hangar_api::dispatch_mcp_bound_request(
                &state,
                &credential.binding,
                format!("forbidden-{method:?}"),
                method,
                json!({}),
            );
            assert!(!response.ok);
        ...
        // Rotation preserves the durable app identity, but changes the exact
        // credential incarnation. An already-running process bound under the
        // old startup token must not silently regain access after reconnect.
        let rotated = hangar_api::mcp_test_register_transport(
            &state,
            "codex",
            vec![
                "read_structure".to_string(),
        ...
        .unwrap();
        assert_eq!(credential.identity_id, rotated.identity_id);
        let stale_after_rotation = hangar_api::dispatch_mcp_bound_request(
            &state,
            &credential.binding,
            "rotated-binding".to_string(),
            AgentMethod::ListCatalog,
            json!({}),
        );
        assert!(!stale_after_rotation.ok);
        assert!(stale_after_rotation.error.unwrap().contains("rotated"));
        let current_after_rotation = hangar_api::dispatch_mcp_bound_request(
            &state,
            &rotated.binding,
        ...
        assert!(current_after_rotation.ok);

        let agent_id = rotated.binding.agent_id();
        assert!(hangar_api::automation_revoke(&state, agent_id).unwrap());
        let stale = hangar_api::dispatch_mcp_bound_request(
            &state,
            &rotated.binding,
            "revoked-binding".to_string(),
            AgentMethod::ListCatalog,
            json!({}),
        );
        assert!(!stale.ok);
        assert!(stale.error.unwrap().contains("revoked"));
    }
