...
> **Historical evidence notice (2026-08):** this file records the v0.1.1 Gate-3 implementation,
> including its former incomplete final-removal toggle. The current contract makes permanent
> removal a central but default-OFF owner capability. Enabling it requires the exact in-app phrase;
> preview, confirmation and batch start all recheck it, and every operation additionally requires an
> immutable preview plus a short-lived, single-use, object/topology-bound confirmation. See
> `docs/PERMANENT_REMOVAL.md`. The retired one-entry command remains fail-closed.
...
local management. The paragraphs below describe the historical v0.1.1 release gate; the current
final-removal behavior is defined by `docs/PERMANENT_REMOVAL.md`.
...
7. **Permanent removal is gated off by default.** Hard unlink, no Recycle Bin. During the v0.1.1
   Gate-3 wave only the one-entry command used the Recover opt-in and a supervised-QA environment
   override existed. The v0.1.3 project/batch path removes that bypass and makes the durable owner
   setting a real gate at preview, confirmation and start. It also controls connected-app
   recommendation visibility. The retired one-entry command remains fail-closed. Enabling the
   capability with the exact in-app phrase still authorizes no object: a verified object-complete
   archive, protected-zone refusal, immutable preview, fresh single-use confirmation and exact-
   handle/journal boundaries remain mandatory. Hiding the control is not a safety boundary.
...
green before merging or building a mutation-enabled artifact. The current canonical release
invocation is `-AgentAutomation -SkipTauriBuild`; it runs the isolated feature lanes and
compile-only Windows release builds without creating a publishable bundle:
...
- `npm run check` (tsc + vitest), Local/Connector frontend isolation and compile-only
  Windows Local/Connector desktop + MCP-sidecar releases
...
The repository ships no GitHub Actions workflow or Dependabot configuration. Remote automation
cannot consume CI credits or create routine notifications and is not release evidence; the
hash-bound local gate is the source of truth.
...
Every current finding has focused regression or direct harness coverage. The tracked
acceptance specification defines the mandatory gates; separately sealed external acceptance
evidence records the completed release-GUI journey for one exact clean candidate.
...
> Historical v0.1.1 evidence: the implementation described below did not fully gate
> the modern batch API. The v0.1.3 contract in
> [`../PERMANENT_REMOVAL.md`](../PERMANENT_REMOVAL.md) makes the owner capability
> default-OFF and rechecks it at preview, confirmation and start, in addition to the
> fresh preview-bound confirmation.

4. **Historical v0.1.1 final-remove block.** `runMutationFinalRemove` previously short-circuited
   on `import.meta.env.PROD`; its migration-era `finalRemoveEnabled` opt-in did not cover the
   modern batch path. In v0.1.3 the one-entry surface is retired fail-closed. The shipped Recovery
   batch flow now has a real, durable, default-OFF `mutation_final_remove_enabled` gate at preview,
   confirmation and start. Enabling requires `ENABLE PERMANENT REMOVAL`; disabling blocks start
   immediately. Even while enabled, immutable preview, exact topology-group selection, fresh scoped
   confirmation, verified backup/object archive and supervised execution remain mandatory.
