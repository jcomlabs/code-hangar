...
- Known local folders: Documents, Desktop, Downloads, `C:\AI`, `source`,
  `repos`, `projects`, `dev`.
- OneDrive is not excluded: registered roots and roots named by app registries or
  local sessions remain first-class candidates. Only broad automatic traversal
  of OneDrive Documents/Desktop is omitted. An explicit cloud-backed Deep Scan
  uses a depth-8, 10,000-directory, 10-second marker-walk budget and never opens
  online-only file bodies.
...
- Cloud-backed content reads use Windows no-recall handles; online-only entries
  remain metadata-only and no discovery path changes a provider pin.
