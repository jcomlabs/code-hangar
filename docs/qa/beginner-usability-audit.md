...
> **Historical v0.1.1 evidence.** This audit records the product that was
> exercised in July 2026; its pass counts are not v0.1.3 release evidence. The
> current Local edition contains no AI/MCP or external-network client. The
> optional Connector contains local, body-free MCP plus a distinct experimental
> AI Assist surface: AI Assist starts off and can use a user-selected local
> model or external provider only after exact outbound preview and approval.
