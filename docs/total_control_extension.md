...
- ✅ **Waves 1–4** — read_body + backup_protected + move_to_holding + final_remove
  ...
  A connected app's total-control authority is off by default and never grants execution by
  itself. The same durable owner-controlled permanent-removal capability gates connected-app
  recommendation visibility and Local preview/confirmation/start, starts OFF and requires the
  exact in-app activation phrase. An AI-originated single-entry recommendation cannot execute at
  approval; it hands the owner to the same object-complete, immutable-batch review and fresh human
  confirmation.
...
| `permanent_delete` | yes | no | highest gate: the connected app needs default-off total-control authority; approval then opens the same immutable object/topology batch review and fresh human confirmation used by local Recover. No AI request can arm or bypass the local-user flow. |
...
- ✅ **Final-remove request scoping and identity.** A final-remove request resolves
  ...
  other privileged requests. Approval deliberately does not convert that entry id
  into deletion authority: the owner must open the local project/batch review.
...
4. **`permanent_delete`** (irreversible; connected-app total-control gate plus the
   same immutable, digest-bound project/batch confirmation as local Recover; the
   local-user flow has no persistent runtime opt-in).
