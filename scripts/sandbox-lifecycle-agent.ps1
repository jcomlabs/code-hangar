...
function Get-StreamSha256 {
  ...
  try {
    $Stream.Position = 0
    $value = ([System.BitConverter]::ToString($sha.ComputeHash($Stream))).Replace('-', '').ToLowerInvariant()
    $Stream.Position = 0
    return $value
  } finally { $sha.Dispose() }
}

function Assert-InstalledPath {
  param([string]$Path, [string]$InstallRoot, [string]$Label)
  if (-not [System.IO.Path]::IsPathFullyQualified($Path) -or -not [System.IO.Path]::IsPathFullyQualified($InstallRoot)) {
    throw "$Label and install root must be fully qualified."
  }
  $full = [System.IO.Path]::GetFullPath($Path)
  $root = [System.IO.Path]::GetFullPath($InstallRoot).TrimEnd('\')
  $prefix = $root + '\'
  if (-not $full.StartsWith($prefix, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "$Label escapes its registry-selected install root."
  }
  $volumeRoot = [System.IO.Path]::GetPathRoot($full)
  $current = $volumeRoot
  foreach ($segment in $full.Substring($volumeRoot.Length).Split('\', [System.StringSplitOptions]::RemoveEmptyEntries)) {
...
function Get-InstalledArtifact {
  param([string]$Path, [string]$InstallRoot, [string]$Role)
  $full = Assert-InstalledPath -Path $Path -InstallRoot $InstallRoot -Label "Installed $Role"
  if (-not (Test-Path -LiteralPath $full -PathType Leaf)) { return $null }
  $stream = [System.IO.FileStream]::new($full, 'Open', 'Read', 'Read')
  try {
    if ($stream.Length -le 0) { throw "Installed $Role is empty: $full" }
    return [pscustomobject]@{
      role = $Role
      relativePath = [System.IO.Path]::GetRelativePath([System.IO.Path]::GetFullPath($InstallRoot), $full).Replace('\', '/')
      canonicalPath = $full
...
function Get-InstalledArtifacts {
  param([string]$InstallRoot, [string]$Edition, [string]$UninstallerPath)
  if ([string]::IsNullOrWhiteSpace($InstallRoot)) { return @() }
  $root = [System.IO.Path]::GetFullPath($InstallRoot)
  $candidates = [System.Collections.Generic.List[object]]::new()
  $candidates.Add([pscustomobject]@{ role = 'parent'; path = Join-Path $root 'code-hangar-desktop.exe' })
  $candidates.Add([pscustomobject]@{ role = 'helper'; path = Join-Path $root 'code-hangar-elevated.exe' })
  if ($Edition -eq 'Connector') {
    $candidates.Add([pscustomobject]@{ role = 'mcp'; path = Join-Path $root 'code-hangar-mcp.exe' })
  }
  if (-not [string]::IsNullOrWhiteSpace($UninstallerPath)) {
    $candidates.Add([pscustomobject]@{ role = 'uninstaller'; path = $UninstallerPath })
  }
  $records = [System.Collections.Generic.List[object]]::new()
  foreach ($candidate in $candidates) {
    if (-not (Test-Path -LiteralPath $candidate.path -PathType Leaf)) { continue }
    $records.Add((Get-InstalledArtifact -Path $candidate.path -InstallRoot $root -Role $candidate.role))
  }
...
function Get-InstalledState {
  ...
  foreach ($registryPath in @(
    ...
    foreach ($entry in @(Get-ItemProperty -Path $registryPath -ErrorAction SilentlyContinue)) {
      ...
      $edition = switch ([string]$entry.DisplayName) {
        'Code Hangar' { 'Local' }
        'Code Hangar AI Connector' { 'Connector' }
        default { 'Unknown' }
      }
      ...
      $entries.Add([pscustomobject]@{
        ...
        artifacts = @(Get-InstalledArtifacts -InstallRoot $installLocation -Edition $edition -UninstallerPath $uninstaller)
