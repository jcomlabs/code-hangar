...
use hangar_appconfig::{Host, RegistrationBinding, ServerSpec};
...
fn prepare(root: &Path, server: &Path) -> Result<(), String> {
    ...
    for host in Host::ALL {
        ...
        let registration_binding =
            RegistrationBinding::from_hex(&credential.identity_id, &credential.state_auth_key_hex)?;
        let spec = ServerSpec {
        ...
        let owner = format!("acceptance-prepare-{}-{timestamp_ms}", host.id());
        let _host_lease = hangar_appconfig::acquire_host_operation_lock(host, &home, &owner)?;
        ...
        .map_err(|error| format!("{} registration prepare failed: {error}", host.label()))?;
        if let Err(error) = prepared.apply() {
            let _ = prepared.rollback();
            let _ = hangar_api::automation_revoke(&state, credential.binding.agent_id());
            return Err(format!(
                "{} registration write failed: {error}",
                host.label()
            ));
        }
        if let Err(error) = prepared.finalize() {
            ...
            let _ = hangar_api::automation_revoke(&state, credential.binding.agent_id());
            return Err(format!(
                "{} registration finalize failed: {error}",
                host.label()
            ));
        }
        clients.insert(
            host.id().to_string(),
            json!({
                "agentId": credential.binding.agent_id(),
                "agentName": host.label(),
                "identityId": credential.identity_id,
                "stateAuthKeyHex": credential.state_auth_key_hex,
...
fn audit(fixture_path: &Path) -> Result<(), String> {
    ...
    for (host_id, client) in clients {
        ...
        let identity_id = client
            ...
            .ok_or_else(|| format!("{host_id} is missing identityId"))?;
...
fn audit_host(
    ...
    for method in required_methods {
        let read = activity.iter().find(|entry| {
            entry.agent_id == Some(agent_id) && entry.method == *method && entry.status == "allowed"
        });
...
fn disconnect(fixture_path: &Path) -> Result<(), String> {
    ...
    for (host_id, client) in clients {
        ...
        let identity_id = client
            ...
            .and_then(Value::as_str)
            .ok_or_else(|| format!("{host_id} is missing identityId"))?;
        let state_auth_key_hex = client
            .get("stateAuthKeyHex")
            .and_then(Value::as_str)
            .ok_or_else(|| format!("{host_id} is missing stateAuthKeyHex"))?;

        // Bind while the credential is live, then prove revocation invalidates
        // this already-open, token-free stdio identity.
        let transport_binding =
            hangar_api::bind_mcp_transport(&state, token, host_id, identity_id)?;
        let registration_binding = RegistrationBinding::from_hex(identity_id, state_auth_key_hex)?;
        let owner = format!("acceptance-disconnect-{host_id}-{agent_id}");
        let _host_lease = hangar_appconfig::acquire_host_operation_lock(host, &home, &owner)?;
        let prepared =
            hangar_appconfig::prepare_unregister_authenticated(host, &home, &registration_binding)?
                .ok_or_else(|| {
                    format!("{host_id} authenticated Code Hangar config entry was not found")
                })?;
        prepared.apply()?;
        ...
        if !revoked {
            let _ = prepared.rollback();
            return Err(format!("{host_id} credential was not live at disconnect"));
        }
        prepared.finalize()?;
        let removed = !hangar_appconfig::inspect(host, &home).status.registered;
        let denied = !hangar_api::dispatch_mcp_bound_request(
            &state,
            &transport_binding,
            format!("revoked-{host_id}"),
