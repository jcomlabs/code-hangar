...
interface ProjectSidebarQueryContext {
...
export function orderSidebarProjects(projects: ProjectSummary[], options: SidebarProjectOptions): OrderedSidebarProjects {
  ...
  const matches = (project: ProjectSummary) => {
    const statusBucket = getStatusBucket(project);
    if (options.appFilter !== "all" && !projectAppMetas(project).some((meta) => meta.slug === options.appFilter)) return false;
    if (options.statusFilter !== "all" && statusBucket !== options.statusFilter) return false;
    return projectMatchesSidebarQuery(project, options.query ?? "", {
      statusBucket,
      archived: archivedProjectIds.has(project.id)
    });
  };
...
export function projectMatchesSidebarQuery(
  ...
  query: string,
  context: ProjectSidebarQueryContext = {}
) {
  const tokens = normalizeProjectSearchText(query).split(/\s+/).filter(Boolean);
  if (tokens.length === 0) return true;
  const appLabels = projectAppMetas(project).flatMap((meta) => [meta.slug, meta.label]);
  const statusBucket = context.statusBucket ?? projectStatusBucketFromScanState(project.scanState ?? "scanned");
  const statusKeywords = statusBucket === "needs-scan" ? ["needs scan", "needs-scan", "outdated"] : [statusBucket];
  const catalogKeywords = [
    ...statusKeywords,
    context.archived === true ? "archived" : context.archived === false ? "active" : "",
    project.pinned ? "pinned favorite favourite" : "",
    project.protectedLevel ? `protected ${project.protectedLevel}` : "",
    project.isCurrent ? "current" : "",
    isDemoProject(project) ? "demo example" : ""
  ];
  const haystack = [
    project.name,
  ...
  const normalizedHaystack = normalizeProjectSearchText(haystack);
...
export function projectSearchKeyAction(key: string, query: string, resultCount: number): ProjectSearchKeyAction {
  if (key === "Enter" && query.trim().length > 0 && resultCount > 0) return "open-first";
