  ...
  return (
    <span className={`help-popover${compact ? " compact" : ""}`}>
      <button
        ...
        type="button"
        aria-label={compact ? `Help: ${title}` : undefined}
        aria-expanded={open}
        aria-controls={panelId}
        data-help={`Open help for ${title}.`}
...
export function previewFileTypeLabel(preview?: PreviewFileIdentity | null) {
  ...
  if (preview.fileKind === "cloudPlaceholder") return "Cloud-only · content not indexed";
  if (preview.fileKind === "symlink") return "Link";
...
export function countUpInitialValue(value: number, startFrom?: number): number {
  return startFrom != null && startFrom < value ? startFrom : value;
}
...
export function CountUp({
  ...
  const initialValue = countUpInitialValue(value, startFrom);
