...
import { documentSearchQueryTerms, prepareDocumentSearchFilters } from "./documentSearch";
...
const fixtureFinalRemoveProjects: FinalRemoveBatchPreview["projects"] = [
  {
    groupId: "fixture-markdown-project",
    projectName: "Fixture Markdown Project",
    originalRoot: "C:\\AI\\Fixture Markdown Project",
    totalObjects: 3,
    ready: 1,
    needsArchiveV2: 1,
    blocked: 1,
  ...
  {
    groupId: "fixture-git-project",
    projectName: "Fixture Git-like Project",
    originalRoot: "D:\\AI\\Fixture Git-like Project",
    totalObjects: 2,
    ready: 1,
    needsArchiveV2: 0,
    blocked: 1,
    blockedSubtrees: [{ root: "shared-model.bin", count: 1, reasonCodes: ["externalHardlink"] }]
  }
];

const fixtureFinalRemoveObjects: FinalRemoveObjectDecision[] = [
  {
    entryId: 101,
    groupId: "fixture-markdown-project",
    topologyGroupId: "fixture-topology-readme",
    relativePath: "README.md",
    kind: "file",
    lifecycle: "held",
    eligibility: "ready",
    reasonCode: "archiveVerified",
    reason: "The object-complete recovery archive and restore round-trip are verified.",
    archiveId: "fixture-archive-101",
    objectArchiveState: "objectCompleteVerified",
    heldVolumeId: "fixture-volume-c",
    heldVolumeLabel: "Windows (C:)",
    logicalBytes: 1_240,
    allocatedBytes: 4_096,
    measurement: "exactStreams"
  },
  {
    entryId: 102,
    groupId: "fixture-markdown-project",
    topologyGroupId: "fixture-topology-context",
    relativePath: "AGENTS.md",
    kind: "file",
    lifecycle: "held",
    eligibility: "needsArchiveV2",
    reasonCode: "legacyContentOnly",
    reason: "The legacy backup proves file content, but not every Windows object stream.",
    remediation: "Windows will ask once to finalize and round-trip-verify the v2 archive for this batch.",
    archiveId: "fixture-archive-102",
    objectArchiveState: "contentOnlyLegacy",
    heldVolumeId: "fixture-volume-c",
    heldVolumeLabel: "Windows (C:)",
    logicalBytes: 5_400,
  ...
  },
  {
    entryId: 103,
    groupId: "fixture-markdown-project",
    topologyGroupId: "fixture-topology-reparse",
    relativePath: "linked-cache",
    kind: "directory",
    lifecycle: "held",
    eligibility: "blocked",
    reasonCode: "unsupportedReparse",
    reason: "This reparse point is not supported by object archive v2.",
    remediation: "Leave this subtree held and review the link target manually.",
    objectArchiveState: "none",
    heldVolumeId: "fixture-volume-c",
    heldVolumeLabel: "Windows (C:)",
    logicalBytes: 0,
    allocatedBytes: null,
  ...
  },
  {
    entryId: 201,
    groupId: "fixture-git-project",
    topologyGroupId: "fixture-topology-build",
    relativePath: "build\\old-output.bin",
    kind: "file",
    lifecycle: "held",
    eligibility: "ready",
    reasonCode: "archiveVerified",
    reason: "The object-complete recovery archive and restore round-trip are verified.",
    archiveId: "fixture-archive-201",
    objectArchiveState: "objectCompleteVerified",
    heldVolumeId: "fixture-volume-d",
    heldVolumeLabel: "Projects (D:)",
    logicalBytes: 12_000,
  ...
  },
  {
    entryId: 202,
    groupId: "fixture-git-project",
    topologyGroupId: "fixture-topology-hardlink",
    relativePath: "models\\shared-model.bin",
    kind: "file",
    lifecycle: "held",
    eligibility: "blocked",
    reasonCode: "externalHardlink",
    reason: "Another hardlink to this file object exists outside the held project.",
    remediation: "The complete hardlink topology must be reviewed as one atomic group.",
    objectArchiveState: "pending",
    heldVolumeId: "fixture-volume-d",
    heldVolumeLabel: "Projects (D:)",
    logicalBytes: 24_000,
    allocatedBytes: 24_576,
    measurement: "exactStreams"
  }
];

// Deterministic fixture-only 256-bit-looking values. This intentionally avoids runtime
// crypto while exercising the production wire grammar: `v2:` + 64 lowercase hex for
// preview digests, and 64 lowercase hex for one-shot capability tokens.
function fixtureCanonicalHex(input: string): string {
  let output = "";
  let state = 0x811c9dc5;
  for (let round = 0; round < 8; round += 1) {
    for (let index = 0; index < input.length; index += 1) {
      state ^= input.charCodeAt(index) + round;
      state = Math.imul(state, 0x01000193);
    }
    state ^= round * 0x9e3779b1;
    output += (state >>> 0).toString(16).padStart(8, "0");
  }
  return output;
}

const fixtureFinalRemovePreviews = new Map<string, FinalRemoveBatchPreview>();

function fixtureFinalRemovePreview(scope: FinalRemoveScope): FinalRemoveBatchPreview {
  const selectedGroupIds = scope.kind === "project"
    ? new Set([scope.groupId])
    : scope.kind === "groups"
      ? new Set(scope.groupIds)
      : new Set(fixtureFinalRemoveProjects.map((project) => project.groupId));
  const projects = fixtureFinalRemoveProjects.filter((project) => selectedGroupIds.has(project.groupId));
  const objects = fixtureFinalRemoveObjects.filter((object) => selectedGroupIds.has(object.groupId));
  const eligibleObjects = objects.filter((object) => (
    object.lifecycle === "held" && (object.eligibility === "ready" || object.eligibility === "needsArchiveV2")
  ));
  const volumeIds = new Set(objects.map((object) => object.heldVolumeId));
  const volumes = Array.from(volumeIds).map((volumeId) => {
    const volumeObjects = objects.filter((object) => object.heldVolumeId === volumeId);
    const eligibleVolumeObjects = eligibleObjects.filter((object) => object.heldVolumeId === volumeId);
    const heldAllocatedBytes = volumeObjects.reduce((total, object) => total + (object.allocatedBytes ?? object.logicalBytes), 0);
    const projectedReleaseBytes = eligibleVolumeObjects.reduce((total, object) => total + (object.allocatedBytes ?? object.logicalBytes), 0);
    const hasEstimate = volumeObjects.some((object) => object.measurement !== "exactStreams");
    return {
      volumeId,
      label: volumeObjects[0]?.heldVolumeLabel ?? volumeId,
      alreadyFreedFromSourceBytes: volumeId === "fixture-volume-c" ? 4_096 : 0,
      heldAllocatedBytes,
      projectedReleaseBytes,
      archiveRetainedAllocatedBytes: volumeId === "fixture-volume-c" ? 6_144 : 8_192,
      freeBytesBefore: null,
      freeBytesAfter: null,
      observedDeltaBytes: null,
      quality: hasEstimate ? "estimated" as const : "exactObjectAllocation" as const
    };
  });
  const groupKey = projects.map((project) => project.groupId).sort().join("+") || "empty";
  const previewHex = fixtureCanonicalHex(`preview:${groupKey}`);
  const preview: FinalRemoveBatchPreview = {
    previewId: `fixture-preview:${previewHex}`,
    previewDigest: `v2:${previewHex}`,
    expiresAt: "2099-12-31T23:59:59Z",
    projects,
    objects,
    volumes,
    eligibleTopologyGroupIds: Array.from(new Set(eligibleObjects.map((object) => object.topologyGroupId))),
    requiresElevation: eligibleObjects.some((object) => object.eligibility === "needsArchiveV2"),
    maxDeleteObjects: eligibleObjects.length,
    blockedObjects: objects.filter((object) => object.eligibility === "blocked").length,
    archivesRetained: true
  };
  fixtureFinalRemovePreviews.set(preview.previewId, preview);
  return preview;
}

const fixtureFinalRemoveConfirmations = new Map<string, {
  previewId: string;
  previewDigest: string;
  selectedTopologyGroupIds: string[];
  selectedEntryIds: number[];
}>();
const fixtureFinalRemoveJobs = new Map<string, FinalRemoveBatchStatus>();
...
export const fixtureApi = {
  ...
  async sessionPreview(path: string, reveal = false, options?: { maxBytes?: number; loadFull?: boolean }): Promise<SessionPreview> {
    ...
    return {
      ...
      text:
        ...
        `Example referenced path: C:/Users/Person/Projects/ExampleProject\n` +
  ...
  async filePreview(nodeId: number, mode: PreviewMode, recordRecent = true, policy?: PreviewPolicy, projectId?: number | null): Promise<FilePreview> {
    const file = files.find((candidate) => candidate.nodeId === nodeId && (projectId == null || candidate.projectId === projectId));
    if (!file) {
  ...
  async fileReveal(nodeId: number, mode: PreviewMode, policy?: PreviewPolicy, projectId?: number | null): Promise<FilePreview> {
    const file = files.find((candidate) => candidate.nodeId === nodeId && (projectId == null || candidate.projectId === projectId));
    if (!file) {
      return blockedPreview(nodeId, 0, "", "Missing file", mode, "The requested node is not in the fixture index.", "missing");
    }
  ...
  async searchDocuments(filters: { query: string; projectId?: number | null; indexedKind?: string; pathFilter?: string; nameFilter?: string; limit?: number; includeFixtureProjects?: boolean }): Promise<DocumentSearchResult> {
    ...
    const needle = boundedFilters.query.trim().toLowerCase();
    const terms = documentSearchQueryTerms(needle);
    const limit = boundedFilters.limit;
    const pathNeedle = boundedFilters.pathFilter?.trim().toLowerCase() ?? "";
    const nameNeedle = boundedFilters.nameFilter?.trim().toLowerCase() ?? "";
    const visibleProjectIds = boundedFilters.includeFixtureProjects === false
      ? new Set(projects.filter((project) => project.source !== "fixture").map((project) => project.id))
      : null;
    if (needle.length < 2 || terms.length === 0) return { hits: [], truncated: false, durationMs: 0 };
    const matches = files
      .filter((file) => {
        const body = file.body?.toLowerCase();
        return !file.isSensitive && !file.protectedLevel && body != null && terms.every((term) => body.includes(term));
      })
      .filter((file) => !visibleProjectIds || visibleProjectIds.has(file.projectId))
      .filter((file) => !boundedFilters.projectId || file.projectId === boundedFilters.projectId)
      .filter((file) => {
        if (boundedFilters.indexedKind === "context") return file.isContext;
        if (boundedFilters.indexedKind === "markdown") return file.isMarkdown;
        return true;
      })
  ...
  async nodeRelationships(projectId: number, nodeId: number): Promise<NodeRelationships> {
    const relationships = buildFixtureRelationships();
    return {
      nodeId,
      projectId,
      outgoing: relationships.filter((relationship) => relationship.sourceNodeId === nodeId && relationship.source.projectId === projectId).map((relationship) => relationship.target),
      incoming: relationships.filter((relationship) => relationship.targetNodeId === nodeId && relationship.target.projectId === projectId).map((relationship) => relationship.source),
      issues: buildFixtureRelationshipIssues().filter((issue) => issue.nodeId === nodeId && issue.projectId === projectId)
    };
  },
  ...
  async projectGraphMap(projectId: number, limit = 300): Promise<GraphMap> {
    ...
    const edges = [
      { sourceNodeId: 90001, sourceProjectId: project.id, targetNodeId: 90002, kind: "workflow_references_model", confidence: "High", evidence: "ckpt_name: sdxl_base.safetensors" }
    ];
  ...
  async projectDiscoveryReport(limit = 100, sessionLimit = limit): Promise<ProjectDiscoveryReport> {
    const candidates = projects.map((project, index) => ({
      ...
      score: index === 0 ? 92 : 54,
      sourceKinds: index === 0 ? ["known_folder", "codex_sessions", "opencode_state", "omp_sessions"] : ["known_folder"],
      signals: [
        {
          kind: "readme",
    ...
    const sessions: ProjectDiscoveryReport["sessions"] = visualAcceptanceState === "saturated"
      ...
      : projects.length > 0
        ? [
          ...
          {
            path: "fixture://opencode/opencode.db#opencode-session=fixture-open-code",
            displayName: "OpenCode fixture conversation",
            sourceKind: "opencode_state",
            sourceLabel: "OpenCode projects and sessions",
          ...
          {
            path: "fixture://omp/sessions/fixture-oh-my-pi.jsonl",
            displayName: "Oh My Pi fixture conversation",
            sourceKind: "omp_sessions",
            sourceLabel: "Oh My Pi sessions",
            sessionKind: "Oh My Pi",
            confidence: "High",
            linkedProjectPaths: [projects[0].path],
            linkedRegisteredProjectIds: [projects[0].id],
            association: "registered_project"
    ...
    return {
      ...
      sessions: sessionLimit === 0 ? sessions : sessions.slice(0, sessionLimit),
      searchedLocations: [
        {
  ...
  async projectDiscoveryDeepScan(rootPath: string, limit = 250, sessionLimit = limit): Promise<ProjectDiscoveryReport> {
    const report = await this.projectDiscoveryReport(limit, sessionLimit);
    return {
      ...report,
      searchedLocations: [
        {
  ...
  async detectInstalledApps(): Promise<InstalledApp[]> {
    ...
    return [
      { id: "opencode", label: "OpenCode", present: true },
      { id: "omp", label: "Oh My Pi", present: true },
  ...
  async nodeOrphanStatus(projectId: number, nodeId: number): Promise<OrphanStatus> {
    const file = files.find((candidate) => candidate.projectId === projectId && candidate.nodeId === nodeId);
    if (!file) {
      return {
        nodeId,
        projectId,
  ...
  async pinItem(nodeId: number, itemKind: string, projectId?: number | null): Promise<void> {
    const file = files.find((candidate) => candidate.nodeId === nodeId && (projectId == null || candidate.projectId === projectId));
    if (itemKind !== "project" && projectId != null && !file) {
      throw new Error("The requested item does not belong to that project.");
    }
    const ownerProjectId = itemKind === "project" ? nodeId : projectId ?? file?.projectId ?? null;
    if (!pinnedItems.some((item) => item.nodeId === nodeId && item.projectId === ownerProjectId && item.itemKind === itemKind)) {
      pinnedItems = [{ nodeId, projectId: ownerProjectId, itemKind, path: file?.path ?? "", pinnedAt: new Date().toISOString() }, ...pinnedItems];
    }
  },

  async unpinItem(nodeId: number, itemKind: string, projectId?: number | null): Promise<void> {
    if (itemKind !== "project" && projectId != null && !files.some((candidate) => candidate.nodeId === nodeId && candidate.projectId === projectId)) {
      throw new Error("The requested item does not belong to that project.");
    }
  ...
  async securityStatus(): Promise<SecurityStatus> {
    return {
      ...
      ...(connectorAcceptanceFixture ? {
        outboundNetwork: "available only through explicit Connector actions in the desktop runtime",
        agentIpc: "Connector acceptance fixture; app registration requires the desktop runtime"
      } : {})
    };
  },
  ...
  async mutationFinalRemovePreview(scope: FinalRemoveScope): Promise<FinalRemoveBatchPreview> {
    return fixtureFinalRemovePreview(scope);
  },

  async mutationFinalRemoveConfirm(
    previewId: string,
    previewDigest: string,
    selectedTopologyGroupIds: string[]
  ): Promise<FinalRemoveConfirmationToken> {
    const preview = fixtureFinalRemovePreviews.get(previewId);
    if (!preview
      || !/^fixture-preview:[0-9a-f]{64}$/u.test(previewId)
      || !/^v2:[0-9a-f]{64}$/u.test(previewDigest)
      || preview.previewDigest !== previewDigest) {
      throw new Error("The fixture final-cleanup preview is no longer valid.");
    }
    const uniqueTopologyGroupIds = Array.from(new Set(selectedTopologyGroupIds));
    const eligibleIds = new Set(preview.eligibleTopologyGroupIds);
    const selectedIds = new Set(uniqueTopologyGroupIds);
    ...
    const selectionIsAtomic = uniqueTopologyGroupIds.every((id) => {
      const members = heldMembersByTopology.get(id) ?? [];
      return eligibleIds.has(id)
        && members.length > 0
        && members.every((object) => object.eligibility === "ready" || object.eligibility === "needsArchiveV2");
    });
    const selectedObjects = preview.objects.filter((object) => (
      ...
      && selectedIds.has(object.topologyGroupId)
    ));
    if (uniqueTopologyGroupIds.length === 0
      || uniqueTopologyGroupIds.length !== selectedTopologyGroupIds.length
      || !selectionIsAtomic
      || selectedObjects.length > preview.maxDeleteObjects) {
      throw new Error("The fixture selection includes no eligible topology groups.");
    }
    const token = fixtureCanonicalHex(`token:${previewId}:${previewDigest}:${selectedTopologyGroupIds.join("+")}`);
    fixtureFinalRemoveConfirmations.set(token, {
      previewId,
      previewDigest,
      selectedTopologyGroupIds: [...selectedTopologyGroupIds],
      selectedEntryIds: selectedObjects.map((object) => object.entryId)
    });
    return { token, previewId, previewDigest, expiresAt: "2099-12-31T23:59:59Z" };
  },

  async mutationFinalRemoveBatchStart(request: FinalRemoveBatchStart): Promise<FinalRemoveBatchStartSummary> {
    const confirmation = fixtureFinalRemoveConfirmations.get(request.confirmationToken);
    if (!confirmation
      || confirmation.previewId !== request.previewId
      || confirmation.previewDigest !== request.previewDigest
      || confirmation.selectedTopologyGroupIds.join("\0") !== request.selectedTopologyGroupIds.join("\0")) {
      throw new Error("The fixture final-cleanup confirmation is missing, expired or bound to another preview.");
    }
    fixtureFinalRemoveConfirmations.delete(request.confirmationToken);
    const selectedEntryIds = new Set(confirmation.selectedEntryIds);
    const selectedObjects = fixtureFinalRemoveObjects.filter((object) => selectedEntryIds.has(object.entryId));
    const items = selectedObjects.map((object) => object.topologyGroupId === "fixture-topology-build"
      ? {
          entryId: object.entryId,
          state: "deleteFailed" as const,
          reasonCode: "interrupted" as const,
          error: "Synthetic fixture failure after the archive remained verified."
        }
      : { entryId: object.entryId, state: "deleted" as const });
    const deletedEntryIds = new Set(items.filter((item) => item.state === "deleted").map((item) => item.entryId));
    const failedEntryIds = new Set(items.filter((item) => item.state === "deleteFailed").map((item) => item.entryId));
    const selectedProjectIds = Array.from(new Set(selectedObjects.map((object) => object.groupId)));
    const projects = selectedProjectIds.map((groupId) => ({
      groupId,
      deleted: selectedObjects.filter((object) => object.groupId === groupId && deletedEntryIds.has(object.entryId)).length,
      kept: 0,
      failed: selectedObjects.filter((object) => object.groupId === groupId && failedEntryIds.has(object.entryId)).length
    }));
    const volumeIds = Array.from(new Set(selectedObjects.map((object) => object.heldVolumeId)));
    const volumes = volumeIds.map((volumeId) => {
      const volumeObjects = selectedObjects.filter((object) => object.heldVolumeId === volumeId);
      const released = volumeObjects
        .filter((object) => deletedEntryIds.has(object.entryId))
        .reduce((total, object) => total + (object.allocatedBytes ?? object.logicalBytes), 0);
      return {
        volumeId,
        label: volumeObjects[0]?.heldVolumeLabel ?? volumeId,
        alreadyFreedFromSourceBytes: volumeId === "fixture-volume-c" ? 4_096 : 0,
        heldAllocatedBytes: volumeObjects.reduce((total, object) => total + (object.allocatedBytes ?? object.logicalBytes), 0),
        projectedReleaseBytes: released,
        archiveRetainedAllocatedBytes: volumeId === "fixture-volume-c" ? 6_144 : 8_192,
        freeBytesBefore: 1_000_000,
        freeBytesAfter: 1_000_000 + released,
        observedDeltaBytes: released,
        quality: "observedNoisy" as const
      };
    });
    const batchId = `fixture-batch-${fixtureFinalRemoveJobs.size + 1}`;
    const jobId = `fixture-final-remove-job-${fixtureFinalRemoveJobs.size + 1}`;
    const failedObjects = failedEntryIds.size;
    fixtureFinalRemoveJobs.set(jobId, {
      progress: { batchId, phase: "finished", total: selectedObjects.length, completed: selectedObjects.length, currentPath: null },
      result: {
        batchId,
        status: failedObjects > 0 ? "partial" : "completed",
        requestedObjects: selectedObjects.length,
        deletedObjects: deletedEntryIds.size,
        keptObjects: 0,
        failedObjects,
        projects,
        volumes,
  ...
  async mutationFinalRemoveBatchStatus(jobId: string): Promise<FinalRemoveBatchStatus> {
    const status = fixtureFinalRemoveJobs.get(jobId);
    if (!status) throw new Error("The fixture final-cleanup job was not found.");
    return status;
  },

  async mutationFinalRemoveBatchStop(jobId: string): Promise<void> {
    const status = fixtureFinalRemoveJobs.get(jobId);
    if (!status || status.result) return;
    fixtureFinalRemoveJobs.set(jobId, {
      progress: { ...status.progress, phase: "interrupted" },
      result: {
        batchId: status.progress.batchId,
        status: "interrupted",
        requestedObjects: status.progress.total,
        deletedObjects: status.progress.completed,
        keptObjects: Math.max(0, status.progress.total - status.progress.completed),
        failedObjects: 0,
        projects: [],
        volumes: [],
        items: [],
  ...
  async mutationRecoveryDashboard(): Promise<MutationRecoveryDashboard> {
    const activeJob = Array.from(fixtureFinalRemoveJobs.entries()).find(([, status]) => !status.result);
    if (activeJob) {
      const [jobId, status] = activeJob;
      return {
        available: true,
        finalRemove: {
          state: "active",
          ...
          message: "A synthetic final-cleanup batch is active."
        },
        message: "Synthetic mutation recovery state is available."
      };
    }
    return {
      available: true,
      finalRemove: {
        ...
        jobId: null,
        phase: null,
        message: "No synthetic final-cleanup batch is active or interrupted."
      },
      message: "Synthetic mutation recovery state is available."
    };
  },
...
function fixtureRiskReport(plan: OperationPlan): RiskReport {
  ...
  return {
    ...
    relationshipEvidenceComplete: plan.relationshipEvidenceComplete,
