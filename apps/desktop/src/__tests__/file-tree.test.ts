...
import { buildTreeFilterKeep, fileTreeEmptyCopy, openTreeFile, visibleTreeItems } from "../views/project-center/FileTree";
import type { TreePage, TreeSort } from "../views/project-center/types";

const fileTreeSource = readFileSync(new URL("../views/project-center/FileTree.tsx", import.meta.url), "utf8");
...
describe("file tree item filtering and sorting", () => {
  it("keeps rarely changed visibility controls in one compact disclosure", () => {
    expect(fileTreeSource).toContain('<details className="tree-view-options">');
    expect(fileTreeSource).toContain("Folders");
    expect(fileTreeSource).toContain("Files");
  });

  it("opens a file with its project membership, not its node id alone", () => {
    const opened: Array<[number, number]> = [];
    const file = navItem({ nodeId: 73, projectId: 20, itemKind: "file" });

    expect(openTreeFile(file, (nodeId, projectId) => opened.push([nodeId, projectId]))).toBe(true);
    expect(opened).toEqual([[73, 20]]);
    expect(openTreeFile(navItem({ nodeId: 73, projectId: 10, itemKind: "directory" }), () => undefined)).toBe(false);
  });
