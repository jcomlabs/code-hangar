...
import { documentSearchCriteriaKey, duplicateSearchCriteriaKey, orphanSearchCriteriaKey, resolveProjectPickerInput, retainRunningDuplicateConfirmations, scopeForDiscoveryEntry, scopeForDocumentSearchEntry } from "../documentSearch";
import { canRunCurrentProjectScope, canRunDocumentSearch, canSubmitDocumentSearch, currentProjectScopeLabel, defaultSessionGroupCollapsed, discoverSessionFacts, discoverSessionHelp, DiscoverDuplicatesView, DiscoverSearchView, documentAdvancedFilterCount, documentHitPathLabel, documentHitSnippet, documentSafeManageProject, duplicateConfirmationGroupKey, duplicateGroupCountLabel, groupDuplicateGroupsForDisplay, groupLostCandidatesForDisplay, hiddenDiscoveryCandidateCount, orphanCandidateFacts, orphanCandidatePathLabel, orphanResultCountLabel, projectCandidateSignalChips, searchMinPresetForMode, shouldShowLostPresetPicker, type DuplicateConfirmStateMap } from "../views/DiscoverView";
...
describe("Discover duplicate groups", () => {
  it("keeps confirmation identity separate when projects share a physical node id", () => {
    const outer = duplicateGroup(9, ["shared/model.bin", "shared/copy.bin"]);
    const nested = {
      ...outer,
      members: outer.members.map((member) => ({ ...member, projectId: 2 }))
    };

    expect(duplicateConfirmationGroupKey(outer)).not.toBe(duplicateConfirmationGroupKey(nested));
    expect(duplicateConfirmationGroupKey(outer)).toContain("1:90");
    expect(duplicateConfirmationGroupKey(nested)).toContain("2:90");
  });
...
describe("Discover document search hits", () => {
  const renderDocumentSearch = ({
    ...
    searchRan?: boolean;
  } = {}) => renderToStaticMarkup(createElement(DiscoverSearchView, {
    documentQuery: "README",
    setDocumentQuery: () => undefined,
    documentScope: "all",
    setDocumentScope: () => undefined,
    documentProjectInput: "",
    setDocumentProjectInput: () => undefined,
    documentProjectResolution: { kind: "empty", project: null, matches: [] },
    documentKind: kind,
    setDocumentKind: () => undefined,
    documentPathFilter: pathFilter,
    setDocumentPathFilter: () => undefined,
    documentNameFilter: nameFilter,
    setDocumentNameFilter: () => undefined,
    documentLimit: limit,
    setDocumentLimit: () => undefined,
    documentSearching: false,
    runDocumentSearch: () => undefined,
    documentSearchRan: searchRan,
    documentHits: [],
    documentSearchTruncated: false,
    documentSearchDuration: null,
    documentSearchError: null,
    projects: [],
    openDocumentHit: () => undefined,
    showFileMenu: () => undefined,
    selectedProjectId: null,
    showReview: () => undefined
  }));

  it("keeps the primary document search path visible and advanced filters collapsed by default", () => {
    const markup = renderDocumentSearch();

    expect(markup).toContain('aria-label="Content keywords"');
    expect(markup).toContain("Where to search");
    expect(markup).toContain("Type");
    expect(markup).toContain('type="submit"');
    expect(markup).toContain("More filters");
    expect(markup).toMatch(/<details class="advanced-filter-details"><summary/);
  });

  it("opens More filters automatically and summarizes every non-default filter", () => {
    const markup = renderDocumentSearch({ pathFilter: "docs", nameFilter: "AGENTS", limit: 50 });

    expect(documentAdvancedFilterCount(" docs ", " AGENTS ", 50)).toBe(3);
    expect(markup).toMatch(/<details class="advanced-filter-details" open=""><summary/);
    expect(markup).toContain('aria-label="3 advanced filters active"');
    expect(markup).toContain("3 active");
  });

  it("preserves advanced path, name and limit values inside the disclosure", () => {
    const markup = renderDocumentSearch({ pathFilter: "docs/prompts", nameFilter: "README", limit: 100 });

    expect(markup).toContain('value="docs/prompts"');
    expect(markup).toContain('value="README"');
    expect(markup).toContain('<option value="100" selected="">100</option>');
    expect(documentAdvancedFilterCount("   ", "", 10)).toBe(0);
  });

  it("turns a zero-result narrow search into direct widening actions", () => {
    const markup = renderDocumentSearch({ pathFilter: "docs", kind: "context", searchRan: true });

    expect(markup).toContain("No indexed documents match this search");
    expect(markup).toContain("Search all indexed file types");
    expect(markup).toContain("Clear extra filters and search again");
  });
  ...
  it("does not allow Current project search without a selected project", () => {
    ...
    expect(canRunDocumentSearch("current", 7, [{ id: 7 }])).toBe(true);
    expect(canRunDocumentSearch("current", 7, [{ id: 1 }])).toBe(false);
    expect(canRunDocumentSearch("current", 7, [])).toBe(false);
  ...
  it("blocks submission while a typed project name is ambiguous", () => {
    const resolution = resolveProjectPickerInput([
      { id: 1, name: "MeaningOfLife", path: "C:\\Work\\MeaningOfLife" },
      { id: 2, name: "MeaningOfLife Tools", path: "C:\\Work\\MeaningOfLife-Tools" }
    ], "meaning");

    expect(resolution.kind).toBe("ambiguous");
    expect(canSubmitDocumentSearch("README", false, resolution.kind !== "resolved")).toBe(false);
  });
  ...
  it("offers Safe Manage only for the active project that the search actually targets", () => {
    const projects = [{ id: 1, name: "Alpha" }, { id: 2, name: "Beta" }];

    expect(documentSafeManageProject("current", 1, null, projects)?.id).toBe(1);
    expect(documentSafeManageProject("project", 1, 1, projects)?.id).toBe(1);
    expect(documentSafeManageProject("project", 1, 2, projects)).toBeNull();
    expect(documentSafeManageProject("all", 1, null, projects)).toBeNull();
    expect(documentSafeManageProject("current", 9, null, projects)).toBeNull();
  });
});

describe("Discover search criteria", () => {
  it("hides the saved-preset chooser until at least one preset exists", () => {
    expect(shouldShowLostPresetPicker([])).toBe(false);
    expect(shouldShowLostPresetPicker([{ name: "Old drafts" }])).toBe(true);
  });
  ...
  it("tracks the chosen project scope without making All projects stale", () => {
    const base = {
      query: "README",
    ...
    expect(documentSearchCriteriaKey(base)).not.toBe(documentSearchCriteriaKey({ ...base, projectId: 7 }));
    expect(documentSearchCriteriaKey({ ...base, scope: "all" })).toBe(
      documentSearchCriteriaKey({ ...base, scope: "all", projectId: 7 })
    );
  });
