...
describe("sidebar project filtering", () => {
  ...
  it("matches project search against names, folder paths, aliases and app labels", () => {
    ...
    expect(projectMatchesSidebarQuery(antigravityProject, "sample 3d chatgpt")).toBe(true);
    expect(projectMatchesSidebarQuery({ ...antigravityProject, name: "Aplicação Médica" }, "aplicacao medica")).toBe(true);
    expect(projectMatchesSidebarQuery({ ...antigravityProject, source: "scan" }, "scan")).toBe(false);
    expect(projectMatchesSidebarQuery(antigravityProject, "README contents")).toBe(false);
    expect(projectMatchesSidebarQuery(antigravityProject, "missing")).toBe(false);
  });

  it("matches useful catalog keywords without pretending to search file contents", () => {
    const trackedProject: ProjectSummary = {
      ...realProject,
      id: 4,
      name: "TrackedProject",
      pinned: true,
      protectedLevel: "sensitive",
      isCurrent: true,
      scanState: "outdated"
    };

    expect(projectMatchesSidebarQuery(trackedProject, "pinned protected current needs scan")).toBe(true);
    expect(projectMatchesSidebarQuery(trackedProject, "favorite outdated")).toBe(true);
    expect(projectMatchesSidebarQuery(trackedProject, "archived", { archived: false })).toBe(false);
  });
  ...
  it("combines search, app filter, status filter and archived grouping", () => {
    ...
    const archivedByKeyword = orderSidebarProjects([beta, gamma, alpha], {
      sort: "name",
      appFilter: "all",
      statusFilter: "all",
      query: "archived needs scan",
      archivedProjectIds: new Set([beta.id]),
      getStatusBucket: (project) => projectStatusBucketFromScanState(project.scanState)
    });
    expect(archivedByKeyword.all.map((project) => project.name)).toEqual(["Beta"]);
  });
  ...
  it("maps project search keyboard shortcuts to list actions", () => {
    ...
    expect(projectSearchKeyAction("Enter", "", 1)).toBe("none");
