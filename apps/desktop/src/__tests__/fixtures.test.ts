...
describe("fixture navigation", () => {
  it("shows one project and scan inventory carrying every fixture provider label", async () => {
    ...
    expect(linked.map((session) => session.sessionKind).sort()).toEqual(["ChatGPT", "Claude", "Oh My Pi", "OpenCode"]);
    expect(linked.map((session) => session.sourceKind).sort()).toEqual(["claude_code_sessions", "codex_sessions", "omp_sessions", "opencode_state"]);
    expect(report.candidates[0].sourceKinds).toEqual(expect.arrayContaining(["opencode_state", "omp_sessions"]));
  });

  it("limits project candidates and sessions independently", async () => {
    const report = await fixtureApi.projectDiscoveryReport(1, 0);
    expect(report.candidates).toHaveLength(1);
    expect(report.sessions.map((session) => session.sessionKind)).toEqual(expect.arrayContaining(["OpenCode", "Oh My Pi"]));
  });
  ...
  it("exposes local markdown relationships", async () => {
    ...
    const readmeRelationships = await fixtureApi.nodeRelationships(readme!.projectId, readme!.nodeId);
    const overviewRelationships = await fixtureApi.nodeRelationships(overview!.projectId, overview!.nodeId);

    expect(readmeRelationships.projectId).toBe(readme!.projectId);
  ...
  it("supports explicit orphan search modes and filters", async () => {
    ...
    const status = await fixtureApi.nodeOrphanStatus(unused!.projectId, unused!.nodeId);
    expect(status.projectId).toBe(unused!.projectId);
  ...
  it("filters document search explicitly", async () => {
    ...
    const reorderedTerms = await fixtureApi.searchDocuments({ query: "hangar code", indexedKind: "context", includeFixtureProjects: true });
    const missingRequiredTerm = await fixtureApi.searchDocuments({ query: "code definitelyabsent", indexedKind: "context", includeFixtureProjects: true });
    const legacyZeroLimit = await fixtureApi.searchDocuments({ query: "code hangar", indexedKind: "all", limit: 0 });
    ...
    expect(reorderedTerms.hits.length).toBeGreaterThan(0);
    expect(missingRequiredTerm.hits).toEqual([]);
    expect(legacyZeroLimit.truncated).toBe(false);
  ...
  it("pins and unpins one file membership without affecting another owner", async () => {
    const outer = { nodeId: 101, projectId: 1 };
    const nested = { nodeId: 201, projectId: 2 };
    await fixtureApi.unpinItem(outer.nodeId, "file", outer.projectId);
    await fixtureApi.unpinItem(nested.nodeId, "file", nested.projectId);
    try {
      await fixtureApi.pinItem(outer.nodeId, "file", outer.projectId);
      await fixtureApi.pinItem(nested.nodeId, "file", nested.projectId);
      expect((await fixtureApi.pinnedItemsList()).filter((item) => item.itemKind === "file")).toMatchObject([
        { ...nested, itemKind: "file" },
        { ...outer, itemKind: "file" }
      ]);

      await fixtureApi.unpinItem(outer.nodeId, "file", outer.projectId);
      expect((await fixtureApi.pinnedItemsList()).filter((item) => item.itemKind === "file")).toEqual([
        expect.objectContaining({ ...nested, itemKind: "file" })
      ]);
    } finally {
      await fixtureApi.unpinItem(outer.nodeId, "file", outer.projectId);
      await fixtureApi.unpinItem(nested.nodeId, "file", nested.projectId);
