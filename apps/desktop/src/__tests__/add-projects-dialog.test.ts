...
describe("isWslInstalledApp", () => {
  ...
  it("leaves the real host-app ids alone", () => {
    for (const id of ["opencode", "omp", "claude", "codex", "cursor", "antigravity", "gemini", "windsurf", "openclaw", "hermes", "pinokio"]) {
...
describe("partitionInstalledApps", () => {
  ...
  it("excludes every wsl* id from the host-app chips (present hosts only)", () => {
    ...
    expect(hostApps.map((app) => app.id)).toEqual(["opencode", "omp", "claude", "codex"]);
  ...
  it("builds honest progress sources without inventing a WSL app", () => {
    expect(deepScanSourceLabels(apps, false)).toEqual(["OpenCode", "Oh My Pi", "Claude Code", "ChatGPT"]);
