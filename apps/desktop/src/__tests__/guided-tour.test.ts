...
describe("edition-specific guided tours", () => {
  ...
  it("adds one clear local app-integration step only to Code Hangar AI Connector", () => {
    ...
    expect(text).toContain("Claude, Codex or Cursor");
    expect(text).toContain("local MCP process");
    expect(text).toContain("explicit project and action scopes");
    expect(text).toContain("comment writes stay off by default");
    expect(text).toContain("Disconnect removes only Code Hangar's registration");
    for (const forbidden of ["local model", "your own API", "provider", "endpoint", "API key", "HTTP"]) {
