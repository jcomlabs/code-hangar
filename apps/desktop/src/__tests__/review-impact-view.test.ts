...
const reviewImpactSource = readFileSync(new URL("../views/ReviewImpactView.tsx", import.meta.url), "utf8");

describe("Safe Manage move-to-holding truth", () => {
  it("does not present holding as final cleanup or promise generic recovered space", () => {
    expect(reviewImpactSource).toContain("supported original item");
    expect(reviewImpactSource).toContain("Held allocation remains until Finish removing");
    expect(reviewImpactSource).toContain("blocked links and objects remain at their original paths");
    expect(reviewImpactSource).toContain("verified backup/archive is retained separately");
    expect(reviewImpactSource).not.toContain("recovered safely");
    expect(reviewImpactSource).not.toContain("Removing a folder empties it fully");
  });
});
