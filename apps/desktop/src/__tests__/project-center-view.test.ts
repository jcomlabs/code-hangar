...
import {
  ...
  projectLoadOverlayStatus,
  projectSelectionRequiredCopy,
...
const projectCenterSource = readFileSync(new URL("../views/ProjectCenterView.tsx", import.meta.url), "utf8");
...
describe("project workspace navigation", () => {
  it("contains only in-workspace tabs and leaves Safe Manage in its scoped entry points", () => {
    const start = projectCenterSource.indexOf('<nav className="project-workspace-nav"');
    const end = projectCenterSource.indexOf("</nav>", start);
    const navigation = projectCenterSource.slice(start, end);

    expect(start).toBeGreaterThanOrEqual(0);
    expect(navigation).not.toContain("showReview");
    expect(navigation).not.toContain("Safe manage");
  });

  it("gives every project-only tab an explicit no-project boundary", () => {
    expect(projectSelectionRequiredCopy("space")).toContain("Choose a project");
    expect(projectSelectionRequiredCopy("connections")).toContain("connection map");
    expect(projectSelectionRequiredCopy("sessions")).toContain("linked local sessions");
    expect(projectSelectionRequiredCopy("space")).not.toContain("sidebar");
  });

  it("separates mutually exclusive file views from the change-lock action", () => {
    expect(projectCenterSource).toContain('className="viewer-mode-actions" role="group"');
    expect(projectCenterSource).toContain('className="toolbar-button viewer-lock-action changes-locked"');
    expect(projectCenterSource).toContain('className="toolbar-button viewer-lock-action changes-unlocked"');
  });

  it("offers narrow layouts a direct accessible escape when the project sidebar is hidden", () => {
    const html = renderToStaticMarkup(createElement(NarrowProjectEscape, {
      onOverview: vi.fn(),
      onAddProjects: vi.fn()
    }));

    expect(html).toContain('aria-label="Project navigation options"');
    expect(html).toContain("No project selected");
    expect(html).toContain("Overview");
    expect(html).toContain("Add Projects");
    expect(projectCenterSource).toContain("!selectedProject ? (");
    expect(projectCenterSource).toContain("Choose or add a project before browsing its files.");
  });
});

describe("project sessions empty state", () => {
  it("offers the local session refresh directly instead of prescribing manual navigation", () => {
    const html = renderToStaticMarkup(createElement(ProjectSessionsHome, {
      projectId: 42,
      sessions: [],
      onOpenSession: vi.fn(),
      onContextMenu: vi.fn(),
      onFindSessions: vi.fn()
    }));

    expect(html).toContain("Find local sessions");
    expect(html).toContain("read-only local conversation map");
    expect(html).not.toContain("Use Discover, then");
  });
});
...
describe("direct file-open latency boundary", () => {
  it("keeps an Explorer Markdown preview visible while its project tree loads", () => {
    expect(projectLoadOverlayStatus("loading", "files", true)).toBeNull();
    expect(projectLoadOverlayStatus("error", "files", true)).toBeNull();
    expect(projectLoadOverlayStatus("loading", "files", false)).toBe("loading");
    expect(projectLoadOverlayStatus("loading", "context", true)).toBe("loading");
  });
});
