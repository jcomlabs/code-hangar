...
describe("UI labels", () => {
  it("uses file-name context for human preview type labels", () => {
    ...
    expect(previewFileTypeLabel({
      displayName: "REMOTE.md",
      displayPath: "REMOTE.md",
      path: "C:\\OneDrive\\project\\REMOTE.md",
      fileKind: "cloudPlaceholder"
    })).toBe("Cloud-only · content not indexed");
  });
...
describe("count-up animation math", () => {
  it("uses an explicit lower mount origin without turning decreases into animations", () => {
    expect(countUpInitialValue(240, 0)).toBe(0);
    expect(countUpInitialValue(10, 20)).toBe(10);
    expect(countUpInitialValue(10)).toBe(10);
  });
