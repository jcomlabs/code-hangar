...
describe("workspace route status text", () => {
  it("keeps the status bar aligned with global navigation", () => {
    ...
    expect(workspaceRouteStatusText({ ...INITIAL_WORKSPACE_ROUTE, primaryView: "safe_manage" })).toBe("Safe Manage portfolio opened.");
  ...
  it("round-trips the global Safe Manage destination without requiring a project", () => {
    expect(parseWorkspaceRoute({
      ...INITIAL_WORKSPACE_ROUTE,
      primaryView: "safe_manage",
      rightPaneView: "plan",
      selectedProjectId: null
    })).toMatchObject({
      primaryView: "safe_manage",
...
describe("transient workspace history", () => {
  it("uses a different cache identity for each project membership of one node", () => {
    expect(fileMembershipKey(10, 73)).toBe("10:73");
    expect(fileMembershipKey(10, 73)).not.toBe(fileMembershipKey(20, 73));
  });
  ...
  it("keeps the owning project when overlapping projects share one physical node", () => {
    const outerProjectFile = { nodeId: 73, projectId: 10 };
    const nestedProjectFile = { nodeId: 73, projectId: 20 };

    const backStack = appendFileHistoryEntry([], outerProjectFile, nestedProjectFile);

    expect(backStack).toEqual([outerProjectFile]);
    expect(appendFileHistoryEntry(backStack, nestedProjectFile, outerProjectFile)).toEqual([
      outerProjectFile,
      nestedProjectFile
    ]);
  });

  it("does not record transient or identical file targets", () => {
    const indexed = { nodeId: 73, projectId: 10 };

    expect(appendFileHistoryEntry([], indexed, indexed)).toEqual([]);
    expect(appendFileHistoryEntry([], { nodeId: -1, projectId: 10 }, indexed)).toEqual([]);
  });
});
