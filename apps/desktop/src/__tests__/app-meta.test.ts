...
import { appMeta, displayAppText, projectAppMeta, projectAppMetas, sessionAppMeta, sourceAppMetas } from "../app-meta";
...
describe("appMeta", () => {
  it("maps known apps to canonical slug + label regardless of casing/variants", () => {
    ...
    expect(appMeta("opencode_state")).toEqual({ slug: "opencode", label: "OpenCode" });
    expect(appMeta("Oh My Pi")).toEqual({ slug: "omp", label: "Oh My Pi" });
    expect(appMeta("omp_sessions")).toEqual({ slug: "omp", label: "Oh My Pi" });
  ...
  it("falls back to Other for unknown or empty values", () => {
    ...
    expect(appMeta("composer")).toEqual({ slug: "other", label: "Other" });
    expect(appMeta("computer_session")).toEqual({ slug: "other", label: "Other" });
  });

  it("derives compact provider labels from scan source kinds without duplicates", () => {
    expect(sourceAppMetas(["opencode_state", "omp_sessions", "opencode_state", "known_folder"])).toEqual([
      { slug: "opencode", label: "OpenCode" },
      { slug: "omp", label: "Oh My Pi" }
    ]);
  });
