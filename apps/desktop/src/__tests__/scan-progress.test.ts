...
import { scanProgressParts, shouldCelebrateStandaloneScan } from "../scanProgress";
...
describe("scan progress copy", () => {
  it("leaves the Deep Scan completion moment to its own progress panel", () => {
    expect(shouldCelebrateStandaloneScan("job-1", "job-1")).toBe(false);
    expect(shouldCelebrateStandaloneScan("job-1", "job-2")).toBe(true);
    expect(shouldCelebrateStandaloneScan("job-1", null)).toBe(true);
  });
