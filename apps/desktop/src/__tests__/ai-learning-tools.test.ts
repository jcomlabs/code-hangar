...
describe("optional guided learning contract", () => {
  ...
  it("shows a cost preview before both provider sends", () => {
    ...
    expect(ui).toContain("aiWalkthroughDisclosure");
    expect(ui).toContain("aiFollowUpDisclosure");
    expect(ui).toContain("confirmExactAiSend(disclosure)");
    expect(ui).toContain("disclosure.previewId");
  ...
  it("keeps walkthrough and follow-up provider paths read-only", () => {
    for (const [start, end] of [
      ["pub(crate) fn ai_prepare_walkthrough_for_path", "fn follow_up_context"],
      ["pub(crate) fn ai_prepare_follow_up_for_path", "/// Full, non-truncated bytes"]
    ]) {
      const body = backend.split(start)[1]?.split(end)[0] ?? "";
      expect(body).toContain("hangar_ai::prepare_request");
      for (const forbidden of ["hangar_ai::send_prepared", "write_file_content", "apply_value_edit", "hangar_mutation", "Command::new"]) {
