...
import { undatedSessionFingerprint } from "../reviewCheckpoint";
...
describe("overview recommended flow", () => {
  ...
  it("builds a cross-project inbox from the saved review cutoffs", () => {
    ...
    expect(reviewInboxTitle("fresh", false, false, null, 1, 1)).toBe("1 new session record needs review · 1 undated");
    expect(reviewInboxTitle("cached", false, false, null, 1, 1)).toBe("1 new session record needs review · 1 undated · saved index");
    expect(reviewInboxTitle("cached", false, false, null, 0, 0)).toBe("Saved session index needs a fresh local check");
    expect(reviewInboxTitle("unavailable", false, false, null, 0, 0)).toBe("Local session index is temporarily unavailable");
    expect(reviewInboxTitle("fresh", true, false, null, 1, 1)).toBe("Refreshing the local session index");
    expect(reviewInboxTitle("cached", true, false, null, 1, 1)).toBe("Refreshing the saved local session index");

    const acknowledged = {
      ...checkpoint,
      sessionCutoffMs: 300,
      undatedSessionFingerprint: undatedSessionFingerprint(sessions)
    };
    expect(buildReviewInboxItems([{ project, sessions }], [acknowledged])).toEqual([]);

    const withNewUndated = [...sessions, {
      ...sessions[2],
      path: "fixture://session-new-undated.jsonl",
      displayName: "New undated session"
    }];
    expect(buildReviewInboxItems([{ project, sessions: withNewUndated }], [acknowledged])).toMatchObject([{
      unreviewedCount: 0,
      unknownTimestampCount: 2
    }]);
  });
});
...
describe("overview information density", () => {
  it("keeps technical Git and detector details collapsed on first render", () => {
    const html = renderToStaticMarkup(createElement(OverviewView, {
      ...
      reviewProjectGroups: [],
      reviewInventoryState: "fresh",
      reviewInventoryRefreshing: false,
      reviewInventoryError: null,
      onRefreshReviewInventory: () => undefined,
...
describe("overview inventory health", () => {
  it("summarizes stale roots as the primary overview state", () => {
    ...
    expect(health.facts).toEqual([
      { label: "Healthy roots", value: 34 },
      { label: "Stale roots", value: 6 },
  ...
  it("surfaces partial scans when roots are otherwise fresh", () => {
    ...
    expect(health.facts.map((fact) => fact.label)).toEqual(["Scan roots", "Stale roots", "Partial entries"]);
  });
