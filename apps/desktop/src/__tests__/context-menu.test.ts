...
describe("context menu file capabilities", () => {
  it("keeps source actions for readable code and context files", () => {
  ...
  it("shows the menu title and each consecutive section only once", () => {
    const html = renderToStaticMarkup(createElement(ContextMenu, {
      menu: {
        x: 10,
        y: 10,
        label: "Tools for selected text",
        items: [
          { id: "inspect", label: "Inspect", section: "Selection", onSelect: () => undefined },
          { id: "review", label: "Check risks", section: "Selection", onSelect: () => undefined },
    ...
    expect(html).toContain("Tools for selected text");
    expect(html.match(/Selection/g)).toHaveLength(1);
