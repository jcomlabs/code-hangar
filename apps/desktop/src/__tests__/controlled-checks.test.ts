...
describe("controlled correction checks contract", () => {
  ...
  it("enforces the Windows resource boundary and bounded output in Rust", () => {
    for (const guard of [
      ...
      "CONTROLLED_CHECK_FIXED_ENV.iter().copied()",
      "OUTPUT_LIMIT_BYTES",
      "redact_secrets",
    ]) {
      expect(backend).toContain(guard);
    }
    expect(backend).toContain("project code is not isolated from arbitrary sockets");
    expect(backend).toContain("project code is not network-sandboxed");
  });
