...
describe("local-first AI UX contract", () => {
  ...
  it("keeps Save network-free, Test single-send and model listing explicit and reviewed", () => {
    const explainPanel = ui.slice(
      ui.indexOf("export const AiExplainPanel"),
      ui.indexOf("export const AiAssistKeyCard")
    );
    const providerCard = ui.slice(ui.indexOf("export const AiAssistKeyCard"));
    const providerRefresh = providerCard.slice(
      providerCard.indexOf("  const refreshModels = async (cfg: AiProviderConfig) => {"),
      providerCard.indexOf("  const save = async () => {")
    );
    const saveAction = providerCard.slice(
      providerCard.indexOf("  const save = async () => {"),
      providerCard.indexOf("  const test = async () => {")
    );
    const testAction = providerCard.slice(
      providerCard.indexOf("  const test = async () => {"),
      providerCard.indexOf("  const saveKey = async () => {")
    );

    expect(saveAction.match(/api\.[A-Za-z0-9_]+\(/g)).toEqual(["api.aiProviderSet("]);
    expect(testAction.match(/api\.aiProviderTest\(/g)).toHaveLength(1);
    expect(testAction.match(/api\.aiProviderTestDisclosure\(/g)).toHaveLength(1);
    expect(testAction).toContain("confirmExactAiSend(disclosure)");
    expect(testAction).toContain("api.aiProviderTest(disclosure.previewId)");
    expect(saveAction).not.toContain("refreshModels");
    expect(testAction).not.toContain("refreshModels");
    expect(testAction).not.toContain("aiProviderModels");

    for (const refresh of [
      explainPanel.slice(explainPanel.indexOf("  const refreshModels = async () => {")),
      providerRefresh
    ]) {
      expect(refresh.match(/api\.aiProviderModelsDisclosure\(/g)).toHaveLength(1);
      expect(refresh.match(/api\.aiProviderModels\(/g)).toHaveLength(1);
      expect(refresh).toContain("confirmExactAiSend(disclosure)");
      expect(refresh).toContain("api.aiProviderModels(disclosure.previewId)");
      expect(refresh).toContain("err instanceof Error ? err.message : String(err)");
      expect(refresh).not.toContain("catch {");
    }
    expect(aiBackend).not.toContain("provider_models_probe_with_key(&config, timeout_secs, key.as_deref())\n                    .unwrap_or_default()");
    expect(aiBackend).toContain("The model-list request timed out before the endpoint responded.");
    expect(aiBackend).toContain("HTTP 401 Unauthorized");
    expect(aiBackend).toContain("invalid or incompatible JSON");
    expect(aiBackend).toContain("no usable model identifiers");
    expect(providerCard.match(/onClick=\{\(\) => void refreshModels\(config\)\}/g)).toHaveLength(1);
    expect(providerCard.match(/onClick=\{\(\) => void save\(\)\}/g)).toHaveLength(1);
    expect(providerCard.match(/onClick=\{\(\) => void test\(\)\}/g)).toHaveLength(1);
  });
  ...
  it("requires the literal request review before Explain or project-summary sends", () => {
    ...
    expect(projectSummary).toContain("const previewId = disclosure.previewId;");
    expect(projectSummary).toContain("aiSummarizeProject(previewId)");
  ...
  it("keeps Connector panels in edition-gated lazy chunks without widening the Local fixture", () => {
    ...
    expect(app).toContain('const connectorFrontendBuild = import.meta.env.MODE === "test" || import.meta.env.MODE === "connector"');
    expect(app).toContain("const EditionLayer = connectorFrontendBuild");
    expect(app).toContain("const EditionSettingsPanel = connectorFrontendBuild");
    expect(app).toContain("const EditionRecapDetailLayer = connectorFrontendBuild");
    expect(app).toContain("editionExtensionActive ? (");
    expect(app).toContain("recapDetailLayer={editionExtensionActive ? EditionRecapDetailLayer : undefined}");
    expect(app).not.toMatch(/AI Assist|provider|API key|MCP|rewrite/i);
