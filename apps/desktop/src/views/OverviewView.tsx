...
import { Activity, ArrowRight, CheckCircle2, FileDiff, FileText, FolderOpen, Inbox, RefreshCw, SlidersHorizontal } from "lucide-react";
import { api } from "../api";
import { ConceptHelp } from "../BeginnerHelp";
import { sessionsSinceReview, undatedSessionsNeedReview } from "../reviewCheckpoint";
import type { AdapterSummary, DashboardSummary, FileChangeEvent, GitRepoSummary, ProjectReviewCheckpoint, ProjectSummary, SessionDiscoveryCandidate, WatcherStatus } from "../types";
...
export function OverviewView({
  ...
  onOpenFiles,
  reviewProjectGroups,
  reviewInventoryState,
  reviewInventoryRefreshing,
  reviewInventoryError,
  onRefreshReviewInventory,
  onOpenRecap,
  onOpenProjectRecap
}: {
  showFlow: boolean;
  ...
  reviewInventoryState: ReviewInventoryState;
  reviewInventoryRefreshing: boolean;
  reviewInventoryError: string | null;
  onRefreshReviewInventory: () => void;
  onOpenRecap: () => void;
  ...
  const [recentChangesRetry, setRecentChangesRetry] = useState(0);
  ...
  const reviewInventoryReady = reviewInventoryState === "fresh" || reviewInventoryState === "cached";
  ...
  useEffect(() => {
    ...
    const load = (showLoading: boolean) => {
      if (showLoading) setRecentChangesLoading(true);
      setRecentChangesError(null);
      void api.recentFileChanges(null, 20)
        .then((changes) => {
          if (current) setRecentChanges(changes);
        })
        .catch((reason: unknown) => {
          if (current) setRecentChangesError(String(reason));
        })
        .finally(() => {
          if (!current) return;
          setRecentChangesLoading(false);
          timer = window.setTimeout(() => load(false), 15_000);
        });
    };
    load(true);
    return () => {
    ...
    <>
      ...
      {showFlow ? (
        <div className="dashboard-card review-inbox" data-help="Projects with AI conversations newer than the point where you last marked them reviewed. Opening one compares those records with file changes Git can see now.">
          <div className="overview-recap-heading">
            ...
            <div>
              ...
              <div className="heading-with-help">
                <h3>{reviewInboxTitle(reviewInventoryState, reviewInventoryRefreshing, reviewInboxLoading, reviewInboxError, reviewInboxNewSessions, reviewInboxUnknownDates)}</h3>
            ...
            <div className="review-inbox-actions">
              {reviewInventoryState !== "fresh" || reviewInventoryRefreshing || reviewInventoryError ? (
                <button type="button" className="secondary-button compact" disabled={reviewInventoryRefreshing} onClick={onRefreshReviewInventory}>
                  {reviewInventoryRefreshing ? "Refreshing…" : "Find sessions"}
                </button>
              ) : null}
              {reviewInboxError ? <button type="button" className="secondary-button compact" onClick={() => setReviewInboxRetry((value) => value + 1)}>Retry checkpoints</button> : null}
            </div>
          </div>
          {reviewInventoryState === "cached" || reviewInventoryState === "unavailable" || reviewInventoryError ? (
            <div className="review-inbox-clear review-inbox-refresh" role={reviewInventoryError ? "alert" : "status"}>
              <RefreshCw size={17} />
              <span>
                <strong>{reviewInventoryState === "cached" ? "Saved session records need a fresh local check" : "The local session index is not available yet"}</strong>
                <small>{reviewInventoryError
                  ? `The last local refresh failed: ${reviewInventoryError}`
                  : "Use Find sessions to rebuild this read-only local view before relying on an empty inbox."}</small>
              </span>
            </div>
          ) : null}
          ...
          {reviewInventoryState === "fresh" && !reviewInventoryRefreshing && !reviewInboxLoading && !reviewInboxError && !reviewInventoryError && reviewInboxItems.length === 0 ? (
      ...
      {showFlow ? (
        <div className="dashboard-card" data-help="A small local, read-only timeline built from successive project scans. It stores file metadata, not file contents, and never claims which app or agent made a change.">
          <div className="overview-recap-heading">
            <Activity size={18} />
            <div>
              <span>Local change tracker</span>
              <h3>{recentChangesLoading && recentChanges.length === 0 ? "Reading the local timeline…" : `${recentChanges.length} recent file change${recentChanges.length === 1 ? "" : "s"}`}</h3>
              <p>Created, modified and removed files detected after each project's first baseline scan. App names mean “known to use this project”, not proof that the app made the change.</p>
            </div>
            <button type="button" className="secondary-button compact" disabled={recentChangesLoading} onClick={() => setRecentChangesRetry((value) => value + 1)}>
              <RefreshCw size={14} /> {recentChangesLoading ? "Refreshing…" : "Refresh"}
            </button>
          </div>
          {recentChangesError ? <p className="scan-error">Change timeline unavailable: {recentChangesError}</p> : null}
          {!recentChangesLoading && !recentChangesError && recentChanges.length === 0 ? (
            <div className="review-inbox-clear">
              <CheckCircle2 size={17} />
              <span><strong>Baseline ready; no later changes recorded yet</strong><small>The resident scanner will add entries automatically when registered project files change.</small></span>
            </div>
          ) : null}
          {recentChanges.length > 0 ? (
            <div className="review-inbox-list">
              {recentChanges.slice(0, 8).map((change) => (
                <button type="button" key={change.id} onClick={() => onOpenProject(change.projectId)}>
                  <span className="review-inbox-project">
                    <strong>{fileChangeLabel(change)} · {change.displayName}</strong>
                    <small>{change.projectName} · {fileChangeContext(change)}</small>
                  </span>
                  <span className="review-inbox-time">{formatObservedAt(change.observedAt)}</span>
                  <ArrowRight size={15} />
                </button>
              ))}
              {recentChanges.length > 8 ? <small className="review-inbox-overflow">{recentChanges.length - 8} more recent changes are retained in the local timeline.</small> : null}
            </div>
          ) : null}
        </div>
      ) : null}
      ...
      {inventoryDetailsOpen ? (
        <>
          ...
          <div className="dashboard-card" data-help="Local Detectors are built-in passive classifiers. They inspect already-scanned metadata and safe text context without executing project code.">
...
export function buildReviewInboxItems(
  ...
  return groups
    .map(({ project, sessions }): ReviewInboxItem => {
      ...
      const sessionsForReview = sessionsSinceReview(sessions, checkpoint);
      const datedForReview = checkpoint
        ? sessionsForReview.filter((session) => session.modifiedMs != null)
        : sessionsForReview;
      const unknownTimestampCount = checkpoint && undatedSessionsNeedReview(sessions, checkpoint)
        ? sessionsForReview.filter((session) => session.modifiedMs == null).length
        : 0;
...
export function reviewInboxTitle(
  inventoryState: ReviewInventoryState,
  ...
  if (inventoryState === "restoring" || (inventoryState === "unavailable" && inventoryRefreshing)) return "Restoring the local session index";
  if (inventoryState === "unavailable") return "Local session index is temporarily unavailable";
  if (inventoryRefreshing) return inventoryState === "cached"
    ? "Refreshing the saved local session index"
    : "Refreshing the local session index";
  if (checkpointLoading) return "Checking saved review checkpoints";
  if (checkpointError) return "Review checkpoints are temporarily unavailable";
  if (inventoryState === "cached" && unreviewedCount === 0 && unknownTimestampCount === 0) return "Saved session index needs a fresh local check";
  if (unreviewedCount > 0) {
    return `${unreviewedCount} new session record${unreviewedCount === 1 ? " needs" : "s need"} review${unknownTimestampCount > 0 ? ` · ${unknownTimestampCount} undated` : ""}${inventoryState === "cached" ? " · saved index" : ""}`;
  }
...
export type ReviewInventoryState = "restoring" | "fresh" | "cached" | "unavailable";
...
export function overviewInventoryHealth(dashboard: DashboardSummary | null, watcherStatus: WatcherStatus | null) {
  if (!dashboard) {
    return {
      ...
      progress: 14,
      facts: [
        { label: "Scan roots", value: 0 },
        { label: "Stale roots", value: 0 },
        { label: "Partial entries", value: 0 }
      ]
    };
  }
  ...
  if (staleRoots > 0) {
    return {
      ...
      progress,
      facts: [
        { label: "Healthy roots", value: healthyRoots },
        { label: "Stale roots", value: staleRoots },
        { label: "Partial entries", value: partialItems }
      ]
    };
  }
  ...
  if (partialItems > 0) {
    return {
      ...
      progress: 82,
      facts: [
        { label: "Scan roots", value: scanRoots },
        { label: "Stale roots", value: staleRoots },
        { label: "Partial entries", value: partialItems }
      ]
    };
...
// Dashboard metric card. Cached values paint immediately; CountUp only animates
// a later confirmed increase (for example after a completed scan).
...
function fileChangeLabel(change: FileChangeEvent) {
  if (change.changeKind === "created") return "Created";
  if (change.changeKind === "modified") return "Modified";
  if (change.changeKind === "removed") return "Removed";
  return change.changeKind.replaceAll("_", " ");
}

function fileChangeContext(change: FileChangeEvent) {
  if (change.apps.length === 0) return "No supported coding-tool evidence is currently associated with this project";
  const visible = change.apps.slice(0, 2).join(", ");
  const extra = change.apps.length > 2 ? ` +${change.apps.length - 2}` : "";
  return `Project is known to ${visible}${extra}`;
}
