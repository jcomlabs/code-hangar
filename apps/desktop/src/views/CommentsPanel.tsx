...
import { commentsPanelHint, formatCommentMeta, validateCommentBody } from "../comments";
...
const ConnectorCommentPresentation = connectorFrontendBuild
  ? lazy(() => import("./ConnectorCommentPresentation"))
  : null;

export function CommentsPanel({ nodeId, editionExtensionActive = false }: { nodeId: number | null; editionExtensionActive?: boolean }) {
  ...
  return (
    <section className="pane-section comments-panel">
      ...
      />
      <p className="comments-hint">{commentsPanelHint()}</p>
      {editionExtensionActive && ConnectorCommentPresentation ? (
        <Suspense fallback={null}>
          <ConnectorCommentPresentation mode="hint" />
        </Suspense>
      ) : null}
      ...
      {comments.length === 0 && !loading ? (
        ...
        <ul className="comment-list">
          {comments.map((comment) => {
            const localComment = comment.source === "user";
            const editing = editingId === comment.id;
            return (
              <li key={comment.id} className="comment-row">
                <div className="comment-meta">
                  {localComment ? <span>{formatCommentMeta(comment)}</span> : null}
                  {!localComment && editionExtensionActive && ConnectorCommentPresentation ? (
                    <Suspense fallback={null}>
                      <ConnectorCommentPresentation mode="meta" comment={comment} />
                    </Suspense>
                  ) : null}
                </div>
                {editing ? (
                  ...
                  <>
                    ...
                    {deletingId === comment.id ? (
                      ...
                      <button
                        ...
                        className="comment-icon-button"
                        disabled={busy}
                        data-help="Delete this comment. Your files are not affected."
