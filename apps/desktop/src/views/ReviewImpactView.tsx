...
export function ReviewImpactView({
  ...
  lastMutationMove: MutationMoveSummary | null;
  ...
  return (
    <section className="pane-section compact safe-manage">
      ...
      <div className="plan-workflow">
          ...
          <>
            ...
            <section className="plan-step" data-help="What appears to belong to the target and what is excluded from the recoverable estimate. Recoverable bytes are a conservative minimum; '+' / 'Incomplete' mean the scan still has more to count.">
              ...
              <div className="safe-manage-facts">
                ...
                <span data-help="Ready means every present project's Markdown and workflow relationship indexes were rebuilt. If incomplete, an empty dependents list is not proof that no reference exists."><small>Reference evidence</small><strong>{operationPlan.relationshipEvidenceComplete ? "Ready" : "Incomplete · blocked"}</strong></span>
              ...
              {!operationPlan.relationshipEvidenceComplete ? (
                <p className="plan-value-note">Reference evidence is still incomplete. Empty shared/dependent lists do not mean “none”; recoverable bytes remain blocked until the indexes are ready.</p>
              ) : null}
            ...
            <section className="plan-step" data-help="Guidance for what to do next. The report is evidence for you to review later — it is not an action queue and Code Hangar never executes it.">
              ...
              {mutationAvailable ? (
                <button type="button" className="secondary-button plan-recovery-link" data-help="Open Recovery & cleanup to review held allocations, final-cleanup eligibility, retained archives and restore records." onClick={showRecovery}>Open Recovery &amp; cleanup</button>
              ) : null}
            </section>
            ) : null}
          </>
        ...
        {mutationAvailable && lastMutationMove ? (
          <div className="mutation-completion" role="status" aria-live="polite">
            <CheckCircle2 size={18} aria-hidden="true" />
            <div>
              <strong>{lastMutationMove.moved} supported original item{lastMutationMove.moved === 1 ? "" : "s"} moved to recovery holding</strong>
              <span>
                {lastMutationMove.skipped} skipped · {lastMutationMove.failed} failed. Skipped, failed or blocked links and objects remain at their original paths. Held allocation remains until Finish removing; the verified backup/archive is retained separately.
              </span>
            </div>
            <button type="button" className="secondary-button compact" onClick={showRecovery}>Open Recovery &amp; cleanup</button>
          </div>
        ) : null}
          ...
          <details className="plan-step safe-manage-actions">
            ...
            <div className="safe-manage-actions-body">
              ...
              <div className="mutation-controls">
                ...
                <p className="check-row-note" data-help="Move to recovery changes only supported objects in the confirmed preview. Supported originals move out of the source into recovery holding. Anything blocked, skipped or failed—including unsupported links—remains at its original path. Holding continues to use disk allocation until the separate Finish removing review. The verified backup/archive is a separate retained allocation.">
                  Supported originals move to recovery holding. Blocked, skipped or failed links and objects remain at their original paths. Holding keeps using disk space until <strong>Finish removing</strong>; the verified backup/archive is retained separately.
                </p>
              </div>
              ...
              <p className="mutation-footnote" data-help="Held objects remain restorable and keep using their holding-volume allocation. Finish removing is a separate project or batch review that deletes only object-archive-v2-eligible held copies; blocked objects stay held and recovery archives remain separately retained.">
                Restore or <strong>Finish removing</strong> held objects in <strong>Recovery &amp; cleanup</strong>. Final cleanup is separate, keeps blocked objects held and retains recovery archives.
              </p>
            </div>
          </details>
        ) : null}
        {mutationMessage ? <p className="mutation-message" role="status" aria-live="polite" data-help="Latest disk-action status message.">{mutationMessage}</p> : null}
      </div>
    </section>
  );
}
