...
import { AlertTriangle, Eye, FileDiff, FileText, Files, Folder, FolderOpen, HardDrive, Home, LockKeyhole, LockOpen, MessageSquare, Network, Pencil, Pin, PinOff, RefreshCcw, SlidersHorizontal, TerminalSquare } from "lucide-react";
...
export function projectSelectionRequiredCopy(view: "space" | "connections" | "sessions") {
  const labels = {
    space: "space inventory",
    connections: "connection map",
    sessions: "linked local sessions"
  } as const;
  return `Choose a project before opening its ${labels[view]}.`;
}
...
/** A direct Explorer/file-association preview is already useful even while its
 * project tree is still loading. Never replace that document with the project
 * spinner or a tree-load error; those states belong beside the preview. */
export function projectLoadOverlayStatus(
  loadStatus: "idle" | "loading" | "ready" | "error",
  projectView: ProjectView,
  hasPreview: boolean
) {
  if (projectView === "files" && hasPreview) return null;
  return loadStatus === "loading" || loadStatus === "error" ? loadStatus : null;
}
...
export function NarrowProjectEscape({
  onOverview,
  onAddProjects
}: {
  onOverview: () => void;
  ...
  return (
    <section className="narrow-project-escape" aria-label="Project navigation options">
      <div>
        <strong>No project selected</strong>
        <span>The project sidebar is hidden at this window width.</span>
      </div>
      <div className="narrow-project-escape-actions">
        <button className="secondary-button compact" type="button" onClick={onOverview}>
          <Home size={15} aria-hidden="true" />
          Overview
        </button>
        <button className="action-button compact" type="button" onClick={onAddProjects}>
...
export function ProjectCenterView({
  ...
  showSelectedTextMenu: (selectedText: string, event: MouseEvent<HTMLElement>) => void;
  ...
  relationshipsMembership: string | null;
  ...
  onOverview: () => void;
  onAddProjects: () => void;
  /** Enables presentation supplied only by an edition-specific lazy layer. */
  editionExtensionActive?: boolean;
  ...
  const loadOverlayStatus = projectLoadOverlayStatus(loadStatus, projectView, Boolean(preview));
  return (
    <>
      ...
      <div className={`viewer-header ${projectView === "files" ? "" : "project-viewer-crumb"}`}>
        {!selectedProject ? (
          <NarrowProjectEscape onOverview={onOverview} onAddProjects={onAddProjects} />
        ) : null}
        <div>
        ...
        {projectView === "files" && hasPreview ? <div className="viewer-actions">
          ...
          <div className="viewer-mode-actions" role="group" aria-label="File view mode">
          ...
          {editor.available && !changesUnlocked ? (
            <button className="toolbar-button viewer-lock-action changes-locked" type="button" onClick={onRequestChangeAccess} data-help="Project files start locked. Read the safety warning and explicitly unlock this project before any edit controls become available.">
              <LockKeyhole size={15} />
              Changes locked
            </button>
          ) : null}
          {editor.available && changesUnlocked ? (
            <button className="toolbar-button viewer-lock-action changes-unlocked" type="button" onClick={onRelockChanges} data-help="Lock project-file changes again. This closes the edit controls without changing the project.">
              <LockOpen size={15} />
              Lock changes
            </button>
          ) : null}
          ...
          >
            {projectView === "context" ? (
              <ProjectContextHome
                ...
                onOpen={(nodeId, projectId) => void openNodeInTree(nodeId, projectId)}
            ...
            ) : projectView === "space" && selectedProject ? (
              <ProjectSpaceHome
                project={selectedProject}
              ...
              <div className="empty-state">{projectSelectionRequiredCopy("space")}</div>
            ) : projectView === "connections" && selectedProject ? (
              ...
              <div className="empty-state">{projectSelectionRequiredCopy("connections")}</div>
            ) : projectView === "sessions" && selectedProject ? (
              <ProjectSessionsHome
                ...
                onFindSessions={onFindSessions}
              />
            ) : projectView === "sessions" ? (
              <div className="empty-state">{projectSelectionRequiredCopy("sessions")}</div>
            ) : projectView === "files" && !selectedProject ? (
              <div className="empty-state">Choose or add a project before browsing its files.</div>
            ) : preview ? (
              <PreviewPane preview={preview} onReveal={revealPreview} canReveal={zoneAllowSensitiveReveal} onOpenProtectedSettings={openProtectedSettings} editing={changesUnlocked && previewMode === "edit"} valuesEditing={changesUnlocked && previewMode === "values"} changeAuthorized={changesUnlocked} editor={editor} onFileMutated={onFileMutated} setStatusText={setStatusText} onOpenRecap={() => setProjectView("recap")} onSelectedTextContextMenu={showSelectedTextMenu} />
            ) : folderExplanation ? (
