...
import { confirmExactAiSend, connectorApi as api, formatAiCredentialUse } from "../connectorApi";
import { clearAiTask, hashSnippet, startAiStreamingTask, useAiTask } from "../aiTasks";
import type { AiExplainPreview, AiLocalProviderCandidate, AiProviderConfig, AiProviderFormat, AiProviderMode, AiSendDisclosure } from "../connectorTypes";
...
export const AiExplainPanel = memo(function AiExplainPanel({
  ...
  const refreshModels = async () => {
    ...
    setError(null);
    try {
      const disclosure = await api.aiProviderModelsDisclosure(
        config.mode,
        config.baseUrl,
        config.model,
        config.format
      );
      if (!confirmExactAiSend(disclosure)) return;
      const list = await api.aiProviderModels(disclosure.previewId);
  ...
  const send = () => {
    ...
    startAiStreamingTask(key, lens, title, subtitle, async (onDelta) => {
      const text = await api.aiReadStream(disclosure.previewId, onDelta);
        ...
        {!blocked && config && (target.kind === "text" || preview) ? (
          <>
            ...
            <div className="ai-controls">
              ...
              <button type="button" className="action-button subtle" disabled={providerOff} onClick={() => void refreshModels()} data-help="Ask the configured endpoint for its model list to fill the Model suggestions. This is the only request made before you press Explain, and only when you click.">
                Refresh models
              </button>
            </div>
            ...
            {disclosure ? (
              <section className="ai-send-disclosure" aria-label="Exact AI request">
                ...
                <dl>
                  ...
                  <div><dt>Credential</dt><dd>{formatAiCredentialUse(disclosure.credentialUse)}</dd></div>
                ...
                <small className="muted">The saved credential value stays hidden inside Rust. Its exact use is disclosed above. Send consumes this exact frozen request once; if it expires or anything changes, Code Hangar requires a new review.</small>
              </section>
            ) : null}
          </>
        ) : null}
...
export const AiAssistKeyCard = memo(function AiAssistKeyCard() {
  ...
  // Local backend reads only — no mount-time model fetch. Save and Test each perform
  // only their named request; model listing happens only through Refresh models.
  useEffect(() => {
  ...
  // Explicit model dropdown request for the given draft config — read-only, never persists.
  const refreshModels = async (cfg: AiProviderConfig) => {
    if (cfg.mode === "off") {
      setModels([]);
      return;
    }
    ...
    try {
      ...
      const list = await api.aiProviderModels(disclosure.previewId);
      setModels([...new Set(list)]);
      setMessage(`Loaded ${list.length} model ${list.length === 1 ? "identifier" : "identifiers"}.`);
    } catch (err) {
      setModels([]);
      setMessage(err instanceof Error ? err.message : String(err));
    }
  ...
  const test = async () => {
    ...
    try {
      ...
      const disclosure = await api.aiProviderTestDisclosure(
        config.mode,
        config.baseUrl.trim(),
        config.model.trim(),
        config.format
      );
      if (!confirmExactAiSend(disclosure)) {
        setMessage("Provider test cancelled. Nothing was sent.");
        return;
      }
      const result = await api.aiProviderTest(disclosure.previewId);
      setMessage(`✓ ${result}`);
    } catch (err) {
      setMessage(err instanceof Error ? err.message : String(err));
    } finally {
  ...
  return (
    ...
    >
      ...
      <p className="muted help-copy">
        <strong>Experimental and less tested than the local tools.</strong> Leave this Off, use a model running on this computer, or connect an API with your own key. Nothing is sent until you approve the exact destination and request body.
      </p>
      ...
      {showEndpoint ? (
        <>
          ...
          <div className="ai-field">
            ...
            <button type="button" className="secondary-button" disabled={busy} onClick={() => void refreshModels(config)} data-help="Fetch the endpoint's model list into the suggestions above. Nothing is sent besides that request, and only when you click.">
              Refresh models
            </button>
          </div>
          </div>
