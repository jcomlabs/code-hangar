...
import { confirmExactAiSend, connectorApi as api } from "../connectorApi";
import type {
  AiExplainPreview,
  AiFollowUpResult,
  AiGlossaryEntry,
...
export const AiLearningTools = memo(function AiLearningTools({
  ...
  const sendWalkthrough = async () => {
    ...
    setWalkthroughError(null);
    try {
      const disclosure = await api.aiWalkthroughDisclosure(target.nodeId, selectedSections, level, model);
      if (!confirmExactAiSend(disclosure)) {
        setWalkthroughError("Cancelled. Nothing was sent.");
        return;
      }
      setWalkthroughResult(await api.aiWalkthroughFile(target.nodeId, selectedSections, level, model, disclosure.previewId));
    } catch (error) {
      setWalkthroughError(error instanceof Error ? error.message : String(error));
    } finally {
      setWalkthroughBusy(false);
  ...
  const sendFollowUp = async () => {
    ...
    setFollowError(null);
    try {
      const question = followQuestion.trim();
      const disclosure = await api.aiFollowUpDisclosure(
        target.nodeId,
        followSection,
        conversationId,
        question,
      ...
      if (!confirmExactAiSend(disclosure)) {
        setFollowError("Cancelled. Nothing was sent.");
        return;
      }
