...
import { confirmExactAiSend, connectorApi } from "../connectorApi";
import type { AiExplainPreview, AiProviderMode } from "../connectorTypes";
import type { RecapAiSourceMode, SessionChangeSet } from "../types";
...
export const RecapAiLayer = memo(function RecapAiLayer({
  ...
  async function runGuide() {
    ...
    setResult(null);
    try {
      const lens = mode === "story" ? "narration" : mode === "check" ? "review" : "learning";
      const disclosure = await connectorApi.aiChangeDisclosure(
        projectId,
        sessionPaths,
        sourceMode,
        lens,
        mode === "learn" ? selection?.filePath ?? null : null,
        mode === "learn" ? selection?.editIndex ?? null : null,
        "vibe",
        ""
      );
      ...
      const next = mode === "story"
        ? await connectorApi.aiNarrateSessionChanges(projectId, sessionPaths, sourceMode, "vibe", "", disclosure.previewId)
        : mode === "check"
          ? await connectorApi.aiReviewChangeSet(projectId, sessionPaths, sourceMode, "vibe", "", disclosure.previewId)
          : selection
            ? await connectorApi.aiExplainChange(projectId, sessionPaths, sourceMode, selection.filePath, selection.editIndex, "vibe", "", disclosure.previewId)
            : null;
