...
/** Open one file using its full project membership. Returning false keeps
 * directories and placeholder rows from accidentally entering file preview. */
export function openTreeFile(
  item: Pick<NavItem, "itemKind" | "nodeId" | "projectId">,
  openNode: (nodeId: number, projectId: number) => void
) {
  if (item.itemKind === "directory" || !item.nodeId) return false;
  openNode(item.nodeId, item.projectId);
  return true;
}
    ...
    <div className="file-tree">
      <div className="file-tree-header">
        ...
        <div className="tree-toolbar" aria-label="File tree display and sort controls">
          <details className="tree-view-options">
            <summary data-help="Choose whether the loaded tree shows folders, files or both.">
              <Files size={13} /> Visible
            </summary>
            <div className="tree-view-options-popover">
              <button className={treeShowFolders ? "active" : ""} type="button" aria-pressed={treeShowFolders} onClick={() => setTreeShowFolders((value) => !value)} data-help="Show or hide folders in the file tree. Enabled by default.">
                <Folder size={13} /> Folders
              </button>
              <button className={treeShowFiles ? "active" : ""} type="button" aria-pressed={treeShowFiles} onClick={() => setTreeShowFiles((value) => !value)} data-help="Show or hide files in the file tree. Enabled by default.">
                <Files size={13} /> Files
              </button>
            </div>
          </details>
...
const TreeNode = memo(function TreeNode({
  ...
  const cloudHelp = item.cloudOnly
    ? " Cloud-only: metadata is visible, but content was not opened or indexed and no download was requested."
    : "";
  const rowHelp = item.itemKind === "directory"
    ...
    : `File tree file ${item.displayName}.${cloudHelp} ${fileVisual?.previewable && !item.cloudOnly ? "Usually viewable as text inside Code Hangar." : "May need the Windows default app if Code Hangar cannot preview it."} Right-click for safe actions. Path: ${item.displayPath || item.path}.`;
    ...
    <div className="tree-node">
      <div className={`tree-row ${item.nodeId === activeNodeId ? "selected" : ""} ${showProtectedMetadata && (item.isSensitive || item.protectedLevel) ? "protected-visible" : ""}`} data-tree-node-id={item.nodeId ?? undefined} data-help={rowHelp} onContextMenu={(event) => onContextMenu(item, event)}>
        ...
        <span className="tree-meta">
          {item.cloudOnly ? (
            <span className="tree-cloud-only" title="Cloud-only · content not indexed · zero download requested">
              <CloudOff size={12} /> Cloud-only
            </span>
          ) : null}
