...
function ProjectSummaryCard({ projectId, editionExtensionActive }: { projectId: number; editionExtensionActive: boolean }) {
  ...
  return (
    <section className="project-home-section project-summary-card" data-help="A quick read of what this project is, based on its README and manifest files.">
...
export function ProjectContextHome({
  ...
  onOpen: (nodeId: number, projectId: number) => void;
    ...
    return (
      <div className="project-home">
        <div className="project-choice-empty">
          ...
          <p>Choose a known project from the project list or Quick Open. If none is available, add one from the top bar.</p>
  ...
  return (
    <div className="project-home">
      <div className="project-home-intro">
        ...
        <h2>{project.name}</h2>
        <p>Read the files most likely to describe this project first. The full file inventory stays one click away under Files.</p>
      </div>
      ...
      <ProjectSummaryCard key={project.id} projectId={project.id} editionExtensionActive={editionExtensionActive} />
      {overlapWarning ? (
      ...
      <CommentsPanel nodeId={project?.id ?? null} editionExtensionActive={editionExtensionActive} />
    </div>
  );
}
...
export function ProjectConnectionsHome({
  ...
  const relationshipsReady = preview !== null
    && relationshipsMembership === `${preview.projectId}:${preview.nodeId}`
    && relationships !== null;
          ...
          {graphMap.issues.length ? (
            <div className="connection-list graph-issue-list">
              {shownIssues.map((issue, index) => (
                <button type="button" key={`${issue.projectId ?? graphMap.projectId}:${issue.nodeId}-${issue.kind}-${issue.target}-${index}`} onClick={() => onOpen(issue.nodeId, issue.projectId ?? graphMap.projectId)} onContextMenu={(event) => {
                ...
                <details className="graph-cache-group">
                  ...
                  <div className="connection-list">
                    {shownCacheIssues.map((issue, index) => (
                      <button type="button" key={`${issue.projectId ?? graphMap.projectId}:${issue.nodeId}-${issue.kind}-${issue.target}-cache-${index}`} onClick={() => onOpen(issue.nodeId, issue.projectId ?? graphMap.projectId)} onContextMenu={(event) => {
                        const node = graphNodeById.get(issue.nodeId);
                        onContextMenu({ nodeId: issue.nodeId, projectId: issue.projectId ?? node?.projectId, path: graphIssueContextPath(issue, node), label: node?.displayName ?? issue.target, itemKind: node?.itemKind ?? "file" }, event);
                      }} data-help={`${graphIssueActionHelp(issue.kind, issue.target)} ${graphIssueHelp(issue.kind)} Right-click for more tools.`}>
                        <AlertTriangle size={15} />
    ...
    <div className="project-home" ref={rootRef}>
      ...
      {sessions.length === 0 ? (
        <div className="project-session-empty">
          <strong>No linked local sessions found yet</strong>
          <span>Refresh the read-only local conversation map for this project and the other registered projects.</span>
          <button type="button" className="action-button" onClick={onFindSessions}>
            <Search size={14} /> Find local sessions
          </button>
        </div>
