...
import { displayAppText, sourceAppMetas } from "../app-meta";
import { markdownToPlainText } from "../markdown";
import { DOCUMENT_SEARCH_MATCH_EXPLANATION, DOCUMENT_SEARCH_MAX_RESULTS, documentSearchQueryError, projectPickerInputStatus, type DocumentSearchScope, type ProjectPickerResolution } from "../documentSearch";
...
const DOCUMENT_SEARCH_DEFAULT_LIMIT = 10;

export function shouldShowLostPresetPicker(presets: readonly unknown[]) {
...
export interface DocumentSearchRunOverrides {
...
export function documentAdvancedFilterCount(pathFilter: string, nameFilter: string, limit: number) {
  return Number(Boolean(pathFilter.trim()))
    + Number(Boolean(nameFilter.trim()))
    + Number(limit !== DOCUMENT_SEARCH_DEFAULT_LIMIT);
}
        ...
        <>
          ...
          <div className="orphan-list discovery-candidate-list">
            ...
            {visibleCandidates.map((candidate) => {
              ...
              const providerMetas = sourceAppMetas(candidate.sourceKinds);
              return (
                    ...
                    <div className="signal-chip-list">
                      {providerMetas.map((meta) => (
                        <span
                          ...
                          key={`${candidate.path}-provider-${meta.slug}`}
                          title={`${meta.label} project/session evidence`}
                        >
                          {meta.label}
                        </span>
                      ))}
...
export function DiscoverSearchView({
  ...
  documentScope,
  setDocumentScope,
  documentProjectInput,
  setDocumentProjectInput,
  documentProjectResolution,
  documentKind,
  setDocumentKind,
  documentPathFilter,
  ...
  documentScope: DocumentSearchScope;
  setDocumentScope: (value: DocumentSearchScope) => void;
  documentProjectInput: string;
  setDocumentProjectInput: (value: string) => void;
  documentProjectResolution: ProjectPickerResolution;
  ...
  runDocumentSearch: (overrides?: DocumentSearchRunOverrides) => void;
  ...
  openDocumentHit: (nodeId: number, projectId: number) => void;
  ...
  const currentScopeNeedsProject = !canRunDocumentSearch(documentScope, selectedProjectId, projects);
  const typedProjectNeedsResolution = documentScope === "project" && documentProjectResolution.kind !== "resolved";
  const scopeNeedsProject = currentScopeNeedsProject || typedProjectNeedsResolution;
  const queryError = documentSearchQueryError(documentQuery);
  const projectPickerStatus = documentScope === "project" ? projectPickerInputStatus(documentProjectResolution) : null;
  const safeManageProject = documentSafeManageProject(
    documentScope,
    selectedProjectId,
    documentProjectResolution.kind === "resolved" ? documentProjectResolution.project.id : null,
    projects
  );
  const showDocumentResults = documentSearchRan && !scopeNeedsProject;
  const searchEnabled = canSubmitDocumentSearch(documentQuery, documentSearching, scopeNeedsProject, queryError);
  const advancedFilterCount = documentAdvancedFilterCount(documentPathFilter, documentNameFilter, documentLimit);
  ...
  return (
    <section className="pane-section">
      <form className="filter-panel" aria-busy={documentSearching} data-help="Document Search runs only when you press Search. It searches indexed non-sensitive content." onSubmit={(event) => {
        ...
        <input autoFocus className="search-input" type="search" value={documentQuery} onChange={(event) => setDocumentQuery(event.target.value)} placeholder="Content keywords…" aria-label="Content keywords" data-help="Search indexed document body text. Sensitive, protected and non-indexed file bodies are excluded." />
        <div className="filter-grid">
          <label data-help="Search the active project, type another project by name or path, or search every indexed project.">
            Where to search
            <select value={documentScope} onChange={(event) => setDocumentScope(event.target.value as DocumentSearchScope)}>
              <option value="current">{currentProjectLabel}</option>
              <option value="project">Choose project by name…</option>
              <option value="all">All projects</option>
            </select>
          </label>
          {documentScope === "project" ? (
            <label className="document-project-picker" data-help="Choose the project scope by its exact name, alias or local path. This field does not search file contents.">
              Project name or path
              <input
                type="search"
                list="document-search-projects"
                value={documentProjectInput}
                onChange={(event) => setDocumentProjectInput(event.target.value)}
                placeholder="Type a project name…"
                aria-describedby="document-project-picker-status"
                aria-invalid={documentProjectResolution.kind === "none" || documentProjectResolution.kind === "ambiguous"}
              />
              <datalist id="document-search-projects">
                {projects.map((project) => (
                  <option
                    key={project.id}
                    value={project.path}
                    label={`${project.name} — ${displayLocalPath(project.path)}`}
                  />
                ))}
              </datalist>
            </label>
        ...
        <details
          className="advanced-filter-details"
          open={advancedFiltersOpen}
          onToggle={(event) => setAdvancedFiltersOpen(event.currentTarget.open)}
        >
          <summary data-help="Optionally narrow results by path or file name, or change the maximum number returned.">
            More filters
            {advancedFilterCount > 0 ? (
              <>
                {" "}
                <span className="signal-chip" aria-label={`${advancedFilterCount} advanced filter${advancedFilterCount === 1 ? "" : "s"} active`}>
          ...
          <div className="filter-grid">
            ...
            <label data-help={`Cap the returned rows. Document Search always stops at ${DOCUMENT_SEARCH_MAX_RESULTS} results.`}>
              Limit
              <select value={documentLimit} onChange={(event) => setDocumentLimit(Number(event.target.value))}>
                ...
                <option value={DOCUMENT_SEARCH_MAX_RESULTS}>{DOCUMENT_SEARCH_MAX_RESULTS} (maximum)</option>
              </select>
            </label>
          </div>
        </details>
        <p className="muted filter-note">{DOCUMENT_SEARCH_MATCH_EXPLANATION}</p>
        ...
        {documentScope === "project" ? (
          <p id="document-project-picker-status" className={typedProjectNeedsResolution ? "warning-inline" : "muted filter-note"} role={typedProjectNeedsResolution ? "status" : undefined}>
            {documentProjectResolution.kind === "resolved"
              ? `Searching only ${documentProjectResolution.project.name} · ${displayLocalPath(documentProjectResolution.project.path)}. The content keywords stay in the field above.`
              : projectPickerStatus}
          </p>
        ) : null}
        {queryError ? <p className="warning-inline" role="alert">{queryError}</p> : null}
        <button className="action-button" type="submit" disabled={!searchEnabled} data-help={scopeNeedsProject ? "Choose one unambiguous project first, or choose All projects under Where to search." : queryError ?? (documentQuery.trim().length < 2 ? "Type at least two characters before searching." : "Run this bounded document search once with the selected filters.")}>{scopeNeedsProject ? "Choose project first" : queryError ? "Shorten search" : documentSearching ? "Searching..." : documentQuery.trim().length < 2 ? "Type 2+ characters" : "Search"}</button>
        {documentSearchError ? <p className="warning-inline" role="alert">{documentSearchError}</p> : null}
      </form>
      {showDocumentResults && !documentSearching ? <p className="muted result-empty">Showing {documentHits.length}{documentSearchTruncated ? "+" : ""} result{documentHits.length === 1 ? "" : "s"}{documentSearchDuration != null ? ` in ${documentSearchDuration} ms` : ""}.</p> : null}
      <div className="search-hits">
        ...
        {showDocumentResults && !documentSearching && documentHits.length === 0 ? (
          <div className="empty-action-panel">
            <div className="empty-action-copy">
              <strong>No indexed documents match this search.</strong>
              <span>Widen one active filter below, or change the content keywords above.</span>
            </div>
            <div className="empty-action-buttons">
              {documentKind !== "all" ? (
                <button type="button" className="secondary-button compact" onClick={() => {
                  setDocumentKind("all");
                  void runDocumentSearch({ kind: "all" });
                }}>Search all indexed file types</button>
              ) : null}
              {documentScope !== "all" ? (
                <button type="button" className="secondary-button compact" onClick={() => {
                  setDocumentScope("all");
                  void runDocumentSearch({ scope: "all" });
                }}>Search all projects</button>
              ) : null}
              {documentPathFilter.trim() || documentNameFilter.trim() ? (
                <button type="button" className="secondary-button compact" onClick={() => {
                  setDocumentPathFilter("");
                  setDocumentNameFilter("");
                  void runDocumentSearch({ clearAdvanced: true });
                }}>Clear extra filters and search again</button>
              ) : null}
            </div>
          </div>
        ) : null}
        {showDocumentResults && !documentSearching ? documentHits.map((hit) => {
          ...
          return (
            <button className="hit-row document-hit-row" key={`${hit.projectId}:${hit.nodeId}`} type="button" data-help={`Open search result ${hit.title} at ${pathLabel}. Right-click for safe file actions.`} onClick={() => openDocumentHit(hit.nodeId, hit.projectId)} onContextMenu={(event) => showFileMenu({ nodeId: hit.nodeId, projectId: hit.projectId, path: hit.path, label: hit.title }, event)}>
      ...
      {documentSearchRan && safeManageProject ? (
        <div className="project-next-step" data-help="After discovery, open Safe Manage for a read-only local review of the selected project.">
          <div>
            ...
            <span>Review {safeManageProject.name} for ownership, references, protection and scan gaps.</span>
          </div>
...
export function canRunDocumentSearch(
  scope: DocumentSearchScope,
  selectedProjectId: number | null,
  projects?: ReadonlyArray<Pick<ProjectSummary, "id">>
) {
  if (scope === "project") return true;
  if (scope === "current" && projects && !projects.some((project) => project.id === selectedProjectId)) return false;
  return canRunCurrentProjectScope(scope, selectedProjectId);
}

export function canSubmitDocumentSearch(query: string, searching: boolean, currentScopeNeedsProject: boolean, queryError = documentSearchQueryError(query)) {
  return query.trim().length >= 2 && !queryError && !searching && !currentScopeNeedsProject;
}

export function documentSafeManageProject<T extends Pick<ProjectSummary, "id">>(
  scope: DocumentSearchScope,
  selectedProjectId: number | null,
  typedProjectId: number | null,
  projects: readonly T[]
): T | null {
  const scopedProjectId = scope === "current"
    ? selectedProjectId
    : scope === "project"
      ? typedProjectId
      : null;
  // Safe Manage acts on the active sidebar project. A global search has no one
  // owner, and a typed search must not silently send the user to another project.
  if (scopedProjectId == null || scopedProjectId !== selectedProjectId) return null;
  return projects.find((project) => project.id === scopedProjectId) ?? null;
}
...
export function DiscoverOrphansView({
  ...
  openNode: (nodeId: number, projectId: number) => void;
  showFileMenu: (target: FileMenuTarget, event: MouseEvent<HTMLElement>) => void;
  projects: ProjectSummary[];
  selectedProjectId: number | null;
}) {
        ...
        <div className="filter-grid">
          ...
          {orphanMode === "lost" ? (
            <>
              ...
              {shouldShowLostPresetPicker(savedLostPresets) ? (
      ...
      {orphanMode === "lost" ? (
        ...
        <div className="orphan-list">
          ...
          {orphanCandidates?.candidates.map((candidate) => (
            <div className="orphan-row" key={`${candidate.projectId}:${candidate.nodeId}`}>
              <button className="orphan-row-main" type="button" onClick={() => void openNode(candidate.nodeId, candidate.projectId)} onContextMenu={(event) => showFileMenu({ nodeId: candidate.nodeId, projectId: candidate.projectId, path: candidate.path, label: candidate.displayName }, event)} data-help={`Open unreferenced asset candidate ${candidate.displayName} for metadata review. Right-click for safe file actions.`}>
                <strong>{candidate.displayName}</strong>
                <span className="orphan-row-facts" aria-label="Candidate facts">
...
export function duplicateConfirmationGroupKey(
  ...
  return `${group.sizeBytes}:${group.hashPartial}:${group.members
    .map((member) => `${member.projectId}:${member.nodeId}`)
      ...
      <div className="duplicate-group" key={`${group.id}-${group.hashPartial}`} data-help={`${group.memberCount} files have the same size and matching sampled content. Confirm reads the complete files to check whether every byte is identical.`}>
        ...
        <div className="duplicate-member-list">
          {group.members.map((member) => (
            <button className="duplicate-member-row" type="button" key={`${member.projectId}:${member.nodeId}`} onClick={() => void openNode(member.nodeId, member.projectId)} onContextMenu={(event) => showFileMenu({ nodeId: member.nodeId, projectId: member.projectId, path: member.path, label: member.displayName }, event)} data-help={`Open duplicate candidate ${member.displayName} for metadata review. Right-click for safe file actions.`}>
