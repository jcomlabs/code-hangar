...
import { AlertTriangle, CheckCircle2, Download, FileDiff, GitBranch, History, MessageSquare, RefreshCcw, ShieldCheck } from "lucide-react";
...
export function reviewCheckpointSavedMessage(reviewedAt: string) {
  return `Review point saved for ${new Date(reviewedAt).toLocaleString()}. Future “New since I last reviewed” views start after this point.`;
}
...
export const RecapView = memo(function RecapView({
  ...
  const scopedSessions = useMemo(
    () => scope === "all" || !checkpoint
      ? orderedSessions
      : sessionsSinceReview(orderedSessions, checkpoint),
  ...
  async function markReviewed() {
    ...
    try {
      ...
      const next = await api.markProjectReviewed(
        projectId,
        Math.max(Date.now(), latestSession),
        undatedSessionFingerprint(orderedSessions)
      );
      setCheckpoint(next);
      setScope("since");
      setSelectedKey("combined");
      setReviewStatus(reviewCheckpointSavedMessage(next.reviewedAt));
  ...
  return (
    <div className="project-home recap-home">
      <div className="project-home-intro recap-intro">
        ...
        {reviewStatus ? <small className="recap-receipt-status" role="status" aria-live="polite">{reviewStatus}</small> : null}
        ...
        <section className="recap-detail" aria-live="polite">
          ...
          {loading ? (
            ...
            DetailLayer && (selectedKey === "combined" || selectedKey === "git" || selectedSession)
              ? <Suspense fallback={<div className="recap-state"><RefreshCcw className="spin" size={20} /><strong>Preparing optional AI tools</strong></div>}>
                  <DetailLayer
