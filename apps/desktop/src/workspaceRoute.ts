...
export type PrimaryView = "project" | "overview" | "discover" | "safe_manage" | "review" | "recovery" | "settings";
...
/** Stable identity for one indexed file in navigation history. A physical node
 * may belong to overlapping projects, so nodeId alone is not sufficient. */
export interface FileHistoryEntry {
  nodeId: number;
  projectId: number;
}

/** A physical node can be inventoried by more than one overlapping project.
 * Any UI cache derived from navigation context must therefore use both ids. */
export function fileMembershipKey(projectId: number, nodeId: number) {
  return `${projectId}:${nodeId}`;
}

export function sameFileHistoryEntry(
  left: Pick<FileHistoryEntry, "nodeId" | "projectId">,
  right: Pick<FileHistoryEntry, "nodeId" | "projectId">
) {
  return left.nodeId === right.nodeId && left.projectId === right.projectId;
}

export function shouldRecordFileHistoryEntry(
  current: Pick<FileHistoryEntry, "nodeId" | "projectId"> | null,
  next: Pick<FileHistoryEntry, "nodeId" | "projectId">
) {
  return current !== null
    && current.nodeId > 0
    && current.projectId > 0
    && !sameFileHistoryEntry(current, next);
}

export function appendFileHistoryEntry(
  stack: FileHistoryEntry[],
  current: Pick<FileHistoryEntry, "nodeId" | "projectId"> | null,
  next: Pick<FileHistoryEntry, "nodeId" | "projectId">,
  limit = 50
) {
  if (!current || !shouldRecordFileHistoryEntry(current, next)) return stack;
  const boundedLimit = Math.max(0, Math.floor(limit));
  if (boundedLimit === 0) return [];
  return [
    ...stack,
    { nodeId: current.nodeId, projectId: current.projectId }
  ].slice(-boundedLimit);
}
...
const PRIMARY_VIEWS: readonly PrimaryView[] = ["project", "overview", "discover", "safe_manage", "review", "recovery", "settings"];
...
export function workspaceRouteStatusText(route: { primaryView: PrimaryView; projectView?: ProjectView; discoverView?: DiscoverView; settingsView?: SettingsView }) {
  switch (route.primaryView) {
    ...
    case "safe_manage":
      return "Safe Manage portfolio opened.";
    case "recovery":
