...
// Order matters: the first entry whose normalized whole-token/phrase match appears
// in the raw value wins. Keep the most specific apps before generic ones.
const KNOWN_APPS: KnownApp[] = [
  { slug: "opencode", label: "OpenCode", match: ["opencode"] },
  { slug: "omp", label: "Oh My Pi", match: ["oh my pi", "omp"] },
...
export function appMeta(raw: string | null | undefined): AppMeta {
  if (!raw) return OTHER;
  // Provider ids arrive as labels, snake_case source kinds and slash-separated
  // combined labels. Match whole normalized tokens/phrases: bare `includes("omp")`
  // used to classify unrelated words such as "composer" as Oh My Pi.
  const normalized = ` ${raw.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim()} `;
  for (const app of KNOWN_APPS) {
    if (app.match.some((needle) => normalized.includes(` ${needle.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim()} `))) {
...
/** Known provider identities represented by discovery source kinds, deduped in source order. */
export function sourceAppMetas(sourceKinds: readonly string[]): AppMeta[] {
  const map = new Map<string, AppMeta>();
  for (const sourceKind of sourceKinds) {
    const meta = appMeta(sourceKind);
    if (meta.slug !== OTHER.slug && !map.has(meta.slug)) map.set(meta.slug, meta);
  }
  return [...map.values()];
}
