...
The Code Hangar **Local edition** is local-only by design. The separately
installed, opt-in **AI Connector edition** is the sole edition-aware exception
described below; it does not change the Local artifact or dependency graph.

The Local edition does not read online documentation, fetch remote repository
previews, call GitHub APIs, call package registries, download adapters, upload
telemetry, call provider APIs, send project contents anywhere, or perform remote
Git operations. The Connector edition may send only the exact bounded,
redacted provider payload that the user selected, inspected and explicitly
confirmed. It adds no telemetry, updater, remote Git, registry, documentation or
background network path.
...
The Local edition has no agent or provider input surface. The Connector edition
may accept local agent input through explicitly enabled IPC/MCP and the response
to an exact user-reviewed provider request. Both are treated as untrusted,
evidence-bounded input. Neither agent nor provider output overrides safety
policy, records an owner decision or authorizes mutation.

The base build must have no outbound network capability. Local IPC for agents
and provider access must be behind the explicit Connector feature/build boundary
and must not exist in the base build. Connector AI recommendations are advisory:
they may differ from the deterministic Safe Manage baseline, but they never
record a user decision or authorize an action.
